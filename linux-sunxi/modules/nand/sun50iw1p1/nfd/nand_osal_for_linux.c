...
#define  NAND_DRV_VERSION_1		0x5025
#define  NAND_DRV_DATE			0x20190130
...
int NAND_SetClk(__u32 nand_index, __u32 nand_clk0, __u32 nand_clk1)
{
	...
	if (nand_index == 0) {
		...
		if (clk_set_rate(nand0_clk, rate))
			printk("%s: set nand0_clk to %dHZ failed! nand_clk: 0x%x\n",
...
int NAND_GetVoltage(void)
{
	...
	if (IS_ERR(regu1))
		...
		if (IS_ERR(regu1)) {
			printk
			    ("nand:some err happen,fail to enable regulator vcc-nand!\n");
			return -1;
		}
	...
	if (IS_ERR(regu2))
		...
		if (IS_ERR(regu2)) {
			printk
			    ("nand:some err happen,fail to enable regulator vcc-io!\n");
			return -1;
		}
...
int NAND_ReleaseVoltage(void)
{
	...
	if (!IS_ERR(regu1)) {
		...
		if (IS_ERR(regu1))
			printk("nand:some err happen,fail to disable regulator vcc-nand!");
	...
	if (!IS_ERR(regu2)) {
		...
		if (ret)
			printk("nand:regulator disable fail ret is %x\n", ret);
		if (IS_ERR(regu2))
			printk("nand:some err happen,fail to disable regulator vcc-io!");
