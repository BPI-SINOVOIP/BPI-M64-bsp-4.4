...
#define DEV_DBG_EN  1
#if (DEV_DBG_EN == 1)
#define vfe_dev_dbg(x, arg...) pr_debug("[CSI_DEBUG][BF3903]"x, ##arg)
#else
#define vfe_dev_dbg(x, arg...)
#endif
#define vfe_dev_err(x, arg...) pr_err("[CSI_ERR][BF3903]"x, ##arg)
#define vfe_dev_print(x, arg...) pr_info("[CSI][BF3903]"x, ##arg)
...
static int bf3903_sensor_read(struct v4l2_subdev *sd,
				...
				struct v4l2_subdev_format *fmat)
{
	...
	ret = 0;
	if (wsize->regs) {
		ret = bf3903_sensor_write_array(sd, wsize->regs,
				wsize->regs_size);
		if (ret < 0)
			return ret;
	}
...
static int sensor_g_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	...
	pr_info("[King]----------------- info->tpf.denominator=%d\n",
