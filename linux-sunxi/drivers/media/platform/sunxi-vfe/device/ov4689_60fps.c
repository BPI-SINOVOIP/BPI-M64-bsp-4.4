...
#define DEV_DBG_EN      0
#if (DEV_DBG_EN == 1)
#define vfe_dev_dbg(x, arg...) pr_debug("[ov4689_60fps]"x, ##arg)
#else
#define vfe_dev_dbg(x, arg...)
#endif
#define vfe_dev_err(x, arg...) pr_err("[ov4689_60fps]"x, ##arg)
#define vfe_dev_print(x, arg...) pr_info("[ov4689_60fps]"x, ##arg)
...
static struct regval_list sensor_default_regs[] = {
	...
	/* ; Vsync input,  HREF input, FREX input, GPIO0 input */
	{0x3002, 0x00},
	...
	/* ; Gain delay 1 frame, use sensor gain, exposure delay 1 frame */
	{0x3503, 0x04},
	...
	/*
	*OTP bist enable, OTP BIST compare with zero,
	*OTP power up load data on, OTP power up load setting on,
	*OTP write register load setting off
	*/
	{0x3d85, 0x36},
	...
	{0x3f0a, 0x00}, /* ; PSRAM */
	/*
	*out of range trig off, format chg on,
	*gain chg on, exp chg on, manual trig off, no freeze,
	*always trig off, debug mode
	*/
	{0x4000, 0x71},
	...
	/* ; offset man same off, offset man off, black line output off, */
	{0x4011, 0x00},
	...
	/* ; vfifo pix swap off, dpcm off, vfifo first line is blue line */
	{0x4306, 0x00},
	...
	/*
	*MIPI always high speed off,
	*Clock lane gate off,
	*line short packet off,
	*/
	{0x4800, 0x04},
	...
	/* ; digital gain on, bin on, OTP on, WB gain on, average on, ISP on */
	{0x5000, 0xf3},
	...
	*enable emb clock,
	*enable strobe clock,
	*enable timing control clock,
	*mipi-phy manual reset,
	*reset timing control block
	*/
	{0x301a, 0xf9},
	{0x3508, 0x07}, /* ; Long gain H */
	/*
	*line start after fifo_st,
	*sclock start after SOF,
	*frame start after SOF
	*/
	{0x484b, 0x05},
	...
	/* enable emb clock, enable strobe clock, enable timing control clock */
	{0x301a, 0xf0},
...
/* return 0; */
	...
	if (exp_val > 0xfffff)
		exp_val = 0xfffff;
/* vfe_dev_print("exp_val = %d,gain_val = %d\n",exp_val,gain_val); */
...
/* vfe_dev_print("ov4689 sensor_set_exp = %d, Done!\n", tmp); */
...
/* vfe_dev_print("ov4689 sensor_set_gain = %d, Done!\n", tmp); */
	info->gain = gain_val;
	return 0;
}
