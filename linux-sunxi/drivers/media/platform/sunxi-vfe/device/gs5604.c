...
#define DEV_DBG_EN      0
#if (DEV_DBG_EN == 1)
#define vprintk(x, arg...) pr_debug("[GS5604]"x, ##arg)
#else
#define vprintk(x, arg...)
#endif
#if (DEV_DBG_EN == 1)
#define vfe_dev_dbg(x, arg...) pr_debug("[GS5604]"x, ##arg)
#else
#define vfe_dev_dbg(x, arg...)
#endif

#define vfe_dev_err(x, arg...) pr_err("[GS5604]"x, ##arg)
#define vfe_dev_print(x, arg...) pr_info("[GS5604]"x, ##arg)

#define CAP_BDG 0
#if (CAP_BDG == 1)
#define vfe_dev_cap_dbg(x, arg...) pr_debug("[GS5604_CAP_DBG]"x, ##arg)
#else
				...
				struct regval_list *regs, int array_size)
{
	...
	if (!regs)
	return -EINVAL;
	pr_info("[gs5604]enter sensor_write_array---------------------!!!!!!\n");
	...
	while (i < array_size) {
		if (regs->addr == REG_DLY)
			...
			if (regs[j].width == BYTE)
				LOG_ERR_RET(sensor_write_value8(sd,
				regs->addr, regs->data));
			else if (regs[j].width == WORD)
				LOG_ERR_RET(sensor_write_value16(sd,
				regs->addr, regs->data));
		}
		i++;
		regs++;
	}
	pr_info("[gs5604]exit sensor_write_array-----------------------!!!!!!\n");
	return 0;
}
...
static int sensor_g_single_af(struct v4l2_subdev *sd)
{
	...
	/* msleep(5);   //use in linux-3.10  */
	usleep_range(5000, 5500);
	sensor_read_value8(sd, 0x8b8a, &AF_state);
	/* msleep(1);  //use in linux-3.10   */
...
static int sensor_g_3a_lock(struct v4l2_subdev *sd)
{
	...
	return ((info->auto_focus == 0) ? V4L2_LOCK_FOCUS :
			~V4L2_LOCK_FOCUS | (info->autowb == 0) ?
...
static int sensor_s_init_af(struct v4l2_subdev *sd)
{
	...
	/* AF_EXT : AF driver start */
	sensor_write_value8(sd, 0x000B, 0x01);
	/* AF_AREA_LOW_TYPE1 */
	sensor_write_value16(sd, 0x6666, 0x0000);
	/* AF_AREA_HIGH_TYPE1(AF_SEARCH_AREA_HIGH) = 600 */
	sensor_write_value16(sd, 0x6668, 0x0340);
	/* AF_SEARCH_CORNER_HIGH =500 */
	sensor_write_value16(sd, 0x8B4C, 0x0340);
	/* AF_OVERSRCH_AREA_HIGH = CORNER_HIGH - AREA_HIGH */
	sensor_write_value16(sd, 0x6656, 0x0000);
	/* AF_CAF_PARAM_WOBBLE_STEP */
	sensor_write_value16(sd, 0x6622, 0x0004);
	/* AF_CAF_CLIMB_STEP */
	sensor_write_value16(sd, 0x6624, 0x0010);
	/* AF_LENSPOS_ON_AFNG */
	sensor_write_value16(sd, 0x665A, 0x0190);
...
static int sensor_s_single_af(struct v4l2_subdev *sd)
{
	...
	/* AF unlock */
	sensor_write_value8(sd, 0x5000, 0x00);
	/* af_xcoordinate */
	sensor_write_value16(sd, 0x6a50, af_window_xstart);
	/* af_ycoordinate */
	sensor_write_value16(sd, 0x6a52, af_window_ystart);
	/* af_width */
	sensor_write_value16(sd, 0x6a54, af_window_width);
	/* af_height */
	sensor_write_value16(sd, 0x6a56, af_window_height);
	/* msleep(10); //use in linux-3.10  */
	usleep_range(10000, 10500);
	/* preview single af enable */
	sensor_write_value8(sd, 0x00b2, 0x00);
	/* half release single af enable */
	sensor_write_value8(sd, 0x00b3, 0x00);
	/* movie single af enable */
	sensor_write_value8(sd, 0x00b4, 0x00);
	msleep(50);
...
*	int xstart, ystart, af_width, af_height; //xc, yc;
...
*	pr_info("[gs5604]af zone input af_window_xstart=%d,af_window_ystart=%d,af_window_width=%d,af_window_height=%d\n",
...
/*
*static int sensor_s_sharpness_value(struct v4l2_subdev *sd,
*				unsigned char value)
*{
*	return 0;
*}
...
/*
*static int sensor_s_denoise_value(struct v4l2_subdev *sd,
*				unsigned char value)
*{
*	return 0;
*}
...
*
*	pr_info("[gs5604]enter gs5604_wait_status function:\n ");
*	do {
*	sensor_read_value8(sd, 0x000E, &temp1);
*		temp2 = temp1 & 0x01;
*	pr_info("[gs5604]gs5604_wait_status while1!\r\n");
*	} while (!temp2);
*	sensor_write_value8(sd, 0x0012, 0x01);
*	do {
*	sensor_read_value8(sd, 0x000E, &temp1);
*		temp2 = temp1 & 0x01;
*	pr_info("[gs5604]gs5604_wait_status while2!\r\n");
*	} while (temp2);
*	pr_info("[gs5604]exit gs5604_wait_status function:\n ");
*}
*/
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
	...
	pr_info("[gs5604]sensor_init: stby_mode = %x\n", info->stby_mode);
	...
	ret = sensor_write_array(sd,
			sensor_default_regs, ARRAY_SIZE(sensor_default_regs));
				...
				struct v4l2_subdev_format *fmat)
{
	...
	*/
	...
	if (wsize->height == 480) { /* 640*480 */
		...
		sensor_write_value8(sd, 0x0082, 0x01);
		pr_info("[gs5604] sensor_s_fmt: set resolution 640*480\n");
		msleep(100);
	}
	if (wsize->height == 960) { /* 1280*960 */
		...
		sensor_write_value8(sd, 0x0083, 0x01);
		sensor_write_value8(sd, 0x0082, 0x01);
		pr_info("[gs5604] sensor_s_fmt   960\n");
		msleep(100);
	}
	if (wsize->height == 1200) {
		...
		sensor_write_value8(sd, 0x0089, 0x00);
		/* HSIZE_CAP(2560)) 2592 */
		sensor_write_value16(sd, 0x0090, 0x0640);
		/* VSIZE_CAP(1920) 1944 */
		sensor_write_value16(sd, 0x0096, 0x04b0);
		/* FPSTYPE_CAP(15fps) */
		sensor_write_value8(sd, 0x0086, 0x02);
		/* SENSMODE_CAP(Full) */
		...
		/* UPDATED */
		sensor_write_value8(sd, 0x0082, 0x01);
		pr_info("[gs5604] sensor_s_fmt  1200\n");
		msleep(200);
	}
	if (wsize->height == 1936) { /* 2592*1936 */
		...
		sensor_write_value8(sd, 0x0083, 0x00);
		sensor_write_value8(sd, 0x0082, 0x01);
		pr_info("[gs5604] sensor_s_fmt 55 1936\n");
		msleep(200);
	}
	/**************** end set resolution ******************/
	...
	if (info->capture_mode == V4L2_MODE_VIDEO ||
		...
		if ((info->width != QSXGA_WIDTH) &&
			...
			msleep(50);
			/* sensor_s_single_af */
			ret =  sensor_write_value8(sd, 0x00b2, 0x00);
			if (ret < 0) {
...
static int sensor_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	...
	if (info->capture_mode == V4L2_MODE_IMAGE) {
		vprintk("capture mode is not video mode,");
		vprintk("can not set frame rate!\n");
		return 0;
	}
