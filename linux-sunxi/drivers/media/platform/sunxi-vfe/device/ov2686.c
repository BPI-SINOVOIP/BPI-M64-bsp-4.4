...
#define DEV_DBG_EN    0
#if (DEV_DBG_EN == 1)
#define vfe_dev_dbg(x, arg...) pr_debug("[OV2686]"x, ##arg)
#else
#define vfe_dev_dbg(x, arg...)
#endif
#define vfe_dev_err(x, arg...) pr_err("[OV2686]"x, ##arg)
#define vfe_dev_print(x, arg...) pr_info("[OV2686]"x, ##arg)
...
static struct regval_list sensor_default_regs[] = {
	...
	/* software standby enter at l_blk, frex_ef_sel, cen_blobal_o */
	{0x3021, 0x23},
	{0x3082, 0x2c}, /* PLL */
	{0x3083, 0x00},
	{0x3084, 0x07},
	...
	/* lenc_en, awb_gain_en, lcd_en, avg_en, dgc_en, bc_en, wc_en, blc-en */
	{0x5000, 0xff},
	{0x5001, 0x05}, /* avg_sel after LCD */
	{0x5002, 0x32}, /* dpc_href_s, sof_sel, bias_plus */
	{0x5003, 0x04}, /* bias_man */
	/*
	*uv_dns_en, rng_dns_en, gamma_en, cmxen, cip_en,
	*raw_dns_en, stretch_en, awb_en
	*/
	{0x5004, 0xff},
	...
	/*
	*CIP, m_bColorEdgeEnable, m_bAntiAliasing,
	*m_nDetailSlop=0, m_nNoiseYSlop=5
	*/
	{0x5300, 0xc5},
	...
	*/
	...
	/* RGB_DNS, m_nShadowExtraNoise = 12, m_bSmoothYEnable */
	{0x5480, 0x19},
...
static int sensor_s_autowb(struct v4l2_subdev *sd, int value)
{
	...
	LOG_ERR_RET(sensor_write_array(sd, sensor_wb_auto_regs,
				...
				enum v4l2_power_line_frequency value)
{
	...
	case V4L2_CID_POWER_LINE_FREQUENCY_DISABLED:
		LOG_ERR_RET(sensor_read(sd, 0x3a00, &rdval));
		LOG_ERR_RET(sensor_write(sd, 0x3a00, rdval&0xfe));
		break;
	case V4L2_CID_POWER_LINE_FREQUENCY_50HZ:
		LOG_ERR_RET(sensor_read(sd, 0x3a02, &rdval));
		LOG_ERR_RET(sensor_write(sd, 0x3a02, rdval|0x80));
		LOG_ERR_RET(sensor_read(sd, 0x3a00, &rdval));
		LOG_ERR_RET(sensor_write(sd, 0x3a00, rdval|0x01));
		break;
	case V4L2_CID_POWER_LINE_FREQUENCY_60HZ:
		LOG_ERR_RET(sensor_read(sd, 0x3a02, &rdval));
		LOG_ERR_RET(sensor_write(sd, 0x3a02, rdval&0x7f));
		LOG_ERR_RET(sensor_read(sd, 0x3a00, &rdval));
		LOG_ERR_RET(sensor_write(sd, 0x3a00, rdval|0x01));
		break;
...
static int sensor_s_brightness(struct v4l2_subdev *sd, int value)
{
	...
	LOG_ERR_RET(sensor_write_array(sd, sensor_brightness[value+4].regs,
...
static int sensor_s_contrast(struct v4l2_subdev *sd, int value)
{
	...
	LOG_ERR_RET(sensor_write_array(sd, sensor_contrast[value+4].regs,
...
static int sensor_s_saturation(struct v4l2_subdev *sd, int value)
{
	...
	LOG_ERR_RET(sensor_write_array(sd, sensor_saturation[value+4].regs,
...
static int sensor_s_exp_bias(struct v4l2_subdev *sd, int value)
{
	...
	LOG_ERR_RET(sensor_write_array(sd, sensor_ev[value+4].regs,
				...
				struct sensor_win_size **ret_wsize)
{
	...
	for (index = 0; index < N_FMTS; index++)
		if (sensor_formats[index].mbus_code == fmt->code)/* linux-4.4 */
			break;
	...
	*/
	fmt->field = V4L2_FIELD_NONE;/* linux-4.4 */
	...
	*/
	fmt->width = wsize->width;/* linux-4.4 */
	fmt->height = wsize->height;/* linux-4.4 */
