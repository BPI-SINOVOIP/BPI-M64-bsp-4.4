...
#include <media/v4l2-mediabus.h> /* linux-4.4 */
#include <linux/io.h>
...
#define DEV_DBG_EN    0
#if (DEV_DBG_EN == 1)
#define vfe_dev_dbg(x, arg...) pr_debug("[CSI_DEBUG][nt99252]"x, ##arg)
#else
#define vfe_dev_dbg(x, arg...)
#endif

#define vfe_dev_err(x, arg...) pr_err("[CSI_ERR][nt99252]"x, ##arg)
#define vfe_dev_print(x, arg...) pr_info("[CSI][nt99252]"x, ##arg)
/*
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	SENSOR_ID |= (val);
	vfe_dev_print("nt99252_SENSOR_ID=%x", SENSOR_ID);
	if (ret < 0) {
		vfe_dev_err("sensor_read err at sensor_detect!\n");
		return ret;
	}

	if (nt99252_SENSOR_ID != SENSOR_ID)
		return -ENODEV;
#if 1
	/* Test */
...
static int sensor_enum_code(struct v4l2_subdev *sd,
			...
			struct v4l2_subdev_mbus_code_enum *code) /* linux-4.4 */
{
	if (code->index >= N_FMTS) /* linux-4.4 */
		return -EINVAL;

	code->code = sensor_formats[code->index].mbus_code; /* linux-4.4 */
...
static int sensor_try_fmt_internal(struct v4l2_subdev *sd,
		...
		struct v4l2_mbus_framefmt *fmt, /* linux-4.4 */
		...
		struct sensor_win_size **ret_wsize)
{
	...
	if (index >= N_FMTS) {
		...
		index = 0;
		fmt->code = sensor_formats[0].mbus_code; /* linux-4.4 */
	}
