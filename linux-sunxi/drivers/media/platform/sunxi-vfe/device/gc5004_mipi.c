...
#define DEV_DBG_EN      0
#if (DEV_DBG_EN == 1)
#define vfe_dev_dbg(x, arg...) pr_debug("[GC5004_mipi]"x, ##arg)
#else
#define vfe_dev_dbg(x, arg...)
#endif
#define vfe_dev_err(x, arg...) pr_err("[GC5004_mipi]"x, ##arg)
#define vfe_dev_print(x, arg...) pr_info("[GC5004_mipi]"x, ##arg)
...
static int sensor_s_gain(struct v4l2_subdev *sd, int gain_val)
{
	...
	if (gain_val < 0x40)
		...
		sensor_write(sd, 0xb1, tmp>>6);
		sensor_write(sd, 0xb2, (tmp<<2)&0xfc);
	} else if (gain_val >= ANALOG_GAIN_5) {
		/* analog gain */
