...
#define DEV_DBG_EN      0
#if (DEV_DBG_EN == 1)
#define vfe_dev_dbg(x, arg...) pr_debug("[OV8850]"x, ##arg)
#else
#define vfe_dev_dbg(x, arg...)
#endif
#define vfe_dev_err(x, arg...) pr_err("[OV8850]"x, ##arg)
#define vfe_dev_print(x, arg...) pr_info("[OV8850]"x, ##arg)
...
static struct regval_list sensor_quxga_regs[] = {
	...
	{0x4004, 0x08}, /* number of black lin */
	 /* BLC trigger every frame,for single capture,for single capture */
	{0x4005, 0x1a},
...
static int sensor_s_exp(struct v4l2_subdev *sd, unsigned int exp_val)
{
	...
	sensor_write(sd, 0x3501, expmid);
	sensor_write(sd, 0x3500, exphigh);
	vfe_dev_print("8850 sensor_set_exp = %d, Done!\n", exp_val);
...
static int sensor_s_gain(struct v4l2_subdev *sd, int gain_val)
{
	...
	vfe_dev_print("8850 sensor_set_gain = %d, Done!\n", gain_val);
	info->gain = gain_val;
