...
#define DEV_DBG_EN   0
#if (DEV_DBG_EN == 1)
#define vfe_dev_dbg(x, arg...) pr_debug("[CSI_DEBUG][NT99142]"x, ##arg)
#else
#define vfe_dev_dbg(x, arg...)
#endif
#define vfe_dev_err(x, arg...) pr_err("[CSI_ERR][NT99142]"x, ##arg)
#define vfe_dev_print(x, arg...) pr_info("[CSI][NT99142]"x, ##arg)
/*
...
*		return 0;
*	vfe_dev_print("%s:value = %d\n", __FUNCTION__, value);
*	if (value == V4L2_RESOLUTION_1080P) {
*		vfe_dev_print("########Can't Support 1080p############\n");
				...
				enum v4l2_power_line_frequency value)
{
	...
	case V4L2_CID_POWER_LINE_FREQUENCY_50HZ:
		sensor_write_nt99142_array(sd, reg_band_50hz_reg,
				ARRAY_SIZE(reg_band_50hz_reg));
			break;
	case V4L2_CID_POWER_LINE_FREQUENCY_60HZ:
		sensor_write_nt99142_array(sd, reg_band_60hz_reg,
				ARRAY_SIZE(reg_band_60hz_reg));
		break;
	default:
		break;
	}
				...
				struct v4l2_subdev_format *fmat)
{
	...
	if (wsize->regs) {
		ret =
		sensor_write_nt99142_array(sd, wsize->regs, wsize->regs_size);
		if (ret < 0)
			return ret;
	}
...
static int sensor_g_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	...
	vfe_dev_print("[King]----------------- info->tpf.denominator=%d\n",
...
static void auto_scene_worker(struct work_struct *work)
{
	...
	*/
	...
	if (ret < 0) {
		vfe_dev_print("auto_scene_worker error.\n");
		return;
	}
	if ((val > 0x84) && (info->night_mode == 0)) {
		...
		if (ret < 0) {
			vfe_dev_print("auto_scene error, return %x!\n", ret);
			return;
		}
		info->night_mode = 1;
		vfe_dev_print("debug switch to night mode\n");
		...
		info->night_mode = 0;
		vfe_dev_print("switch to daylight mode\n");
	}
	queue_delayed_work(info->wq, &info->work, msecs_to_jiffies(500));
}
