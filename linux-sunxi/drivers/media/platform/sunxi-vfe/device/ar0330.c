...
#define DEV_DBG_EN      0
#if (DEV_DBG_EN == 1)
#define vfe_dev_dbg(x, arg...) pr_debug("[AR0330 Raw]"x, ##arg)
#else
#define vfe_dev_dbg(x, arg...)
#endif
#define vfe_dev_err(x, arg...) pr_err("[AR0330 Raw]"x, ##arg)
#define vfe_dev_print(x, arg...) pr_info("[AR0330 Raw]"x, ##arg)
...
static int sensor_s_gain(struct v4l2_subdev *sd, int gain_val)
{
	...
	if (gain_val*100 <= 16 && gain_val*100 < (103*16))
		...
		tmp_gain = (2449700 + gain_val*564520 -
				gain_val*gain_val*1646) / 1000000;
				...
				struct sensor_exp_gain *exp_gain)
{
	...
	pr_info("norm exp_val = %d,gain_val = %d\n", exp_val, gain_val);
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	if (rdval != 0x2604) {
		pr_info("*********sensor error,read id is %x.\n", rdval);
	} else {
		pr_info("*********find ar0330 raw data camera sensor now.\n");
		return 0;
	}
	return -ENODEV;
}
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
	...
	pr_info("the 300e value is 0x%x\n", tmpv);
#endif
...
static long sensor_ioctl(struct v4l2_subdev *sd,
				...
				struct v4l2_subdev_format *fmat)
{
	...
	*sensor_write_array(sd, sensor_oe_enable_regs,
				*RRAY_SIZE(sensor_oe_enable_regs));
	*/
	return 0;
}
