...
#define DEV_DBG_EN  0
#if (DEV_DBG_EN == 1)
#define vfe_dev_dbg(x, arg...) pr_debug("[ar0330_mipi Raw]"x, ##arg)
#else
#define vfe_dev_dbg(x, arg...)
#endif
#define vfe_dev_err(x, arg...) pr_err("[ar0330_mipi Raw]"x, ##arg)
#define vfe_dev_print(x, arg...) pr_info("[ar0330_mipi Raw]"x, ##arg)
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	if (rdval != 0x2604) {
		...
		pr_info("*********find ar0330_mipi raw data camera sensor now.\n");
		return 0;
	}
	return -ENODEV;
}
...
static struct sensor_format_struct {
	...
	{
		.desc      = "Raw RGB Bayer",
		.mbus_code = MEDIA_BUS_FMT_SGRBG12_12X1,
		.regs      = sensor_fmt_raw,
