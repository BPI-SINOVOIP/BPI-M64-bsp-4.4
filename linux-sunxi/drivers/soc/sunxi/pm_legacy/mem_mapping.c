...
void create_mapping(void)
{
	*((volatile __u32 *)(PAGE_TBL_ADDR)) = 0xc4a;
	 ...
	 *	do this here, just in case testing.
	 *	like: jump to resume code for testing.
	**/

	/* Note: 0xc000,0000,is device area; not need to flush cache. */
	/* ref: ./arch/arm/kernel/head.S */
	__cpuc_coherent_kern_range((unsigned long)(PAGE_TBL_ADDR),
				   (unsigned long)(PAGE_TBL_ADDR +
						   (sizeof(u32))));
	local_flush_tlb_kernel_range((unsigned long)(PAGE_TBL_ADDR),
				     (unsigned long)(PAGE_TBL_ADDR +
						     (sizeof(u32))));
	return;
}
