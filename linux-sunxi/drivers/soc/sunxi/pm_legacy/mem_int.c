/*
 * drivers/soc/sunxi/pm_legacy/mem_int.c
 *
...
__s32 mem_int_init(void)
{
	...
	/* printk("gic iar == 0x%x. \n", *(volatile __u32         *)
	 * (IO_ADDRESS(SUNXI_GIC_CPU_PBASE)+0x0c)); */
	...
	*(volatile __u32 *)(GicCDisc + GIC_CPU_CTRL) = 0x1;
	...
	for (i = 4; i < (GIC_400_ENABLE_LEN); i += 4)
		*(volatile __u32 *)(GicDDisc + GIC_DIST_PENDING_CLEAR + i) =
		    0xffffffff;
#endif
	/* the print info just to check the pending state,
	 * actually, after u read iar, u need to access
...
__s32 mem_enable_int(enum interrupt_source_e src)
{
	...
	*(volatile __u32 *)(GicDDisc + GIC_DIST_ENABLE_SET + tmpGrp * 4) |=
	    (1 << tmpSrc);
	/* printk("GicDDisc + GIC_DIST_ENABLE_SET + tmpGrp*4 = 0x%x.
	 * tmpGrp = 0x%x.\n", GicDDisc +
	 * GIC_DIST_ENABLE_SET + tmpGrp*4, tmpGrp); */
	/* printk("tmpSrc = 0x%x. \n", tmpSrc); */
...
__s32 mem_query_int(enum interrupt_source_e src)
{
	...
	/* printk("GicDDisc + GIC_DIST_PENDING_SET + tmpGrp*4 = 0x%x.
	 * tmpGrp = 0x%x.\n", GicDDisc + GIC_DIST_PENDING_SET +
	 * tmpGrp*4, tmpGrp); */
