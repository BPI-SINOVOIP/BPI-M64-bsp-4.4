/*
 * drivers/soc/sunxi/pm_legacy/scenelock/scenelock.c
 *
...
static void print_active_locks(enum SUNXI_POWER_SCENE type)
{
	...
	WARN_ON(type >= SCENE_MAX);
	list_for_each_entry(lock, &active_scene_locks[type], link) {
		pr_info("active scene lock %s\n", lock->name);
	}
}

static long has_scene_lock_locked(enum SUNXI_POWER_SCENE type)
{
...
static long has_scene_lock(enum SUNXI_POWER_SCENE type)
{
	long ret;
...
void scene_lock_init(struct scene_lock *lock,
	enum SUNXI_POWER_SCENE type, const char *name)
{
 ...
 *				if the device driver do want the system be wakenup
 *				while in standby state.
 ...
 *				2. At a31, only gpio pa, pb, pe, pg, pl, pm
 *				int wakeup src is supported.
*/
 ...
 *
 *	function:	if the device driver do not want the system be wakenup
 *				while in standby state again.
 *				the device driver should use this function
...
/**
 *	check_wakeup_state: to get the corresponding
 ...
 */
int scene_set_volt(enum SUNXI_POWER_SCENE scene_type,
	unsigned int bitmap, unsigned int volt_value)
{
	...
	for (i = 0; i < extended_standby_cnt; i++) {
		if (extended_standby[i].scene_type == scene_type) {
			extended_standby[i].soc_pwr_dep.soc_pwr_dm_state.volt[bitmap]
				= volt_value;
			return 0;
		}
	}
...
static int __init scenelocks_init(void)
{
	...
	for (i = 0; i < sizeof(extended_standby)/sizeof(extended_standby[0]);
		i++) {
		spin_lock_irqsave(&extended_standby_list_lock, irqflags);
