/*
 * drivers/soc/sunxi/pm_legacy/scenelock/userscenelock.c
 *
...
static struct rb_root user_scenelocks;

struct user_scenelock *lookup_scene_lock_name(
	const char *buf, int allocate)
{
	struct rb_node **p = &user_scenelocks.rb_node;
	...
	rb_insert_color(&l->node, &user_scenelocks);
	return l;
	...
	const char *buf, size_t n)
{
	...
	if (strlen(l->name) == strlen("usb_standby") &&
		!strncmp(l->name, "usb_standby", strlen("usb_standby"))) {
		enable_wakeup_src(CPUS_USBMOUSE_SRC, 0);
	} else if (strlen(l->name) == strlen("talking_standby") &&
	!strncmp(l->name, "talking_standby", strlen("talking_standby"))) {
		;
	} else if (strlen(l->name) == strlen("mp3_standby") &&
		!strncmp(l->name, "mp3_standby", strlen("mp3_standby"))) {
		;
	}
bad_name:
...
{
	pr_info("lock super standby defaultly!\n");
