...
*********************************************************************************************************
*/
int arisc_hwmsgbox_send_message(struct arisc_message *pmessage,
				unsigned int timeout)
{
	...
	if (pmessage->attr & ARISC_MESSAGE_ATTR_HARDSYN) {
		...
		while (readl
		       (IO_ADDRESS
			(AW_MSGBOX_FIFO_STATUS_REG
			 (ARISC_HWMSGBOX_AC327_SYN_TX_CH))) == 1) {
			/* message-queue fifo is full, waiting always */
			;
		}
		value =
		    ((volatile unsigned long)pmessage) - arisc_sram_a2_vbase;
		ARISC_INF("ac327 send hard syn message : %x\n",
			  (unsigned int)value);
		writel(value,
		       IO_ADDRESS(AW_MSGBOX_MSG_REG
				  (ARISC_HWMSGBOX_AC327_SYN_TX_CH)));

		/* hwsyn messsage must feedback use syn rx channel */
		while (readl
		       (IO_ADDRESS
			(AW_MSGBOX_MSG_STATUS_REG
			 (ARISC_HWMSGBOX_AC327_SYN_RX_CH))) == 0) {
			/* message not valid */
			;
		}
		/* check message valid */
		if (value !=
		    (readl
		     (IO_ADDRESS
		      (AW_MSGBOX_MSG_REG(ARISC_HWMSGBOX_AC327_SYN_RX_CH))))) {
	...
	while (readl
	       (IO_ADDRESS
		(AW_MSGBOX_FIFO_STATUS_REG(ARISC_HWMSGBOX_ARISC_ASYN_RX_CH))) ==
	       1) {
		/* message-queue fifo is full, waiting always */
		;
	}
	...
	ARISC_LOG("ac327 send soft syn or asyn message : %x\n",
		  (unsigned int)value);
	writel(value,
	       IO_ADDRESS(AW_MSGBOX_MSG_REG(ARISC_HWMSGBOX_ARISC_ASYN_RX_CH)));

	/* syn messsage must wait message feedback */
	if (pmessage->attr & ARISC_MESSAGE_ATTR_SOFTSYN) {
		ARISC_ERR
		    ("standby arisc driver not support soft syn message transfer\n");
		return -EINVAL;
	}
...
int arisc_hwmsgbox_feedback_message(struct arisc_message *pmessage,
				    unsigned int timeout)
{
	volatile unsigned long value;
	...
	if (pmessage->attr & ARISC_MESSAGE_ATTR_HARDSYN) {
		...
		while (readl
		       (IO_ADDRESS
			(AW_MSGBOX_FIFO_STATUS_REG
			 (ARISC_HWMSGBOX_ARISC_SYN_RX_CH))) == 1) {
		...
		writel(value,
		       IO_ADDRESS(AW_MSGBOX_MSG_REG
				  (ARISC_HWMSGBOX_ARISC_SYN_RX_CH)));
		return 0;
	}
	...
	if (pmessage->attr & ARISC_MESSAGE_ATTR_SOFTSYN) {
		while (readl
		       (IO_ADDRESS
			(AW_MSGBOX_FIFO_STATUS_REG
			 (ARISC_HWMSGBOX_ARISC_ASYN_RX_CH))) == 1) {
			/* fifo is full, wait */
			;
		}
		/* write message to message-queue fifo. */
		...
		writel(value,
		       IO_ADDRESS(AW_MSGBOX_MSG_REG
				  (ARISC_HWMSGBOX_ARISC_ASYN_RX_CH)));
		return 0;
	}
	/* invalid syn message */
	return -EINVAL;
...
{
	writel((0x1 << (queue * 2)),
	       IO_ADDRESS(AW_MSGBOX_IRQ_STATUS_REG(user)));
...
struct arisc_message *arisc_hwmsgbox_query_message(void)
{
	...
	if (readl
	    (IO_ADDRESS
	     (AW_MSGBOX_MSG_STATUS_REG(ARISC_HWMSGBOX_ARISC_ASYN_TX_CH)))) {
		volatile unsigned long value;
		value =
		    readl(IO_ADDRESS
			  (AW_MSGBOX_MSG_REG(ARISC_HWMSGBOX_ARISC_ASYN_TX_CH)));
		pmessage =
		    (struct arisc_message *)(value + arisc_sram_a2_vbase);
		if (arisc_message_valid(pmessage)) {
			...
			/* print_call_info(); */
			ARISC_ERR
			    ("invalid message received: 1 pmessage = 0x%x. \n",
			     (__u32) pmessage);
			return (struct arisc_message *)(NULL);
		}
		/* clear pending */
		arisc_hwmsgbox_clear_receiver_pending
		    (ARISC_HWMSGBOX_ARISC_ASYN_TX_CH,
	...
	if (readl
	    (IO_ADDRESS
	     (AW_MSGBOX_MSG_STATUS_REG(ARISC_HWMSGBOX_ARISC_SYN_TX_CH)))) {
		volatile unsigned long value;
		value =
		    readl(IO_ADDRESS
			  (AW_MSGBOX_MSG_REG(ARISC_HWMSGBOX_ARISC_SYN_TX_CH)));
		pmessage =
		    (struct arisc_message *)(value + arisc_sram_a2_vbase);
		if (arisc_message_valid(pmessage)) {
			/* message state switch */
			...
			/* print_call_info(); */
			ARISC_ERR
			    ("invalid message received: 2 pmessage = 0x%x. \n",
			     (__u32) pmessage);
			return (struct arisc_message *)(NULL);
		}
		arisc_hwmsgbox_clear_receiver_pending
		    (ARISC_HWMSGBOX_ARISC_SYN_TX_CH, AW_HWMSG_QUEUE_USER_AC327);
		return pmessage;
	}
	/* no valid message */
	return (struct arisc_message *)(NULL);
...
{
	if ((((__u32) pmessage) >=
	     (ARISC_MESSAGE_POOL_START + arisc_sram_a2_vbase))
	    && (((__u32) pmessage) <
		(ARISC_MESSAGE_POOL_END + arisc_sram_a2_vbase))) {
		/* valid message */
		return 1;
	}
	return 0;
