/*
 * drivers/soc/sunxi/pm_legacy/pm.c
 *
...
/* actually, mean ,prepare condition to disable mmu */
...
/*  | PM_STANDBY_PRINT_STANDBY | PM_STANDBY_PRINT_RESUME|
 *  PM_STANDBY_ENABLE_JTAG; */
__u32 debug_mask = PM_STANDBY_TEST;
...
static const struct extended_standby_manager_t *extended_standby_manager_id;
...
*
*Description:
* determine if given system sleep state is supported by the platform;
*
...
*
*Notes      : this is a call-back function, registered into PM core,
*	      and this function will be called before devices suspened;
******************************************************************************
*/
static int aw_pm_begin(suspend_state_t state)
{
	...
	/*must init perfcounter, because delay_us and delay_ms
	 * is depandant perf counter */
...
#define MAX_RETRY_TIMES (5)
	...
	/*  the developer only need to care about: the scene support
	 *  the wakeup src; */
	if (NULL != extended_standby_manager_id) {
...
static int aw_pm_enter(suspend_state_t state)
{
	...
	|| defined(CONFIG_ARCH_SUN8IW9P1)) && defined(CONFIG_AW_AXP)
		...
		/* clean and flush: the data is already in cache, so can
		 * clean the data into sram; */
		/* while, in dma transfer condition, the data is not in cache,
		 * so can not use this api in dma ops. */
 ...
 *Return     : none;
 *
 *Notes      : This function called when the system has just left a sleep
 *		state, right after the nonboot CPUs have been enabled and
 *		before device drivers' early resume callbacks are executed.
 *		This function is opposited to the aw_pm_prepare_late;
 ******************************************************************************
 */
...
*Return     : none
*
*Notes      : This function is called right prior to calling device drivers'
*		regular suspend callbacks. This function is opposited to
*		the aw_pm_prepare function.
******************************************************************************
*/
...
*Return     : none
*
*Notes      : This function is called by the PM core right after resuming
*		devices, to indicate to	the platform that the system has
*		returned to the working state or the transition to the sleep
*		state has been aborted. This function is opposited to
...
*Return     : none
*
*Notes      : This function alled by the PM core if the suspending of devices
*		fails. This callback is optional and should only be implemented
*		by platforms which require special recovery actions in that
*		situation.
******************************************************************************
*/
