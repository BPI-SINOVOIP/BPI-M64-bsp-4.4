/*
 * drivers/soc/sunxi/pm_legacy/mem_mmu_pc.c
 *
...
{
	/* CR0 */
	asm volatile ("mrc p15, 2, %0, c0, c0, 0" : "=r"
			(saved_mmu_state->cssr));
	/* CR1 */
	asm volatile ("mrc p15, 0, %0, c1, c0, 2" : "=r"
			(saved_mmu_state->cacr));

	/* CR3 */
	asm volatile ("mrc p15, 0, %0, c3, c0, 0" : "=r"
	...
	/* CR1 */
	/* busy_waiting(); */
	/* will effect visible addr space */
	asm volatile ("mrc p15, 0, %0, c1, c0, 0" : "=r"
			(saved_mmu_state->cr));
...
void restore_secure_mmu_state(struct mmu_state *saved_mmu_state)
{
	...
	/* busy_waiting(); */
	/* CR0 */
	asm volatile ("mcr p15, 2, %0, c0, c0, 0" : : "r"
			(saved_mmu_state->cssr));
	/* CR1 */
	asm volatile ("mcr p15, 0, %0, c1, c0, 2" : : "r"
			(saved_mmu_state->cacr));
	/* CR3 */
	asm volatile ("mcr p15, 0, %0, c3, c0, 0" : : "r"
			(saved_mmu_state->dacr));
...
/*when translate 0x0000,0000, use ttb0, while ttb0 shoudbe the same with ttb1*/
	...
	asm volatile ("mcr p15, 0, %0, c1, c0, 0" : : "r"
			(saved_mmu_state->cr));
#endif
	/*read id reg */
