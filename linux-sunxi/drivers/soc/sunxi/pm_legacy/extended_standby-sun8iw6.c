/*
 * drivers/soc/sunxi/pm_legacy/extended_standby-sun8iw6.c
 *
...
static struct extended_standby_manager_t extended_standby_manager = {
...
static bool
calculate_pll(int index, struct scene_extended_standby_t *standby_data)
{
	__u32 standby_rate;
...
static bool
calculate_bus(int index, struct scene_extended_standby_t *standby_data)
{
	if (BUS_NUM <= index) {
...
static int check_cfg(void)
{
	...
	if (temp_standby_data.cpux_clk_state.bus_change != 0) {
		for (i = 0; i < BUS_NUM; i++) {
			if ((CLK_SRC_LOSC ==
...
static int
copy_extended_standby_data(struct scene_extended_standby_t *standby_data)
{
	int i = 0;
	int j = 0;
	...
	if (!standby_data) {
		...
		/* for stable reason, remain the higher voltage; */
			if ((temp_standby_data.soc_pwr_dm_state.
			     state & temp_standby_data.soc_pwr_dm_state.
			     sys_mask) != 0) {
			...
			temp_standby_data.cpux_clk_state.exit_pll_en |=
			    standby_data->soc_pwr_dep.cpux_clk_state.
			    exit_pll_en;
			if (standby_data->soc_pwr_dep.cpux_clk_state.
			    pll_change != 0) {
				for (i = 0; i < PLL_NUM; i++) {
					if (standby_data->soc_pwr_dep.
					    cpux_clk_state.
...
/* not supprt add io config, this code just for checking io config. */
			for (j = 0; j < IO_NUM; j++) {
				/*new added*/
				if (standby_data->soc_pwr_dep.soc_io_state.
				    io_state[j].paddr == 0) {
				...
				for (i = 0; i < IO_NUM; i++) {
					...
					if (temp_standby_data.soc_io_state.
					    io_state[i].paddr == 0) {
 ...
 *	return the extended_standby_manager pointer; else return NULL;
...
const struct extended_standby_manager_t *get_extended_standby_manager(void)
{
    ...
    if ((manager_data != NULL) && (manager_data->pextended_standby != NULL)) {
#if defined(CONFIG_AW_AXP)
		/* update sys_mask */
...
bool set_extended_standby_manager(struct scene_extended_standby_t *local_standby)
{
	...
	if (extended_standby_manager.pextended_standby != NULL)
		EXSTANDBY_DBG("leave %s : id 0x%x\n", __func__,
 ...
 *
 *	function:
 *	 the device driver care about the wakeup src.if the device driver do
 *	 want the system be wakenup while in standby state. the device driver
 *	 should use this function to enable corresponding intterupt.
 *	@src:			wakeup src.
 *	@para:
 *	 if wakeup src need para, be the para of wakeup src, else ignored.
 *	notice:
 *	 1. for gpio intterupt, only access the enable bit, mean u need care
 *	    about other config, such as: int mode, pull up or pull down
 *	    resistance, etc.
 *	 2. At a31, only gpio��pa, pb, pe, pg, pl,
 *	    pm��int wakeup src is supported.
*/
 ...
 *
 *	function:
 *	 if the device driver do not want the system be wakenup while in
 *	 standby state again. the device driver should use this function
 ...
 *	 if wakeup src need para, be the para of wakeup src, else ignored.
 *	notice:
 *	 for gpio intterupt, only access the enable bit, mean u need care
 *	 about other config, such as: int mode, pull up or pull down
 *	 resistance, etc.
 */
 ...
 *	 to get the corresponding wakeup src intterupt state,
 *	 enable or disable.
 *
 *	@src:			wakeup src.
 *	@para:
...
/**
 *
 *	function:standby state including locked_scene, power_supply dependancy,
 *	the wakeup src.
 *
