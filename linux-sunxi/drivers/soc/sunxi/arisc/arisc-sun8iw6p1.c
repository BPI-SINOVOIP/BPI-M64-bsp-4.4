/*
 *  drivers/soc/sunxi/arisc/arisc-sun8iw6p1.c
 *
...
static unsigned int arisc_debug_baudrate;
unsigned int arisc_debug_dram_crc_en;
unsigned int arisc_debug_dram_crc_srcaddr;
unsigned int arisc_debug_dram_crc_len;
...
#elif (defined CONFIG_ARCH_SUN50IW2P1) || (defined CONFIG_ARCH_SUN8IW6P1)
	if (clk_prepare_enable(arisc_cfg.dram.pllddr0)) {
		ARISC_ERR("try to enable pll_ddr0 output failed!\n");
		return -EINVAL;
	}
...
#elif (defined CONFIG_ARCH_SUN50IW2P1) || (defined CONFIG_ARCH_SUN8IW6P1)
	arisc_cfg.dram.pllddr0 = of_clk_get_by_name(arisc_cfg.dram.np, "pll_ddr");
	if (!arisc_cfg.dram.pllddr0 || IS_ERR(arisc_cfg.dram.pllddr0)) {
		ARISC_ERR("try to get pll_ddr0 failed!\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_AW_AXP
	/* get power regulator tree */
	axp_get_pwr_regu_tree(para->power_regu_tree);
#endif
...
u32 sunxi_load_arisc(void *image, u32 image_size, void *para, u32 para_size)
{
	...
	if (sunxi_soc_is_secure()) {
		...
		/* clear sram_a2 area */
		memset((void *)arisc_cfg.space[0].vbase, 0, arisc_cfg.space[0].size);
