/*
 * drivers/video/fbdev/sunxi/disp2/tv/tv_ac200.c
 *
 * Copyright (c) 2007-2017 Allwinnertech Co., Ltd.
 * Author: zhengxiaobin <zhengxiaobin@allwinnertech.com>
 *
 * top entry file of driver for sunxi ac200
 *
...
void tv_report_hpd_work(void)
{
	...
	case DISP_TV_NONE:
		extcon_set_state(&cvbs_extcon_dev, STATUE_CLOSE);
		break;

	case DISP_TV_CVBS:
		extcon_set_state(&cvbs_extcon_dev, STATUE_OPEN);
		break;
...
static s32 tv_open(void)
{
	...
	if (mutex_trylock(&tv_mutex)) {
		...
		ret = aw1683_tve_open();
		if (ret == 0)
			pr_info("tve open finish\n");
		else
			pr_info("tve open failed:%d\n", ret);
...
s32 tv_resume(void)
{
	...
	while (!acx00_state) {
		...
		pr_info("%s: wait for acx00 resume finish\n", __func__);
	}
