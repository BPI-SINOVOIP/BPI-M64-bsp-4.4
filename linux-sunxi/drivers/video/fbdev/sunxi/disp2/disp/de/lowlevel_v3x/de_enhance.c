...
static int de_enhance_apply_core(unsigned int screen_id)
{
	...
	other_dirty = ENHANCE_SIZE_DIRTY | ENHANCE_FORMAT_DIRTY | ENHANCE_ENABLE_DIRTY |
		      ENHANCE_MODE_DIRTY;
	for (ch_id = 0; ch_id < chno; ch_id++) {
		...
		if (g_para[screen_id][ch_id]->flags &
		    (other_dirty | ENHANCE_EDGE_DIRTY | ENHANCE_BYPASS_DIRTY))
			de_lti_info2para(screen_id, ch_id, fmt, dev_type,
					  &g_para[screen_id][ch_id]->lti_para,
					  g_para[screen_id][ch_id]->bypass);

		if (g_para[screen_id][ch_id]->flags &
		    (ENHANCE_FORMAT_DIRTY | ENHANCE_ENABLE_DIRTY | ENHANCE_MODE_DIRTY |
		     ENHANCE_SAT_DIRTY | ENHANCE_BYPASS_DIRTY))
		...
		if (g_para[screen_id][ch_id]->flags &
		    (other_dirty | ENHANCE_DNS_DIRTY | ENHANCE_BYPASS_DIRTY) &&
			   ...
			   struct disp_enhance_config *config)
{
	...
	for (ch_id = 0; ch_id < chno; ch_id++) {
		...
		g_para[screen_id][ch_id]->flags |=
					(config[0].flags & ENHANCE_USER_DIRTY);
		bypass = g_para[screen_id][ch_id]->bypass;
		if (config[0].flags & ENHANCE_MODE_DIRTY) {
			/* enhance_mode */
			...
			if (bypass != g_para[screen_id][ch_id]->bypass)
				g_para[screen_id][ch_id]->flags |=
							       ENHANCE_BYPASS_DIRTY;
		}

		if (config[0].flags & ENHANCE_BRIGHT_DIRTY) {
			g_para[screen_id][ch_id]->fce_para.bright_level =
						config[0].info.bright;
		}

		if (config[0].flags & ENHANCE_CONTRAST_DIRTY) {
			g_para[screen_id][ch_id]->fce_para.contrast_level =
						config[0].info.contrast;
		}

		if (config[0].flags & ENHANCE_EDGE_DIRTY) {
			g_para[screen_id][ch_id]->lti_para.level =
						config[0].info.edge;
		}

		if (config[0].flags & ENHANCE_DETAIL_DIRTY) {
		...
		if (config[0].flags & ENHANCE_SAT_DIRTY) {
		...
		if (config[0].flags & ENHANCE_DNS_DIRTY) {
