...
static int mxt_probe_regulators(struct mxt_data *data)
{
	...
	data->reg_vdd = regulator_get(dev, data->pdata->reg_vdd_name);
	...
	data->reg_avdd = regulator_get(dev, data->pdata->reg_avdd_name);
...
static const struct mxt_platform_data *mxt_parse_dt(struct i2c_client *client)
{
	...
	if (pdata->gpio_reset != 0) {
		gpio_request(pdata->gpio_reset, "ctp-reset");
		gpio_direction_output(pdata->gpio_reset, 1);
	}

	if (pdata->gpio_int != 0) {
		sunxi_gpio_to_name(pdata->gpio_int, pin_name);
		config = SUNXI_PINCFG_PACK(SUNXI_PINCFG_TYPE_PUD, INT_GPIO_PULL_UP);
	...
	of_property_read_string(np, "atmel,reg_vdd", &pdata->reg_vdd_name);

	of_property_read_string(np, "atmel,reg_avdd", &pdata->reg_avdd_name);
