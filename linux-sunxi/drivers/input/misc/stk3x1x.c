...
static ssize_t stk_send_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = strict_strtoul(token[0], 16, (unsigned long *)&(addr));
	if (ret < 0) {
		printk(KERN_ERR "%s:strict_strtoul failed, ret=0x%x\n", __func__, ret);
		return ret;
	}
	ret = strict_strtoul(token[1], 16, (unsigned long *)&(cmd));
	if (ret < 0) {
		printk(KERN_ERR "%s:strict_strtoul failed, ret=0x%x\n", __func__, ret);
		return ret;
	}
...
#ifdef CONFIG_PM
static int stk3x1x_suspend(struct i2c_client *client, pm_message_t mesg)
{
	struct stk3x1x_data *ps_data =  i2c_get_clientdata(client);
	int err;
#ifdef CONFIG_SCENELOCK
...
static int stk3x1x_resume(struct i2c_client *client)
{
	...
	if (ls_sensor_info.sensor_power_ldo != NULL) {
		if (0 != regulator_enable(ls_sensor_info.sensor_power_ldo))
...
static int __init stk3x1x_init(void)
{
	...
	if (input_sensor_startup(&(ls_sensor_info.input_type))) {
		printk("%s: ls-stk3x1x fetch paras err.\n", __func__);
		return 0;
	} else {
		ret = input_sensor_init(&(ls_sensor_info.input_type));
		if (0 != ret) {
			printk("%s: ls-stk3x1x initalize platform err.\n", __func__);
		}
	}
