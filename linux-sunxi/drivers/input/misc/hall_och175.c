...
*/
      ...
      /*printk("EARLY_SUSPEND_STATUS=%d\n", EARLY_SUSPEND_STATUS);*/
		...
		if ((level_status == 1) && (sys_status == SUSPEND_STATUS)) { /*leave off*/
			printk("******leave off hall sensor, status is suspend, input KEY_POWER******\n");
		...
		} else if ((level_status == 0) && (sys_status == RESUME_STATUS)) {/*close*/
			printk("******close to hall sensor, status is resume, input KEY_POWER******\n");
			input_report_key(input_dev, KEY_POWER, 1);
			input_sync(input_dev);
			msleep(100);
			input_report_key(input_dev, KEY_POWER, 0);
...
/*	script_item_value_type_e  type;*/
	...
	val.gpio.gpio = of_get_named_gpio_flags(np, "hall_int_port", 0, (enum of_gpio_flags *) &val.gpio);
	if (!gpio_is_valid(val.gpio.gpio))
		pr_err("%s: hall_int_port is invalid. \n", __func__);

/*	type = script_get_item("hall_para", "hall_int_port", &val);
