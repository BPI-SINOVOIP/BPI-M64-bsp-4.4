...
static struct sunxi_iommu_domain *global_iommu_domain;

static struct iommu_group *gt_group;
...
static struct iommu_domain *sunxi_iommu_domain_alloc(unsigned iommu_domain_type)
{
	struct sunxi_iommu_domain *sunxi_domain;
	...
	global_iommu_domain = sunxi_domain;
...
sunxi_iommu_attach_dev(struct iommu_domain *domain, struct device *dev)
{
	...
	if (dev->archdata.iommu == NULL)
		return -ENODEV;
	if (!gt_group) {
		gt_group = iommu_group_alloc();
		if (IS_ERR(gt_group)) {
			dev_err(dev, "Failed to allocate IOMMU group\n");
			return PTR_ERR(gt_group);
		}
	}
	ret = iommu_group_add_device(gt_group, dev);
	return ret;
}
...
{
	int ret = 0;
	static struct dma_iommu_mapping *mappingtemp;
	...
	if (!mappingtemp) {
		mapping =
		arm_iommu_create_mapping(&platform_bus_type, 0, 0x100000000);
		if (IS_ERR(mapping)) {
			ret = PTR_ERR(mapping);
			pr_err("arm_iommu_create_mapping failed\n");
			goto out_0;
		}
		mappingtemp = mapping;
		sunxi_tlb_init(owner, mapping->domain);
	}

	/*
	 * The segment make the dma_map_sg can map bigger than 64k
	 */
	...
	if (arm_iommu_attach_device(dev, mappingtemp)) {
	...
	pr_debug("mastor id:%d, bypass:0x%x\n",
		owner->tlbid, sunxi_iommu_read(owner->data, IOMMU_BYPASS_REG));
...
{
	struct sunxi_iommu_domain *sunxi_domain = global_iommu_domain;
