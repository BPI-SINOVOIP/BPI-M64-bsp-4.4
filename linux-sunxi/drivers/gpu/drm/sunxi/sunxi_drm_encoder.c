...
irqreturn_t sunxi_encoder_vsync_handle(int irq, void *data)
{
	...
	if (!sunxi_irq_query(sunxi_encoder->hw_res,
		encoder, QUERY_VSYNC_IRQ))
		return IRQ_HANDLED;
		...
		if (conct->encoder == encoder) {
			...
			if (sunxi_con->hw_res != NULL &&
				...
				sunxi_con->hw_res->ops->vsync_proc != NULL) {
					sunxi_con->hw_res->ops->vsync_proc(
						sunxi_con);
			}
		}
	}
...
bool sunxi_tcon_dsi_reset(void *data)
{
	struct sunxi_drm_encoder *sunxi_encoder =
		to_sunxi_encoder(data);
	struct sunxi_hardware_res *hw_res;
...
/*
* When DSI and his version is 40, then use
* dsi interrupt as vsync, or use tcon's;
*/
#ifndef DSI_VERSION_40
	sunxi_irq_request(hw_res);
#endif
...
int sunxi_tcon_common_check_line(void *data)
{
	...
	start_delay = tcon_get_start_delay(sunxi_enc->enc_id,
		sunxi_enc->enc_id);
...
int sunxi_tcon_dsi_set_timing(void *data, struct drm_display_mode *mode)
{
	...
	tcon_init(sunxi_encoder->enc_id);
	tcon0_set_dclk_div(sunxi_encoder->enc_id,
		lcd_private->clk_info->tcon_div);
...
int sunxi_tcon_hdmi_set_timing(void *data, struct drm_display_mode *mode)
{
	...
	rate = hw_res->clk_rate
		= hdmi_timing->pixel_clk * (hdmi_timing->pixel_repeat + 1);
	rate_parent = clk_get_rate(hw_res->parent_clk);
	DRM_DEBUG_KMS("sunxi_tcon_hdmi_set_timing, rate:%lu, \
		rate_parent:%lu\n", rate, rate_parent);
	if (!hdmi_is_divide_by(rate_parent, rate)) {
		if (hdmi_is_divide_by(297000000, rate))
			clk_set_rate((struct clk *)rate_parent, 297000000);
		else if (hdmi_is_divide_by(594000000, rate))
			clk_set_rate((struct clk *)rate_parent, 594000000);
	}

	if (hw_res->clk) {
		clk_set_rate(hw_res->clk, rate);
		rate_set = clk_get_rate(hw_res->clk);
	}
	if (hw_res->clk && (rate_set != rate)) {
		for (i = 1; i < 10; i++) {
			rate_parent = rate * i;
			rate_round =
				clk_round_rate(hw_res->parent_clk,
				rate_parent);
			if (rate_round == rate_parent) {
				clk_set_rate(hw_res->parent_clk,
					rate_parent);
				clk_set_rate(hw_res->clk, rate);
				break;
			}
		...
		if (i == 10)
			DRM_ERROR(
				"clk_set_rate fail.we need %ldhz, \
				but get %ldhz\n",
				rate, rate_set);
	}
	...
	tcon1_hdmi_color_remap(sunxi_encoder->enc_id,
		hdmi_private->video_para.is_yuv);
...
int sunxi_drm_encoder_create(struct drm_device *dev,
	...
	enum encoder_ops_type type, void *private)
{
	...
	case ENCODER_OPS_DSI:
		...
		* lcd_p->clk_info->dsi_div;
		hw_res->parent_clk_rate =
			hw_res->clk_rate * lcd_p->clk_info->lcd_div;
		hw_res->en_ctl_by = 1;
		break;
	...
	case ENCODER_OPS_HDMI:
		...
		hw_res->parent_clk = clk_get_parent(hw_res->clk);
		if (IS_ERR(hw_res->parent_clk)) {
			DRM_ERROR("fail to get parent clk for hdmi\n");
			return -EINVAL;
		}
	...
	*/
	DRM_DEBUG_KMS("[%d]:type:%d parent_rate:%lu  rate:%lu. \
		encoder[%d][%d] \n", __LINE__, type,
		hw_res->parent_clk_rate, hw_res->clk_rate,
		sunxi_encoder->enc_id,
