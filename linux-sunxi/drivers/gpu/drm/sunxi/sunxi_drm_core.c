...
bool sunxi_find_irq_res(struct drm_connector *connector, unsigned int *irq,
	void **irq_arg,
	irqreturn_t(**irq_handle)(int, void *), bool *enalbed)
{
...
int sunxi_drm_get_res_info(struct disp_bsp_init_para *para,
...
struct sunxi_hardware_res *sunxi_hwres_init(enum disp_mod_id id)
{
	struct sunxi_hardware_res *hw_res;
...
int sunxi_display_init(struct disp_bsp_init_para *para)
{
	...
	para->reg_base[DISP_MOD_DE] =
		(uintptr_t __force)of_iomap(node, counter);
	...
	for (i = 0; i < DISP_DEVICE_NUM; i++) {
		...
		if (!para->reg_base[DISP_MOD_LCD0 + i]) {
			DRM_ERROR("unable to map timing \
				controller %d registers\n", i);
			goto err_iomap;
		}
		counter++;
	}
	...
	counter = 0;
	for (i = 0; i < DISP_DEVICE_NUM; i++) {
		para->irq_no[DISP_MOD_LCD0 + i] =
			irq_of_parse_and_map(node, counter);
		if (!para->irq_no[DISP_MOD_LCD0 + i]) {
			DRM_ERROR("irq_of_parse_and_map irq %d \
				fail for tcon%d\n", counter, i);
		}
		counter++;
	}
	...
	if (!para->irq_no[DISP_MOD_DE]) {
		DRM_ERROR("irq_of_parse_and_map de \
			irq %d fail for dsi\n", i);
	}
	counter++;
	...
	if (!para->irq_no[DISP_MOD_EINK]) {
		DRM_ERROR("irq_of_parse_and_map eink \
			irq %d fail for dsi\n", i);
	}
	counter++;
#endif
...
int sunxi_drm_init(struct drm_device *dev)
{
	...
	for (i = 0, id = DISP_MOD_DE; i < max_crtc; i++) {
		...
		__LINE__, hw_res->res_id, hw_res->reg_base, hw_res->irq_no);
	}
	...
	for (i = 0; i < max_enc; i++) {
		...
		ret = sunxi_drm_encoder_create(dev,
			possible_up, fix_up, i, hw_res);
		if (ret) {
			goto err;
		}
#if 0
		...
		if (res_irq && hw_res->irq_no && has_res[id][1]) {
			...
			platform_device_add_resources(dev->platformdev,
				res_irq, 1);
			kfree(res_irq);
			res_irq = NULL;
		}
	...
	for (i = 0, id = DISP_MOD_DSI0, lcd_id = 0, hdmi_id = 0;
		...
		case DISP_MOD_LVDS:
			...
			for (j = 0; j < max_enc; j++) {
				if (sunxi_drm_encoder_support(j,
...
#if 0
/*
* When use DSI & 40nm platform, you need register
* DSI interrupt, or use tcon interrupt
*/
#if defined(SUPPORT_DSI) && defined(DSI_VERSION_40)
			if (res_irq && hw_res->irq_no && has_res[id][1]
				&& (id == DISP_MOD_DSI0)) {
			...
			break;
		case DISP_MOD_HDMI:
			sunxi_panel = sunxi_hdmi_pan_init(hw_res, i,
				hdmi_id);
			if (!sunxi_panel) {
				sunxi_hwres_destroy(hw_res);
				continue;
		...
		DRM_DEBUG_KMS("[%d]:id[%d] reg_base[0x%lx] irq_id[%d]\n",
			__LINE__, hw_res->res_id, hw_res->reg_base,
			hw_res->irq_no);

		ret = sunxi_drm_connector_create(dev, possible_up,
			fix_up, i, sunxi_panel, disp_out_type, hw_res);
		if (ret) {
