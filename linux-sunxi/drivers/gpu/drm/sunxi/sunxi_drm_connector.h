...
struct sunxi_drm_connector {
	...
	int dmps;
	/* 0~15  0~7 */
	struct drm_property *brigt_linght_property;
...
#define to_sunxi_connector(x)	container_of(x, \
	struct sunxi_drm_connector, drm_connector)
...
int sunxi_drm_connector_create(struct drm_device *dev,
	...
	struct sunxi_panel *panel, enum disp_output_type disp_out_type,
	struct sunxi_hardware_res *hw_res);
