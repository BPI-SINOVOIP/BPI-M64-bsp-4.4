...
sunxi_drm_connector_detect(struct drm_connector *connector, bool force)
{
	...
	if (sunxi_connector->panel->panel_ops->detect)
		status = sunxi_connector->panel->panel_ops->detect(
			sunxi_connector->panel);
...
static void sunxi_drm_connector_destroy(struct drm_connector *connector)
{
	...
	if (hw_res) {
		/*
		* no need to free hw_res twice,
		* the first time is in sunxi_panel_destroy
		*/
#if 0
...
int sunxi_drm_connector_create(struct drm_device *dev,
	int possible_enc, int fix_enc, int id,
	...
	struct sunxi_hardware_res *hw_res)
{
	...
	case DISP_OUTPUT_TYPE_LCD:
		...
		case LCD_IF_LVDS:
			...
			if (sunxi_encoder_assign_ops(dev,
				fix_enc, ENCODER_OPS_LVDS, panel->private))
				goto out;
			break;
		case LCD_IF_DSI:
			type = DRM_MODE_CONNECTOR_DSI;
			if (sunxi_encoder_assign_ops(dev,
				fix_enc, ENCODER_OPS_DSI, panel->private))
				goto out;
			break;
		case LCD_IF_EDP:
	...
	case DISP_OUTPUT_TYPE_HDMI:
		...
		err = sunxi_encoder_assign_ops(dev, fix_enc,
			ENCODER_OPS_HDMI, panel->private);
		if (err && fix_enc != -1)
			goto out;
		break;
	...
	/*
	* when call sunxi_panel_destroy,
	* it will not find any device, so add it;
	*/
	panel->type = sunxi_connector->disp_out_type;
	...
	list_for_each_entry(encoder,
		...
		if (possible_enc & 1) {
			err = drm_mode_connector_attach_encoder(
				connector, encoder);
			if (err) {
