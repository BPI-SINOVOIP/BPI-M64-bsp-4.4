	...
	struct drm_file *file_priv, struct drm_mode_fb_cmd2 *mode_cmd)
{
	...
	for (i = 0, nr_gem = 0; i < MAX_FB_BUFFER; i++) {
		...
		if (!sunxi_check_gem_memory_type(obj[i], SUNXI_BO_CONTIG)) {
			DRM_ERROR("cannot use this gem memory \
			type for fb.\n");
			ret = -EFAULT;
	...
	if (nr_gem == 0 || nr_gem > max_cnt) {
		DRM_ERROR("cannot use this gem memory type for \
			fb, gem[%d][%08x].\n",
...
struct sunxi_drm_gem_buf *sunxi_drm_framebuffer_buffer(
	struct drm_framebuffer *fb, int index)
{
	struct sunxi_drm_framebuffer *sunxi_fb = to_sunxi_fb(fb);
	...
	gem_obj = sunxi_fb->gem_obj[index];
	if (NULL == gem_obj) {
		return NULL;
	}
	...
	struct drm_gem_object *obj[MAX_FB_BUFFER], int nr_gem, int flags)
{
	...
	/* fb->refcount = 2 */
	ret = drm_framebuffer_init(dev, &sunxi_fb->drm_fb,
