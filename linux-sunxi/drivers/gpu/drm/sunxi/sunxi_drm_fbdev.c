...
int sunxi_drm_fb_mmap(struct fb_info *info, struct vm_area_struct *vma)
{
	...
	struct sunxi_drm_framebuffer *sunxi_fb =
		to_sunxi_fb(sunxi_fbdev->drm_fb_helper.fb);
	...
	if (vm_size > buffer->size) {
		DRM_ERROR("failed to mmap %ld > %llu.\n",
			vm_size, buffer->size);
		return -EINVAL;
	}
	...
	struct drm_framebuffer *fb)
{
	...
	drm_fb_helper_fill_var(fbi, helper,
		fb->width, fb->height / FBDEV_BUF_NUM);
	...
	DRM_DEBUG_KMS("addr:0x%lx  size:%llu\n",
	...
	struct drm_fb_helper_surface_size *sizes)
{
	...
	/* with GPU */
	mode_cmd.pitches[0] = sizes->fb_width * (sizes->surface_bpp >> 3);
...
static struct drm_display_mode *sunx_pick_cmdline_mode(
...
static struct drm_display_mode *sunxi_find_similar_modes(
	struct drm_display_mode *ref_mode,
	struct drm_connector *connector)
{
#ifdef CONFIG_ARM
	unsigned int ref_factor, tmp_factor,
		little_factor = 1000000, cmp_factor;
#endif
#ifdef CONFIG_ARM64
	unsigned long long ref_factor, tmp_factor,
		little_factor = 100000000, cmp_factor;
#endif
	struct drm_display_mode *mode = NULL;
...
#endif
#ifdef CONFIG_ARM64
	ref_factor = (((unsigned long long)ref_mode->hdisplay) << 10)
		/ ref_mode->vdisplay;
#endif

	fix_mode = list_entry(connector->modes.next,
		struct drm_display_mode, head);

	list_for_each_entry(mode, &connector->modes, head) {
		/* check width/height */
		if (mode->hdisplay < ref_mode->hdisplay
			|| mode->vdisplay < ref_mode->vdisplay)
			continue;
#ifdef CONFIG_ARM
		tmp_factor = ((mode->hdisplay) << 10) / mode->vdisplay;
#endif
#ifdef CONFIG_ARM64
		tmp_factor = (((unsigned long long)mode->hdisplay) << 10)
			/ mode->vdisplay;
#endif
	...
	ref_mode->hdisplay, ref_mode->vdisplay,
	fix_mode->hdisplay, fix_mode->vdisplay);
...
retry:
	...
	for (i = 0; i < fb_helper->connector_count; i++) {
		...
		if (ref_modes == NULL && sunxi_connector->disp_out_type ==
			DISP_OUTPUT_TYPE_LCD) {
			ref_modes = list_entry(connector->modes.next,
				struct drm_display_mode, head);
			modes[i] = ref_modes;
			goto retry;
		}

		modes[i] = sunx_pick_cmdline_mode(
		...
		if (ref == -1) {
			modes[i] = list_entry(connector->modes.next,
				struct drm_display_mode, head);
			ref = i;
		}

		if (ref_modes) {
			modes[i] = sunxi_find_similar_modes(ref_modes,
				connector);
		} else {
			modes[i] = sunxi_find_similar_modes(modes[ref],
				connector);
		}
		if (!modes[i])
			continue;
		...
		for (c = 0; c < fb_helper->crtc_count; c++) {
			...
			if ((assign_crtc & (1 << c)) == 0) {
				assign_crtc |= (1 << c);
				crtcs[i] = crtc;
				DRM_INFO("connetor(%d)--->crtc(%d) \
					encoder:%d,resolution(%d x %d)\n",
					i, c, encoder->base.id,
					modes[i]->hdisplay,
					modes[i]->vdisplay);
				break;
...
int sunxi_drm_fbdev_creat(struct drm_device *dev)
{
	...
	drm_fb_helper_prepare(dev, helper, &sunxi_drm_fb_helper);
