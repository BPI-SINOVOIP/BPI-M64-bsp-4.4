...
static struct sg_table *sunxi_gem_map_dma_buf(
	struct dma_buf_attachment *attach,
	enum dma_data_direction dir)
{
	...
	if (sgt->nents == 0) {
		ret = sg_alloc_table(sgt,
			sunxi_buf->sgt->orig_nents, GFP_KERNEL);
		if (ret) {
	...
	/*
	* if attached dev impletment the dma_map_ops,
	* so we must make a sg for it,
	* not use the sunxi_buf's sg. the same reason for the attach.
	* one dma_buf_attach() maybe call many
	* dma_buf_map_attachment() for diffrent dir.
	*/
	if (sunxi_attach->dir != DMA_NONE) {
		/* for invalte and clean cache POC*/
		dma_unmap_sg(attach->dev, sgt->sgl,
			sgt->nents, sunxi_attach->dir);
	}
	...
	DRM_DEBUG_PRIME("buffer size = %llu\n", sunxi_buf->size);
	...
	struct vm_area_struct *vma)
{
	...
	*/
	...
	DRM_DEBUG_PRIME("vma_start:%lx  end:%lx offset:%lu size = %llu\n",
		vma->vm_start, vma->vm_end, vma->vm_pgoff * PAGE_SIZE,
		sunxi_buf->size);
	...
	struct drm_gem_object *obj, int flags)
{
	...
	(void)sunxi_buf;
	exp_info.ops = &sunxi_gem_dmabuf_ops;
	...
	struct dma_buf *dma_buf)
{
	...
	ret = drm_gem_object_init(drm_dev, &buffer->obj, dma_buf->size);
	if (!ret) {
		ret = -ENOMEM;
	...
	sunxi_gem_obj = &buffer->obj;
	buffer->sgt = sgt;
	sunxi_gem_obj->import_attach = attach;

	DRM_DEBUG_PRIME("[%d] dma_addr = 0x%llx, size = %llu\n",
