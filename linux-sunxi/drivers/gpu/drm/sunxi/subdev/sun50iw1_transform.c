...
*  File name   :	de_tr.c
*
*  Description :	display engine 2.0 transform processing
...
int de_tr_set_cfg(tr_info *info)
{
	...
	if ((x0 != 0) || (y0 != 0)) {
		pr_warn("not original point[%d,%d]\n", x0, y0);
		return -1;
	}
	...
	if (fmt >= TR_FORMAT_YUV444_I_AYUV) {
		if ((pitch[0]&0xf) || (pitch[1]&0xf) || (pitch[2]&0xf)) {
			pr_warn("Out buffer need 16 byte align[%x,%x,%x].\n",
				pitch[0], pitch[1], pitch[2]);
			return -1;
		}
	}
	...
	if ((0 != x0) || (0 != y0)) {
		pr_warn("Out buffer not original point[%d,%d].\n", x0, y0);
		return -1;
	}
	...
	/* src */
	if (TR_FORMAT_YUV422_I_VYUY  == info->src_frame.fmt)
		fmt = 0x20;
	else if (TR_FORMAT_YUV422_I_YVYU  == info->src_frame.fmt)
		fmt = 0x21;
	else if (TR_FORMAT_YUV422_I_UYVY  == info->src_frame.fmt)
		fmt = 0x22;
	else if (TR_FORMAT_YUV422_I_YUYV  == info->src_frame.fmt)
		fmt = 0x23;
	else if (TR_FORMAT_YUV422_SP_UVUV == info->src_frame.fmt)
		fmt = 0x24;
	else if (TR_FORMAT_YUV422_SP_VUVU == info->src_frame.fmt)
		fmt = 0x25;
	else if (TR_FORMAT_YUV422_P == info->src_frame.fmt)
		fmt = 0x26;
	else if (TR_FORMAT_YUV420_SP_UVUV == info->src_frame.fmt)
		fmt = 0x28;
	else if (TR_FORMAT_YUV420_SP_VUVU == info->src_frame.fmt)
		fmt = 0x29;
	else if (TR_FORMAT_YUV420_P == info->src_frame.fmt)
		fmt = 0x2a;
	else if (TR_FORMAT_YUV411_SP_UVUV == info->src_frame.fmt)
		fmt = 0x2c;
	else if (TR_FORMAT_YUV411_SP_VUVU == info->src_frame.fmt)
		fmt = 0x2d;
	else if (TR_FORMAT_YUV411_P == info->src_frame.fmt)
		fmt = 0x2e;
	else
		fmt = info->src_frame.fmt;
