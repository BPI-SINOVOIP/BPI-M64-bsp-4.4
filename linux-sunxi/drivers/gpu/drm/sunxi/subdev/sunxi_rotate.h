...
struct sunxi_rotate_task {
	...
	/* 0 ok, -1 error, 1 busy, 2 not start */
	int status;
	bool sleep_mode;
};

#define to_sunxi_rotate_q(x)	\
	container_of(x, struct sunxi_rotate_task, q_head)
#define to_sunxi_rotate_w(x)	\
	container_of(x, struct sunxi_rotate_task, w_head)
#define to_sunxi_rotate_priv(x)	\
	container_of(x, struct sunxi_rotate_private, daemon_work)
