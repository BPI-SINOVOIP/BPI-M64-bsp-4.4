...
static int sunxi_wlan_on(struct sunxi_wlan_platdata *data, bool on_off)
{
	...
	if (!on_off && gpio_is_valid(data->gpio_wlan_power_enable))
		gpio_set_value(data->gpio_wlan_power_enable, 1);
...
#if defined(VIRTUAL_EXTENDED_GPIO)
	printk("------------SUNXI_RF: Set regon for SUN3IW1P1_R6!----------------\n");
	data->gpio_wlan_regon = 2042;
...
#endif
	data->gpio_wlan_power_enable = of_get_named_gpio_flags(np, "wlan_power_enable", 0,
							(enum of_gpio_flags *)
							&config);
	if (!gpio_is_valid(data->gpio_wlan_power_enable)) {
		dev_err(dev, "get gpio wlan_power_enable failed\n");
	} else {
		dev_info(dev,
			 "wlan_power_enable gpio=%d  mul-sel=%d  pull=%d  drv_level=%d  data=%d\n",
			 config.gpio,
			 config.mul_sel,
			 config.pull, config.drv_level, config.data);

		ret = devm_gpio_request(dev, data->gpio_wlan_power_enable,
					"wlan_power_enable");
		if (ret < 0) {
			dev_err(dev, "can't request wlan_power_enable gpio %d\n",
				data->gpio_wlan_power_enable);
			goto end;
		}

		ret = gpio_direction_output(data->gpio_wlan_power_enable, 0);
		if (ret < 0) {
			dev_err(dev,
				"can't request output direction wlan_power_enable gpio %d\n",
				data->gpio_wlan_power_enable);
			goto end;
		}
	}
