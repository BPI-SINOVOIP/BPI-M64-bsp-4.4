...
EXPORT_SYMBOL(USBC_Host_SetFunctionAddress_Default);
...
void USBC_Host_SetHubAddress_Default(__hdle hUSB, __u32 ep_type, __u32 ep_index)
{
	__usbc_otg_t *usbc_otg = (__usbc_otg_t *)hUSB;
...
EXPORT_SYMBOL(USBC_Host_SetHubAddress_Default);

void USBC_Host_SetHubAddress(__hdle hUSB, __u32 ep_type,
		__u32 ep_index, __u32 is_multi_tt, __u8 address)
{
	__usbc_otg_t *usbc_otg = (__usbc_otg_t *)hUSB;
	...
	switch (ep_type) {
	case USBC_EP_TYPE_TX:
		USBC_Writeb(((is_multi_tt << USBC_BP_HADDR_MULTI_TT) | address),
			USBC_REG_TXHADDRx(usbc_otg->base_addr, ep_index));
		break;
...
EXPORT_SYMBOL(USBC_Host_SetHPortAddress_Default);
