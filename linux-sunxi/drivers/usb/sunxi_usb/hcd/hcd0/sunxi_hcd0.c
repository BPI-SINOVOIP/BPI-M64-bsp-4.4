 ...
 */
#ifdef CONFIG_UDC_HIGH_BANDWIDTH_ISO
...
static __s32 pin_init(struct platform_device *pdev, sunxi_hcd_io_t *sunxi_hcd_io)
{
	...
	if (ret) {
		...
		if (strncmp(drv_vbus_name, "axp_ctrl", 8) == 0) {
			...
			/* get drv vbus gpio */
			sunxi_hcd_io->drv_vbus_gpio_set.gpio = of_get_named_gpio(usbc_np,
								KEY_USB_DRVVBUS_GPIO, 0);
			if (gpio_is_valid(sunxi_hcd_io->drv_vbus_gpio_set.gpio))
	...
	if (sunxi_hcd_io->drv_vbus_valid) {
		ret = gpio_request(sunxi_hcd_io->drv_vbus_gpio_set.gpio, "otg_drv_vbus");
		if (ret != 0) {
			DMSG_PANIC("gpio_request failed\n");
			sunxi_hcd_io->drv_vbus_valid = 0;
		} else {
			gpio_direction_output(sunxi_hcd_io->drv_vbus_gpio_set.gpio, 0);
		}
	}
...
#ifndef	SUNXI_USB_FPGA
	if (sunxi_hcd_io->drv_vbus_valid) {
		gpio_free(sunxi_hcd_io->drv_vbus_gpio_set.gpio);
		sunxi_hcd_io->drv_vbus_valid = 0;
	}

	if (sunxi_hcd_io->usb_restrict_valid) {
		gpio_free(sunxi_hcd_io->restrict_gpio_set.gpio);
...
static void sunxi_hcd_board_set_vbus(struct sunxi_hcd *sunxi_hcd, int is_on)
{
	...
	if (sunxi_hcd->sunxi_hcd_io->drv_vbus_valid) {
		__gpio_set_value(sunxi_hcd->sunxi_hcd_io->drv_vbus_gpio_set.gpio, is_on);
	} else {
...
static int sunxi_hcd_core_init(u16 sunxi_hcd_type, struct sunxi_hcd *sunxi_hcd)
{
	...
	if (sunxi_hcd_type == SW_HCD_CONTROLLER_MHDRC) {
...
int sunxi_usb_host0_enable(void)
{
	...
	if (enable_irq_wake(sunxi_hcd->nIrq) == 0) {
		sunxi_hcd->irq_wake = 1;
		device_init_wakeup(dev, 1);
	} else {
		sunxi_hcd->irq_wake = 0;
