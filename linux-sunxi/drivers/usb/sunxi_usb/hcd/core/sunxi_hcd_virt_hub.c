...
static void sunxi_hcd_port_suspend(struct sunxi_hcd *sunxi_hcd, bool do_suspend)
{
	...
	if (do_suspend) {
		...
		DMSG_INFO_HCD0("[sunxi_hcd]: suspend port.\n");
		...
		DMSG_INFO_HCD0("[sunxi_hcd]: suspend portend, resume port.\n");
...
static void sunxi_hcd_port_reset(struct sunxi_hcd *sunxi_hcd, bool do_reset)
{
	...
	power = USBC_Readb(USBC_REG_PCTL(usbc_base));
	if (do_reset) {
		DMSG_INFO_HCD0("[sunxi_hcd]: reset port.\n");
		...
		USBC_Host_SetFunctionAddress_Default(sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle, USBC_EP_TYPE_TX, 0);

		{
			...
			for (i = 1; i <= 5; i++) {
				...
				USBC_Host_SetFunctionAddress_Default(sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle, USBC_EP_TYPE_TX, i);
				USBC_Host_SetFunctionAddress_Default(sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle, USBC_EP_TYPE_RX, i);
			}

			USBC_SelectActiveEp(sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle, old_ep_index);
		}
	} else {
		DMSG_INFO_HCD0("[sunxi_hcd]: reset port stopped.\n");
...
{
	/* clean pctrl reg, when disconnect happened during resetting or suspending */
	if ((sunxi_hcd->port1_status & USB_PORT_STAT_SUSPEND)
		...
		DMSG_INFO_HCD0("[sunxi_hcd]: clean pctrl reg, when disconnect happen during resetting or suspending, status =0x%x\n", sunxi_hcd->port1_status);
			...
			case USB_PORT_FEAT_TEST:
				{
					...
					case 6:
						...
						break;
					default:
						DMSG_PANIC("ERR: unknown SetPortFeature USB_PORT_FEAT_TEST wIndex(%d)\n", wIndex);
						goto error;
					}
				...
				break;
			default:
				DMSG_PANIC("ERR: unknown SetPortFeature wValue(%d)\n", wValue);
				goto error;
			}
