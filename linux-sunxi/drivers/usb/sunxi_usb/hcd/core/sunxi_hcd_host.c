		...
		struct urb *urb, int status)
{
	...
	default:
		DMSG_WRN("wrn: unknown ep type(%d)\n", qh->type);
		break;
	}
...
irqreturn_t sunxi_hcd_h_ep0_irq(struct sunxi_hcd *sunxi_hcd)
{
	...
	if (sunxi_hcd->ep0_stage == SUNXI_HCD_EP0_STATUS) {
		retval = IRQ_HANDLED;
		complete = true;
	}
	...
	if (!complete) {
		...
		if (sunxi_hcd_h_ep0_continue(sunxi_hcd, len, urb)) {
			...
			csr = (sunxi_hcd->ep0_stage == SUNXI_HCD_EP0_IN)
...
#ifndef CONFIG_UDC_HIGH_BANDWIDTH_ISO
...
int sunxi_hcd_urb_dequeue(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	DMSG_INFO_HCD0("[sunxi_hcd]: sunxi_hcd_urb_dequeue, sunxi_hcd(%p, %d, 0x%x), urb(%p, %d, %d), dev = %d, ep = %d, dir = %s\n",
