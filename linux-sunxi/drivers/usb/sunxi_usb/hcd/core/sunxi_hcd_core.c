...
void sunxi_hcd_write_fifo(struct sunxi_hcd_hw_ep *hw_ep, u16 len, const u8 *src)
{
	...
	old_ep_index = USBC_GetActiveEp(
			hw_ep->sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle);
	USBC_SelectActiveEp(
			hw_ep->sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle,
			hw_ep->epnum);
...
void sunxi_hcd_load_testpacket(struct sunxi_hcd *sunxi_hcd)
{
	...
	sunxi_hcd_write_fifo(sunxi_hcd->control_ep,
			sizeof(sunxi_hcd_test_packet),
			sunxi_hcd_test_packet);
...
void sunxi_hcd_start(struct sunxi_hcd *sunxi_hcd)
{
	...
	DMSG_DBG_HCD("sunxi_hcd_start: devctl = 0x%x, epmask = 0x%x, 0x%x\n",
		USBC_Readb(USBC_REG_DEVCTL(usbc_base)),
		sunxi_hcd->epmask, usbc_base);
...
#if defined(CONFIG_ARCH_SUN8IW5) || defined(CONFIG_ARCH_SUN8IW9)
{
	...
	USBC_Writeb((1 << USBC_BP_POWER_H_HIGH_SPEED_EN),
			USBC_REG_PCTL(usbc_base));

	devctl = USBC_Readb(USBC_REG_DEVCTL(usbc_base));
	devctl &= ~(1 << USBC_BP_DEVCTL_SESSION);
	USBC_Writeb(devctl, USBC_REG_DEVCTL(usbc_base));

	USBC_SelectBus(sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle,
			USBC_IO_TYPE_PIO, 0, 0);
...
void sunxi_hcd_platform_try_idle(struct sunxi_hcd *sunxi_hcd,
...
int sunxi_hcd_platform_set_mode(struct sunxi_hcd *sunxi_hcd,
		u8 sunxi_hcd_mode)
{
...
int sunxi_hcd_platform_exit(struct sunxi_hcd *sunxi_hcd)
{
	...
	USBC_ForceId(sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle,
...
#if 0
void sunxi_hcd_dma_completion(struct sunxi_hcd *sunxi_hcd,
...
{
	DMSG_INFO_HCD0("-------sunxi_hcd%d_soft_disconnect---------\n",
		sunxi_hcd->usbc_no);
...
static irqreturn_t sunxi_hcd_stage0_irq(struct sunxi_hcd *sunxi_hcd,
		u8 int_usb, u8 devctl, u8 power)
{
	...
	if (int_usb & (1 << USBC_BP_INTUSB_SOF)) {
		USBC_INT_ClearMiscPending(
				sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle,
				(1 << USBC_BP_INTUSB_SOF));
		handled = IRQ_HANDLED;

		USBC_INT_DisableUsbMiscUint(
				sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle,
				(1 << USBC_BP_INTUSB_SOF));
	}
	...
	if (int_usb & (1 << USBC_BP_INTUSB_RESUME)) {
		...
		if (devctl & (1 << USBC_BP_DEVCTL_HOST_MODE)) {
			if (power & (1 << USBC_BP_POWER_H_SUSPEND)) {
				/* spurious */
				sunxi_hcd->int_usb &=
					~(1 << USBC_BP_INTUSBE_EN_SUSPEND);

				DMSG_INFO_HCD0("sunxi_hcd_stage0_irq, Spurious SUSPENDM\n");
			}
	...
	if (int_usb & (1 << USBC_BP_INTUSB_SESSION_REQ)) {
		...
		USBC_INT_DisableUsbMiscUint(
				sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle,
				(1 << USBC_BP_INTUSBE_EN_SESSION_REQ));
...

{
	...
	if (int_usb & (1 << USBC_BP_INTUSB_RESET)) {
		...
		USBC_Host_SetFunctionAddress_Default(
				sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle,
				USBC_EP_TYPE_TX, 0);
		{
			u32 i = 1;
		...
		/* clear all the irq status of the plugout devie,
		 * now has no hub, so can clear all irqs
		 */
		USBC_INT_ClearMiscPendingAll(
				sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle);
		USBC_INT_ClearEpPendingAll(
				sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle,
				USBC_EP_TYPE_TX);
		USBC_INT_ClearEpPendingAll(
				sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle,
						...
						u8 power)
{
	...
	if ((int_usb & (1 << USBC_BP_INTUSB_DISCONNECT))
			&& !sunxi_hcd->ignore_disconnect) {
		DMSG_INFO_HCD0("\n--IRQ DISCONNECT--\n\n");

		USBC_INT_ClearMiscPending(
				sunxi_hcd->sunxi_hcd_io->usb_bsp_hdle,
				(1 << USBC_BP_INTUSB_DISCONNECT));
...
/*
 * handle all the irqs defined by the sunxi_hcd core.
 * for now we expect: other
 * irq sources //(phy, dma, etc) will be handled first,
 * sunxi_hcd->int_* values
...
static irqreturn_t sunxi_hcd_interrupt(struct sunxi_hcd *sunxi_hcd)
{
	...
	if (sunxi_hcd->int_usb & STAGE0_MASK) {
		retval |= sunxi_hcd_stage0_irq(
				sunxi_hcd, sunxi_hcd->int_usb,
				devctl, power);
	}
...
irqreturn_t generic_interrupt(int irq, void *__hci)
{
	...
	if (retval != IRQ_HANDLED) {
		DMSG_INFO_HCD0("spurious?, int_usb=0x%x, int_tx=0x%x, int_rx=0x%x\n",
