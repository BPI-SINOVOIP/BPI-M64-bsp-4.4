...
/*CUSTOMER_HW_ALLWINNER*/
#ifdef CUSTOMER_HW_ALLWINNER

struct semaphore wlan_chipup_sem;

#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 39)) && defined(CONFIG_PM)
static int pseudo_sdmmc_suspend(struct device *pdev)
{
	return 0;
}

static int pseudo_sdmmc_resume(struct device *pdev)
{
	return 0;
}

static const struct dev_pm_ops pseudo_sdmmc_pm_ops = {
	.suspend	= pseudo_sdmmc_suspend,
	.resume		= pseudo_sdmmc_resume,
};
#endif  /* (LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 39)) && defined(CONFIG_PM) */

static int pseudo_probe(struct sdio_func *func,
			const struct sdio_device_id *id)
{
	printk("==========the WLAN card is detected!=============\n");
/*	if (func && (func->num != 2)) {
		return 0;
	}
*/
...
static void pseudo_remove(struct sdio_func *func)
{
}

static struct sdio_driver pseudo_sdmmc_driver = {
	.probe		= pseudo_probe,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27) && defined(CONFIG_PM)
	.drv = {
	.pm     = &pseudo_sdmmc_pm_ops,
	},
#endif
	};

int wlan_sdio_wait_card(void)
{
	return down_timeout(&wlan_chipup_sem,
			msecs_to_jiffies(POWERUP_WAIT_MS));
}

int wlan_sdio_func_reg_notify(void)
{
	sema_init(&wlan_chipup_sem, 0);
	return sdio_register_driver(&pseudo_sdmmc_driver);
}

void wlan_sdio_func_unreg_notify(void)
{
	msleep(20);
	sdio_unregister_driver(&pseudo_sdmmc_driver);
}

#endif /*CUSTOMER_HW_ALLWINNER*/
