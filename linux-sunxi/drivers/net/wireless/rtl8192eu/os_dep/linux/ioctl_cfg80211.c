	...
	struct station_parameters *params)
{
	...
	if (MLME_IS_MESH(padapter)) {
		...
		if (!scanned) {
			...
			ret = -EINVAL;
			goto release_plink_ctl;
		} else if (!rtw_bss_is_candidate_mesh_peer_and_same_ch(&padapter->mlmepriv.cur_network.network, &scanned->network)){
	...
	, struct mesh_config *conf)
{
	...
	/* TBD */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0))
	conf->rssi_threshold = 0x7fffffff;
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0))
	...
	, u32 mask, const struct mesh_config *nconf)
{
	...
	rtw_mesh_update_forwarding_state(adapter, adapter->mesh_cfg.dot11MeshForwarding);
