...
)
{
	...
	PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "TxPwrTracking path %c: X = 0x%x, Y = 0x%x ele_A = 0x%x ele_C = 0x%x ele_D = 0x%x 0xeb4 = 0x%x 0xebc = 0x%x\n",
...
)
{
	...
	PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "===>ODM_TxPwrTrackSetPwr8192EA\n");
	...
	if (method == TXAGC) {
		...
		PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "odm_TxPwrTrackSetPwr92E CH=%d\n", *(dm->channel));
		...
		if (*(dm->mp_mode) == true) {
			if (rf_path == RF_PATH_A) {
				...
				PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "odm_tx_pwr_track_set_pwr92_e: CCK Tx-rf(A) Power = 0x%x\n", tx_agc);
				...
				PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "odm_tx_pwr_track_set_pwr92_e: OFDM Tx-rf(A) Power = 0x%x\n", tx_agc);
				...
				PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "odm_tx_pwr_track_set_pwr92_e: CCK Tx-rf(B) Power = 0x%x\n", pwr);
				...
				PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "odm_tx_pwr_track_set_pwr92_e: OFDM Tx-rf(B) Power = 0x%x\n", tx_agc);
			}
		} else
#endif
		{
...
)
{
	...
	* 	PHYDM_DBG(dm,ODM_COMP_CALIBRATION, "delay %d ms for One shot, path A LOK & IQK.\n", IQK_DELAY_TIME_92E); */
	...
	if (!(reg_eac & BIT(28)) &&
		...
		PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "pathA TX IQK is not success\n");
	return result;
}
...
)
{
	...
	* 	PHYDM_DBG(dm,ODM_COMP_CALIBRATION, "path-A Rx IQK modify RXIQK mode table!\n"); */
	...
	odm_set_rf_reg(dm, RF_PATH_A, 0x56, RFREGOFFSETMASK, 0x511e0);
	...
	* 	PHYDM_DBG(dm,ODM_COMP_CALIBRATION, "delay %d ms for One shot, path A LOK & IQK.\n", IQK_DELAY_TIME_92E); */
	...
	if (!(reg_eac & BIT(28)) &&
		...
		odm_set_bb_reg(dm, REG_FPGA0_IQK, 0xffffff00, 0x000000);
		odm_set_rf_reg(dm, RF_PATH_A, 0xdf, RFREGOFFSETMASK, 0x180);
		PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "pathA get TXIMR is not success\n");
		return result;
	...
	/* 1 RX IQK */
	/* modify RXIQK mode table */
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "Do RXIQK!\n");
	...
	/*	PA/PAD control by 0x56, and set = 0x0 */
	odm_set_rf_reg(dm, RF_PATH_A, 0xdf, RFREGOFFSETMASK, 0x980);
	odm_set_rf_reg(dm, RF_PATH_A, 0x56, RFREGOFFSETMASK, 0x510e0);
	...
	* 	PHYDM_DBG(dm,ODM_COMP_CALIBRATION, "delay %d ms for One shot, path A LOK & IQK.\n", IQK_DELAY_TIME_92E); */
	...
	if (!(reg_eac & BIT(27)) &&	/*if Tx is OK, check whether Rx is OK*/
		...
		PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "path A Rx IQK is not success!!\n");
...
)
{
	...
	* 	PHYDM_DBG(dm,ODM_COMP_CALIBRATION, "path-B IQK setting!\n"); */
	...
	odm_set_bb_reg(dm, 0x820, BIT(8), 0x0);
	...
	odm_set_bb_reg(dm, REG_FPGA0_IQK, 0xffffff00, 0x808000);
#if 0
	odm_set_bb_reg(dm, 0xe28, 0xffffff00, 0x000000);
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "path A 0xdf = 0x%x\n", odm_get_rf_reg(dm, RF_PATH_A, 0xdf, RFREGOFFSETMASK));
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "path B 0xdf = 0x%x\n", odm_get_rf_reg(dm, RF_PATH_B, 0xdf, RFREGOFFSETMASK));
	...
	* 	PHYDM_DBG(dm,ODM_COMP_CALIBRATION, "delay %d ms for One shot, path B LOK & IQK.\n", IQK_DELAY_TIME_92E); */
	...
	/*enable path-A PI*/
	odm_set_bb_reg(dm, 0x820, BIT(8), 0x1);
	...
	if (!(reg_eac & BIT(31)) &&
		...
		PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "path B TX IQK is not success\n");
	return result;
}
...
)
{
	...
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "Get RXIQK TXIMR!\n");
	/* modify RXIQK mode table
	* 	PHYDM_DBG(dm,ODM_COMP_CALIBRATION, "path-B Rx IQK modify RXIQK mode table!\n"); */
	odm_set_bb_reg(dm, REG_FPGA0_IQK, 0xffffff00, 0x000000);
	...
	/*	PA/PAD all off */
	odm_set_rf_reg(dm, RF_PATH_B, 0xdf, RFREGOFFSETMASK, 0x980);
	odm_set_rf_reg(dm, RF_PATH_B, 0x56, RFREGOFFSETMASK, 0x511e0);
	...
	* 	PHYDM_DBG(dm,ODM_COMP_CALIBRATION, "delay %d ms for One shot, path B LOK & IQK.\n", IQK_DELAY_TIME_92E); */
	...
	if (!(reg_eac & BIT(31)) &&
		...
		odm_set_bb_reg(dm, REG_FPGA0_IQK, 0xffffff00, 0x000000);
		odm_set_rf_reg(dm, RF_PATH_B, 0xdf, RFREGOFFSETMASK, 0x180);
		PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "pathB get TXIMR is not success\n");
		return result;
	}
	...
	/* modify RXIQK mode table
	* 	PHYDM_DBG(dm,ODM_COMP_CALIBRATION, "path-B Rx IQK modify RXIQK mode table 2!\n"); */
	odm_set_bb_reg(dm, REG_FPGA0_IQK, 0xffffff00, 0x000000);
	...
	/*	PA/PAD all off */
	odm_set_rf_reg(dm, RF_PATH_B, 0xdf, RFREGOFFSETMASK, 0x980);
	odm_set_rf_reg(dm, RF_PATH_B, 0x56, RFREGOFFSETMASK, 0x510e0);
	/*	odm_set_rf_reg(dm, RF_PATH_B, 0x56, RFREGOFFSETMASK, 0x51000 ); */
	...
	* 	PHYDM_DBG(dm,ODM_COMP_CALIBRATION, "delay %d ms for One shot, path B LOK & IQK.\n", IQK_DELAY_TIME_92E); */
	...
	if (!(reg_eac & BIT(30)) &&/*if Tx is OK, check whether Rx is OK*/
		...
		PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "path B Rx IQK is not success!!\n");
	return result;
}
...
)
{
	...
	if (final_candidate == 0xFF)
		...
		if (is_tx_only) {
			PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "_phy_path_a_fill_iqk_matrix_92e only Tx OK\n");
			return;
		}
...
#if 0
	odm_set_bb_reg(dm, 0x520, MASKBYTE2, 0x0);
#else
	for (i = 0 ; i < (IQK_MAC_REG_NUM - 1); i++)
...
)
{
	...
	*/
	...
	/* odm_set_bb_reg(dm, 0x522, MASKBYTE0, 0x7f); */
	/* odm_set_bb_reg(dm, 0x550, MASKBYTE0, 0x15); */
	/* odm_set_bb_reg(dm, 0x551, MASKBYTE0, 0x00); */

	odm_set_bb_reg(dm, 0x520, 0x00ff0000, 0xff);
	/*	odm_set_bb_reg(dm, 0x550, 0x0000ffff, 0x0015); */
...
)
{
	...
	if ((dm->ext_lna) && !(dm->ext_pa)) { /* external LNA / external PA = 1 /0 */
		...
		odm_set_bb_reg(dm, 0x930, 0x0f000000, 0x7);
		odm_set_bb_reg(dm, 0x938, 0xf, 0x7);
		odm_set_bb_reg(dm, 0x938, 0x0f000000, 0x7);
		odm_set_bb_reg(dm, 0x92c, MASKDWORD, 0x00410041);
		...
		odm_set_bb_reg(dm, 0x938, 0xf, 0x7);
		odm_set_bb_reg(dm, 0x938, 0x0f000000, 0x7);
		odm_set_bb_reg(dm, 0x92c, MASKDWORD, 0x00000000);
	}
	...
	*PHYDM_DBG(dm,ODM_COMP_CALIBRATION, "IQK setting!\n");*/
	...
	for (i = 0 ; i < retry_count ; i++) {
		...
		if (path_aok == 0x01) {
			...
			PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "path A Tx IQK Fail!!\n");
#if 0
	...
	PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "_phy_iq_calibrate_8192e() <==\n");
}
...
)
{
	...
	odm_set_bb_reg(dm, 0xce4, 0xc0000000, 0x0);
...
)
{
	...
	if (is_recovery) {
		PHYDM_DBG(dm, ODM_COMP_INIT, "PHY_IQCalibrate_92E: Return due to is_recovery!\n");
	...
	if (final_candidate != 0xff) {
		...
		PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "IQK: TX0_X=%x TX0_Y=%x RX0_X=%x RX0_Y=%x TX1_X=%x TX1_Y=%x RX1_X=%x RX1_Y=%x\n ", rege94, rege9c, regea4, regeac, regeb4, regebc, regec4, regecc);
...
)
{
	...
	/* MAIN: WiFi */
	u32 reg = odm_get_mac_reg(dm, 0x64, MASKDWORD);
	reg &= 0xFFFFF000;
	reg = (is_main) ? (reg | 0x00000100) : (reg | 0x00000004);
	odm_set_mac_reg(dm, 0x64, MASKDWORD, reg);
...
)
{
	...
	if (odm_get_mac_reg(dm, 0x64, 0xFFF) == 0x100)
		return true;
	else
		return false;
