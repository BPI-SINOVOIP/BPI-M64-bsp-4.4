...
)
{
	...
	PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "pwr_tracking_limit_cck=%d      pwr_tracking_limit_ofdm=%d\n", pwr_tracking_limit_cck, pwr_tracking_limit_ofdm);
...
)
{
	...
	PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "Power Tracking tx_rate=0x%X\n", tx_rate);
	PHYDM_DBG(dm, ODM_COMP_TX_PWR_TRACK, "Call get_delta_swing_table_8192e Power Tracking tx_rate=0x%X\n", tx_rate);
...
)
{
	...
	*/
	...
	odm_set_bb_reg(dm, 0x040, 0x20, 0x0);
