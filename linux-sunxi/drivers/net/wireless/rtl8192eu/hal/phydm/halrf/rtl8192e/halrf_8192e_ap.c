...
)
{
	...
	* 	PHYDM_DBG(dm,ODM_COMP_CALIBRATION, "path-A Rx IQK modify RXIQK mode table!\n"); */
	...
	odm_set_rf_reg(dm, RF_PATH_A, 0x56, RFREGOFFSETMASK, 0x51100);
...
)
{
	...
	*/
#if 1
	odm_set_bb_reg(dm, 0x520, MASKBYTE2, 0xff);
#else
	...
	while ((((odm_get_rf_reg(dm, 0x0, 0x0, bMaskDWord) & 0xf0000)  >> 16) == 0x2) && (((odm_get_rf_reg(dm, 0x1, 0x0, bMaskDWord) & 0xf0000)  >> 16) == 0x2))
		ODM_delay_ms(1);
...
)
{
	...
	odm_set_rf_reg(dm, 0x0, 0x0, RFREGOFFSETMASK, 0x10000);
	odm_set_bb_reg(dm, REG_FPGA0_IQK, 0xffffff00, 0x808000);
}
	...
	if (is2T) {
		...
		for (i = 0 ; i < retry_count ; i++) {
			...
			if (path_bok == 0x01) {
				...
				PHYDM_DBG(dm, ODM_COMP_CALIBRATION, "path B Tx IQK Fail!!\n");
				result[t][4] = 0x100;
				result[t][5] = 0x0;
			}
#if 0
