...
void rtw_chk_candidate_peer_notify(_adapter *adapter, struct wlan_network *scanned)
{
	...
	if (!rtw_bss_is_candidate_mesh_peer_and_same_ch(&mlme->cur_network.network, &scanned->network)		
...
int rtw_mesh_addr_resolve(_adapter *adapter, struct xmit_frame *xframe, _pkt *pkt)
{
	...
	is_da_mcast = IS_MCAST(attrib->dst);
	if (!is_da_mcast) {
		struct sta_info *next_hop; 
		bool mpp_lookup = 1;
	
		mpath = rtw_mesh_path_lookup(adapter, attrib->dst);
		if (mpath) {
			mpp_lookup = 0;
			next_hop = rtw_rcu_dereference(mpath->next_hop);
			if (!next_hop
		...
		if (mpp_lookup) {
			mppath = rtw_mpp_path_lookup(adapter, attrib->dst);
			if (mppath)
	...
	if (_rtw_memcmp(adapter_mac_addr(adapter), attrib->src, ETH_ALEN) == _TRUE
		&& !(mppath && _rtw_memcmp(mppath->mpp, attrib->dst, ETH_ALEN) == _FALSE)
	) {
		/* Intra MBSS, no AE */
		if (is_da_mcast) {
			attrib->mesh_frame_mode = MESH_BMCAST_DATA;
			_rtw_memcpy(attrib->ra, attrib->dst, ETH_ALEN);
			_rtw_memcpy(attrib->ta, adapter_mac_addr(adapter), ETH_ALEN);
			_rtw_memcpy(attrib->msa, attrib->src, ETH_ALEN);
		} else {
			attrib->mesh_frame_mode = MESH_UCAST_DATA;
			_rtw_memcpy(attrib->ta, adapter_mac_addr(adapter), ETH_ALEN);
			_rtw_memcpy(attrib->mda, attrib->dst, ETH_ALEN);
			_rtw_memcpy(attrib->msa, attrib->src, ETH_ALEN);
		...
		if (IS_MCAST(mesh_da)) {
			attrib->mesh_frame_mode = MESH_BMCAST_PX_DATA;
			_rtw_memcpy(attrib->ra, attrib->dst, ETH_ALEN);
			_rtw_memcpy(attrib->ta, adapter_mac_addr(adapter), ETH_ALEN);
			...
			_rtw_memcpy(attrib->ta, adapter_mac_addr(adapter), ETH_ALEN);
			_rtw_memcpy(attrib->mda, mesh_da, ETH_ALEN);
			_rtw_memcpy(attrib->msa, adapter_mac_addr(adapter), ETH_ALEN);
			/* RA needs to be resolved */
			res = rtw_mesh_nexthop_resolve(adapter, xframe);
		}
...
void rtw_mesh_tx_build_mctrl(_adapter *adapter, struct pkt_attrib *attrib, u8 *buf)
{
	...
	if (attrib->mfwd_ttl) {
		...
		mctrl->seqnum = (cpu_to_le32(attrib->mfwd_seq));
	} else {
...
fwd_chk:
	...
	if (_rtw_memcmp(da , mda, ETH_ALEN) == _TRUE
		&& _rtw_memcmp(sa , msa, ETH_ALEN) == _TRUE
	) {
		if (is_mda_bmc) {
			xattrib->mesh_frame_mode = MESH_BMCAST_DATA;
			_rtw_memcpy(xattrib->ra, mda, ETH_ALEN);
		} else {
			xattrib->mesh_frame_mode = MESH_UCAST_DATA;
			_rtw_memcpy(xattrib->ra, fwd_ra, ETH_ALEN);
		}
	} else {
		if (is_mda_bmc) {
			xattrib->mesh_frame_mode = MESH_BMCAST_PX_DATA;
			_rtw_memcpy(xattrib->ra, mda, ETH_ALEN);
		} else {
			xattrib->mesh_frame_mode = MESH_UCAST_PX_DATA;
			_rtw_memcpy(xattrib->ra, fwd_ra, ETH_ALEN);
		}
	}
