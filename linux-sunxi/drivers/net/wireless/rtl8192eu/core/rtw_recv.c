...
void count_rx_stats(_adapter *padapter, union recv_frame *prframe, struct sta_info *sta)
{
	...
	if (psta) {
		...
		rtw_soml_byte_update(padapter, pattrib->data_rate, sz);
#endif
	}
	...
	while (a_len > ETH_HLEN) {
		...
		if (sub_pkt == NULL) {
			...
			if (act & RTW_RX_MSDU_ACT_INDICATE) {
				...
				#endif
				rtw_free_xmitframe(&padapter->xmitpriv, fwd_frame);
			}
			break;
		}
		...
		if (act & RTW_RX_MSDU_ACT_FORWARD) {
			_pkt *fwd_pkt = sub_pkt;

			if (act & RTW_RX_MSDU_ACT_INDICATE) {
				fwd_pkt = rtw_os_pkt_copy(sub_pkt);
				if (!fwd_pkt) {
					#ifdef DBG_TX_DROP_FRAME
			...
			if (rtw_xmit_posthandle(padapter, fwd_frame, fwd_pkt) < 0) {
				#ifdef DBG_TX_DROP_FRAME
...
static int recv_process_mpdu(_adapter *padapter, union recv_frame *prframe)
{
	...
	if (pattrib->amsdu) {
		...
		#ifdef CONFIG_RTW_MESH
		if (act & RTW_RX_MSDU_ACT_FORWARD) {
			_pkt *fwd_pkt = prframe->u.hdr.pkt;

			if (act & RTW_RX_MSDU_ACT_INDICATE) {
				fwd_pkt = rtw_os_pkt_copy(prframe->u.hdr.pkt);
				if (!fwd_pkt) {
					#ifdef DBG_TX_DROP_FRAME
					RTW_INFO("DBG_TX_DROP_FRAME %s rtw_os_pkt_copy fail\n", __func__);
					#endif
