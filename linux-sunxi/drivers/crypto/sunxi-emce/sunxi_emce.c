...
{
	if (readl(emce_dev->base_addr + EMCE_VER) == 0x100100)
...
void sunxi_emce_set_task_des(int data_len)
{
	...
	task_des_buf->com_ctrl =
		(reg->key2_len << 12) | (reg->key1_len << 8) |
			(reg->crypto_mode << 4);
	task_des_buf->data_len = data_len;
	for (i = 0; i < 8; i++) {
...
void sunxi_emce_set_task_load(u32 para)
{
	if (emce_dev->emce_vision == EMCE_VISION_1)
		return;
