...
#include "sun8iw6-tdm.h"
#include "sunxi-pcm.h"

#define	DRV_NAME	"sunxi-tdm"
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		if ((params_channels(params)) == 1) {
			regmap_write(sunxi_daudio->regmap,
						SUNXI_DAUDIO_TX0CHMAP0, 0x00);
		} else if ((params_channels(params)) == 2) {
			regmap_write(sunxi_daudio->regmap,
				SUNXI_DAUDIO_TX0CHMAP0, 0x10);
		} else {
			dev_err(sunxi_daudio->dev, "unsupported channel count\n");
#if 0
				regmap_write(sunxi_daudio->regmap,
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:sunxi-tdm");
