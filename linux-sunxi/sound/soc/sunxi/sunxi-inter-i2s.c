	...
	struct snd_soc_dai *dai)
{
	...
	codec_wr_control(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_FAT0,
													0x3, SR, rs_value);

	if (sample_resolution == 24)
		codec_wr_control(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_FCTL,
														0xf, RXOM, 0x1);
	else
		codec_wr_control(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_FCTL,
														0xf, RXOM, 0x5);
								...
								int div_id, int samplerate)
{
	...
	codec_wr_control(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_CLKD,
...
static int sunxi_i2s_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	/*data format*/
	codec_wr_control(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_FAT0, 0x3, FMT, 0);
	...
	struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		codec_wr_control(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_TXCHSEL,
		...
		codec_wr_control(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_TXCHMAP,
												0xff, TX_CH0_MAP, reg_val);
		pr_debug("channels:%d,SUNXI_DA_TXCHMAP:%x\n", substream->runtime->channels,
							readl(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_TXCHMAP));
		/*SDO ON*/
		codec_wr_control(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_CTL,
														0x1, SDO_EN, 1);
		/* I2S0 TX ENABLE */
		codec_wr_control(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_CTL,
														0x1, TXEN, 1);
		...
		codec_wr_control(sunxi_i2s->sunxi_i2s_membase+SUNXI_DA_RXCHMAP,
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	/* enable hub mode for play */
		codec_wr_control(sunxi_i2s_global->sunxi_i2s_membase+SUNXI_DA_FCTL,
															0x1, HUB_EN, 1);
	} else {
		codec_wr_control(sunxi_i2s_global->sunxi_i2s_membase+SUNXI_DA_FCTL,
															0x1, HUB_EN, 0);
