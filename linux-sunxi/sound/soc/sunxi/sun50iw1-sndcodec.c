...
struct mc_private {
	...
	struct snd_soc_jack linein_jack;
...
static int sunxi_audio_init(struct snd_soc_pcm_runtime *runtime)
{
	...
	if (gpio_is_valid(jack_gpios[0].gpio)) {
		ret = snd_soc_card_jack_new(runtime->card, "Linein Jack", SND_JACK_LINEIN, &ctx->linein_jack, jack_pins, 1);
		if (ret) {
			pr_err("can not creat jack for Linein \n");
			return ret ;
		}
		ret = snd_soc_jack_add_gpios(&ctx->linein_jack, ARRAY_SIZE(jack_gpios), jack_gpios);
		if (ret) {
...
static int sunxi_machine_probe(struct platform_device *pdev)
{
	...
	*/
	...
	jack_gpios[0].gpio = of_get_named_gpio_flags(np, "linein_detect", 0, (enum of_gpio_flags *)&config);
	if (!gpio_is_valid(jack_gpios[0].gpio)) {
		jack_gpios[0].gpio = -1;
		pr_err("[ sndcodec ] err:try to get linein_detect failed!\n");
	}
