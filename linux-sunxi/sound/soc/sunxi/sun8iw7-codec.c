/*
 * sound\soc\sunxi\sun8iw11-codec.c
...
static const unsigned int lineout_tlv[] = {
	...
	1, 31, TLV_DB_SCALE_ITEM(-4800, 150, 1),
};
...
static const struct soc_enum left_lineout_enum =
	SOC_ENUM_SINGLE(SUNXI_MIC2G_LINEOUT_CTR, LINEOUTL_SRC,
...
static const struct soc_enum right_lineout_enum =
	SOC_ENUM_SINGLE(SUNXI_MIC2G_LINEOUT_CTR, LINEOUTR_SRC,
...
static void adchpf_config(struct snd_soc_codec *codec)
{
	snd_soc_update_bits(codec, SUNXI_ADC_DRC_HHPFC,
			(0xFF<<ADC_HHPF_CONF), (0xFF<<ADC_HHPF_CONF));
	snd_soc_update_bits(codec, SUNXI_ADC_DRC_LHPFC,
			(0xFFFF<<ADC_LHPF_CONF), (0xFAC1<<ADC_LHPF_CONF));
}
...
static void adchpf_enable(struct snd_soc_codec *codec, bool on)
{
	if (on) {
		snd_soc_update_bits(codec, SUNXI_ADC_DAP_CTR,
				(0x01<<ENADC_DRC), (0x01<<ENADC_DRC));
		snd_soc_update_bits(codec, SUNXI_ADC_DAP_CTR,
				(0x01<<ADC_DRC_HPF_EN), (0x01<<ADC_DRC_HPF_EN));
	} else {
		snd_soc_update_bits(codec, SUNXI_ADC_DAP_CTR,
				(0x01<<ENADC_DRC), (0x0<<ENADC_DRC));
		snd_soc_update_bits(codec, SUNXI_ADC_DAP_CTR,
				(0x01<<ADC_DRC_HPF_EN), (0x0<<ADC_DRC_HPF_EN));
	}
}

static inline void dacdrc_config(struct snd_soc_codec *codec)
{
}

static inline void dachpf_config(struct snd_soc_codec *codec)
{
}

static inline void adcdrc_enable(struct snd_soc_codec *codec, bool on)
{
}

static inline void dacdrc_enable(struct snd_soc_codec *codec, bool on)
{
}

static inline void adcagc_enable(struct snd_soc_codec *codec, bool on)
{
}

static inline void dachpf_enable(struct snd_soc_codec *codec, bool on)
{
}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	static int sun8iw7_lineout_event(struct snd_soc_dapm_widget *w,
				struct snd_kcontrol *k, int event)
{
...
#ifdef SUN8IW7_DAPM_LINEOUT
		snd_soc_update_bits(codec, SUNXI_PAEN_LINEOUT_CTR,
			(1<<LINEOUTEN), (1<<LINEOUTEN));
		mdelay(50);
#endif
		if (sunxi_internal_codec->pa_gpio_used) {
			gpio_set_value(sunxi_internal_codec->pa_gpio, 1);
	/*
	...
	*/
		...
		snd_soc_update_bits(codec, SUNXI_MIC2G_LINEOUT_CTR,
				(1<<LINEOUTL_EN), (1<<LINEOUTL_EN));
		snd_soc_update_bits(codec, SUNXI_MIC2G_LINEOUT_CTR,
				(1<<LINEOUTR_EN), (1<<LINEOUTR_EN));
		msleep(100);
...
#ifdef SUN8IW7_DAPM_LINEOUT
		snd_soc_update_bits(codec, SUNXI_PAEN_LINEOUT_CTR,
				(1<<LINEOUTEN), (0<<LINEOUTEN));
#endif
		if (sunxi_internal_codec->pa_gpio_used) {
			gpio_set_value(sunxi_internal_codec->pa_gpio, 0);
			mdelay(10);
		}
		snd_soc_update_bits(codec, SUNXI_MIC2G_LINEOUT_CTR,
				(1<<LINEOUTL_EN), (0<<LINEOUTL_EN));
		snd_soc_update_bits(codec, SUNXI_MIC2G_LINEOUT_CTR,
				(1<<LINEOUTR_EN), (0<<LINEOUTR_EN));
...
static const struct snd_kcontrol_new sunxi_codec_controls[] = {
	...
	SOC_SINGLE_TLV("LINEIN Mixer volume", SUNXI_LINEIN_GCTR,
				LINEING, 0x7, 1, linein_tlv),
	SOC_DOUBLE_TLV("MIC gain volume", SUNXI_MIC_GCTR,
	...
	SOC_SINGLE_TLV("MIC2 boost volume", SUNXI_MIC2G_LINEOUT_CTR,
...
static const struct snd_kcontrol_new left_output_mixer[] = {
	...
	SOC_DAPM_SINGLE("MIC2 Boost Switch", SUNXI_LOMIX_SRC,
					LMIX_MIC2_BST, 1, 0),
	SOC_DAPM_SINGLE("MIC1 Boost Switch", SUNXI_LOMIX_SRC,
					LMIX_MIC1_BST, 1, 0),
};
...
static const struct snd_kcontrol_new right_output_mixer[] = {
	...
	SOC_DAPM_SINGLE("MIC2 Boost Switch", SUNXI_ROMIX_SRC,
					RMIX_MIC2_BST, 1, 0),
	SOC_DAPM_SINGLE("MIC1 Boost Switch", SUNXI_ROMIX_SRC,
					RMIX_MIC1_BST, 1, 0),
};

static const struct snd_kcontrol_new left_input_mixer[] = {
	SOC_DAPM_SINGLE("ROMIX Switch", SUNXI_LADCMIX_SRC,
					LADC_ROUT_MIX, 1, 0),
	SOC_DAPM_SINGLE("LOMIX Switch", SUNXI_LADCMIX_SRC,
					LADC_LOUT_MIX, 1, 0),
	SOC_DAPM_SINGLE("LINEINL Switch", SUNXI_LADCMIX_SRC,
					LADC_LINEINL, 1, 0),
...
static const struct snd_kcontrol_new right_input_mixer[] = {
	SOC_DAPM_SINGLE("LOMIX Switch", SUNXI_RADCMIX_SRC,
					RADC_LOUT_MIX, 1, 0),
	SOC_DAPM_SINGLE("ROMIX Switch", SUNXI_RADCMIX_SRC,
					RADC_ROUT_MIX, 1, 0),
	SOC_DAPM_SINGLE("LINEINR Switch", SUNXI_RADCMIX_SRC,
					RADC_LINEINR, 1, 0),
...
static const struct snd_soc_dapm_widget sunxi_codec_dapm_widgets[] = {
	SND_SOC_DAPM_AIF_IN_E("DACL", "Playback", 0, SUNXI_DAC_PA_SRC, DACALEN,
				0, sunxi_playback_event,
				SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_POST_PMD),
	SND_SOC_DAPM_AIF_IN_E("DACR", "Playback", 0, SUNXI_DAC_PA_SRC, DACAREN,
				0, sunxi_playback_event,
				SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_PGA("MIC1 PGA", SUNXI_MIC1G_MICBIAS_CTR,
			MIC1_AMPEN, 0, NULL, 0),
	SND_SOC_DAPM_PGA("MIC2 PGA", SUNXI_MIC2G_LINEOUT_CTR,
			MIC2AMPEN, 0, NULL, 0),
	...
	SND_SOC_DAPM_LINE("LINEOUT", sun8iw7_lineout_event),
};
...
static void sunxi_codec_init(struct snd_soc_codec *codec)
{
  ...
  /*enable lineout*/
#ifndef SUN8IW7_DAPM_LINEOUT
	snd_soc_update_bits(codec, SUNXI_PAEN_LINEOUT_CTR, (1<<LINEOUTEN),
	(1<<LINEOUTEN));
#endif
				...
				int cmd, struct snd_soc_dai *dai)
{
	...
	case	SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
			regmap_update_bits(sunxi_internal_codec->regmap,
					SUNXI_DAC_FIFO_CTR,
...
static int sunxi_codec_suspend(struct snd_soc_codec *codec)
{
	...
	if (sunxi_internal_codec->pa_gpio_used)
		sunxi_gpio_iodisable(sunxi_internal_codec->pa_gpio);
...
static int sunxi_codec_resume(struct snd_soc_codec *codec)
{
	...
	if (clk_prepare_enable(sunxi_internal_codec->pllclk)) {
		dev_err(sunxi_internal_codec->dev,
	...
	if (sunxi_internal_codec->pa_gpio_used) {
		gpio_direction_output(sunxi_internal_codec->pa_gpio, 1);
		gpio_set_value(sunxi_internal_codec->pa_gpio, 0);
	}
...
static struct label reg_labels[] = {
	...
	LABEL(SUNXI_LINEOUT_PA_GAT),
	LABEL(SUNXI_LOMIX_SRC),
	LABEL(SUNXI_ROMIX_SRC),
	LABEL(SUNXI_DAC_PA_SRC),
	LABEL(SUNXI_LINEIN_GCTR),
	LABEL(SUNXI_MIC_GCTR),
	LABEL(SUNXI_PAEN_LINEOUT_CTR),
	LABEL(SUNXI_MIC2G_LINEOUT_CTR),
	...
	LABEL(SUNXI_BIAS_DA16_CAL_CTR0),
	LABEL(SUNXI_BIAS_DA16_CAL_CTR1),
	LABEL(SUNXI_DA16_CALI_DATA),
	LABEL(SUNXI_BIAS_CALI_DATA),
	LABEL(SUNXI_BIAS_VERIFY),
	LABEL_END,
};
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = sscanf(buf, "%d,%d,0x%x,0x%x", &rw_flag, &input_reg_group,
			(unsigned int *)&input_reg_offset, (unsigned int *)&input_reg_val);
...
static int  sunxi_internal_codec_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(sunxi_internal_codec->pllclk)) {
		...
		if (IS_ERR_OR_NULL(sunxi_internal_codec->moduleclk)) {
			...
			if (clk_prepare_enable(
				...
				dev_err(&pdev->dev, "moduleclk enable fail\n");
				ret = -EBUSY;
				goto err_pllclk_put;
			}
		}
	...
	if (ret < 0) {
		...
		sunxi_internal_codec->pa_vol = 0;
	} else {
		sunxi_internal_codec->pa_vol = temp_val;
	}

	ret = of_get_named_gpio(np, "audio_pa_ctrl", 0);
	if (ret >= 0) {
		sunxi_internal_codec->pa_gpio_used = 1;
		sunxi_internal_codec->pa_gpio = ret;
		if (!gpio_is_valid(sunxi_internal_codec->pa_gpio)) {
			dev_err(&pdev->dev, "gpio-pa is valid\n");
			ret = -EINVAL;
			goto err_analog_iounmap;
		} else {
			ret = devm_gpio_request(&pdev->dev,
			sunxi_internal_codec->pa_gpio, "pa");
			if (ret) {
				dev_err(&pdev->dev,
					"failed to request pa-spk gpio\n");
				ret = -EBUSY;
				goto err_analog_iounmap;
			} else {
				gpio_direction_output(
				sunxi_internal_codec->pa_gpio, 1);
				gpio_set_value(
				sunxi_internal_codec->pa_gpio, 0);
			}
		}
	} else {
		sunxi_internal_codec->pa_gpio_used = 0;
	}
