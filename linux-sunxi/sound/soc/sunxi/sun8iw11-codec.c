...
static const struct snd_kcontrol_new sunxi_codec_controls[] = {
	...
	SOC_SINGLE_TLV("HPOUT volume", SUNXI_HP_VOLC,
...
static const struct snd_kcontrol_new left_output_mixer[] = {
	...
	SOC_DAPM_SINGLE("LINEINLR Switch", SUNXI_LOMIX_SRC,
					LMIX_LINEINLR, 1, 0),
...
static const struct snd_kcontrol_new right_output_mixer[] = {
	...
	SOC_DAPM_SINGLE("LINEINLR Switch", SUNXI_ROMIX_SRC,
					RMIX_LINEINLR, 1, 0),
...
static const struct snd_kcontrol_new left_input_mixer[] = {
	...
	SOC_DAPM_SINGLE("LINEINL Switch", SUNXI_LADCMIX_SRC,
					LADC_LINEINL, 1, 0),
	SOC_DAPM_SINGLE("LINEINLR Switch", SUNXI_LADCMIX_SRC,
					LADC_LINEINLR, 1, 0),
...
static const struct snd_kcontrol_new right_input_mixer[] = {
	...
	SOC_DAPM_SINGLE("LINEINR Switch", SUNXI_RADCMIX_SRC,
					RADC_LINEINR, 1, 0),
	SOC_DAPM_SINGLE("LINEINLR Switch", SUNXI_RADCMIX_SRC,
					RADC_LINEINLR, 1, 0),
...
static const struct snd_kcontrol_new phoneout_mixer[] = {
	...
	SOC_DAPM_SINGLE("MIC2 Boost Switch", SUNXI_PHONEOUT_CTR,
					PHONEOUTS2, 1, 0),
	SOC_DAPM_SINGLE("MIC1 Boost Switch", SUNXI_PHONEOUT_CTR,
					PHONEOUTS3, 1, 0),
};
...
static int  sunxi_internal_codec_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		...
		sunxi_internal_codec->headphonevol = 59;
	} else {
	...
	pr_debug("headphonevol:%d, spkervol:%d, pa_sleep_time:%d\n",
	...
	if (ret < 0) {
		pr_err("[audio-codec]adcagc_cfg configurations missing.\n");
		ret = -EINVAL;
	} else {
	...
	if (ret < 0) {
		pr_err("[audio-codec]adcdrc_cfg configurations missing.\n");
		ret = -EINVAL;
	} else {
	...
	if (ret < 0) {
		pr_err("[audio-codec]adchpf_cfg configurations missing.\n");
		ret = -EINVAL;
	} else {
	...
	if (ret < 0) {
		pr_err("[audio-codec]dacdrc_cfg configurations missing.\n");
		ret = -EINVAL;
	} else {
	...
	if (ret < 0) {
		pr_err("[audio-codec]dachpf_cfg configurations missing.\n");
		ret = -EINVAL;
	} else {
	...
	if (ret >= 0) {
		...
		if (!gpio_is_valid(sunxi_internal_codec->spk_gpio)) {
			...
			if (ret) {
				dev_err(&pdev->dev,
					"failed to request gpio-spk gpio\n");
				ret = -EBUSY;
				goto err_analog_iounmap;
			} else {
				gpio_direction_output(
					sunxi_internal_codec->spk_gpio, 1);
				gpio_set_value(
					sunxi_internal_codec->spk_gpio, 0);
			}
		}
	} else {
