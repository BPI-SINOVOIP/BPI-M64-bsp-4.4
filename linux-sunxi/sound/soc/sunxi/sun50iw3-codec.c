...
static int codec_init(struct sunxi_codec *sunxi_internal_codec)
{
	...
	snd_soc_update_bits(
	    sunxi_internal_codec->codec, SPKL_CTRL, (0x1f << SPKOUT_VOL),
	...
	/*snd_soc_update_bits(sunxi_internal_codec->codec, EARPIECE_CTRL1,*/
	/*(0x1f<<ESP_VOL),*/
	/*(sunxi_internal_codec->gain_config.earpiecevol<<ESP_VOL));*/
	snd_soc_update_bits(
...
static const struct snd_kcontrol_new ac_loutmix_controls[] = {
	...
	SOC_DAPM_SINGLE("PHONEINP-PHONEINN Switch", OL_MIX_CTRL,
						LMIXMUTEPHONEPN, 1, 0),
};
...
static const struct snd_kcontrol_new ac_routmix_controls[] = {
	...
	SOC_DAPM_SINGLE("PHONEINN-PHONEINP Switch", OR_MIX_CTRL,
						LMIXMUTEPHONENP, 1, 0),
};
...
static const struct soc_enum ac_lineout_func_enum =
	SOC_ENUM_SINGLE(LINEOUT_CTRL0, LINEOUTR_SRC,
			ARRAY_SIZE(ac_lineout_func_sel), ac_lineout_func_sel);
...
static const struct snd_kcontrol_new ac_ladcmix_controls[] = {
	...
	SOC_DAPM_SINGLE("PHONINP Switch", L_ADCMIX_SRC,
					LADCMIXMUTEPHONEP, 1, 0),
	SOC_DAPM_SINGLE("PHONINP-PHONINN Switch", L_ADCMIX_SRC,
					LADCMIXMUTEPHONEPN, 1, 0),
};
...
static const struct snd_kcontrol_new ac_radcmix_controls[] = {
	...
	SOC_DAPM_SINGLE("PHONINN Switch", R_ADCMIX_SRC,
					LADCMIXMUTEPHONEN, 1, 0),
	SOC_DAPM_SINGLE("PHONINN-PHONINP Switch", R_ADCMIX_SRC,
					LADCMIXMUTEPHONENP, 1, 0),
};
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk(KERN_NOTICE "ret:%d, reg_group:%d,"
			" reg_offset:%d, reg_val:0x%x\n",
...
static int sunxi_internal_codec_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(sunxi_internal_codec->pllclk)
		...
		if (clk_set_parent(sunxi_internal_codec->moduleclk,
			...
			pr_err("try to set parent of sunxi_spdif->moduleclk"
					"to sunxi_spdif->pllclk failed!"
					" line = %d\n", __LINE__);
	...
	if (ret < 0) {
		pr_err("[audio-codec]spkervol  missing or invalid.\n");
		ret = -EINVAL;
		goto err1;
	} else {
	...
	if (ret < 0) {
		pr_err("[audio-codec]earpiecevol missing or invalid.\n");
		ret = -EINVAL;
		goto err1;
	} else {
...
static void sunxi_internal_codec_shutdown(struct platform_device *pdev)
{
	...
	/*
	* avoid some case when device register to device module but not register
	* to ALSA,this will happen when shutdown at kernel start stage
	*/
