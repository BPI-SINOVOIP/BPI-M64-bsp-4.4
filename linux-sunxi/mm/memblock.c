...
static int memblock_debug_show(struct seq_file *m, void *private)
{
	...
	for (i = 0; i < type->cnt; i++) {
		...
		if (sizeof(phys_addr_t) == 4)
			seq_printf(m, "0x%08lx..0x%08lx, size:%dK\n",
				   (unsigned long)reg->base,
				   (unsigned long)(reg->base + reg->size - 1), reg->size >> 10);
		else
			seq_printf(m, "0x%016llx..0x%016llx, size:%dK\n",
				   (unsigned long long)reg->base,
				   (unsigned long long)(reg->base + reg->size - 1), reg->size >> 10);
