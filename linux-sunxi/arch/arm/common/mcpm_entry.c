...
extern int gic_cpu_if_down(unsigned int gic_nr);
...
extern int sun8i_is_wfi_mode(int pcpu, int pcluster);
int mcpm_wait_for_cpu_powerdown(unsigned int cpu, unsigned int cluster)
{
	...
	for (i = 0; i < 1000; i++) {
		...
		if (cpumask_test_cpu(cpu, &dead_cpus[cluster]) &&
		sun8i_is_wfi_mode(cpu, cluster)) {
			local_irq_disable();
			arch_spin_lock(&mcpm_lock);
			platform_ops->wait_for_powerdown(cpu, cluster);
			arch_spin_unlock(&mcpm_lock);
			local_irq_enable();
...
int mcpm_cpu_clear_status(int cpu, int pcpu, int pcluster)
{
	cpumask_clear_cpu(pcpu, &dead_cpus[pcluster]);
