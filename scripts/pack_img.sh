...
${BOARD_DIR}/bin/bl31.bin:image/monitor.fex
...
function do_prepare()
{
	...
	IMG_NAME="${PACK_PLATFORM}_${PACK_BOARD}_${PACK_DEBUG}.img"
...
function do_ini_to_dts()
{
    ...
    local DTC_DEP_FILE=${PACK_TOPDIR}/linux-sunxi/arch/$ARCH/boot/dts/sunxi/.${PACK_CHIP}-${PACK_DTB}.dtb.d.dtc.tmp
	local DTC_SRC_PATH=${PACK_TOPDIR}/linux-sunxi/arch/$ARCH/boot/dts/sunxi/
	local DTC_SRC_FILE=${PACK_TOPDIR}/linux-sunxi/arch/$ARCH/boot/dts/sunxi/.${PACK_CHIP}-${PACK_DTB}.dtb.dts.tmp

	local DTC_COMPILER=${PACK_TOPDIR}/linux-sunxi/scripts/dtc/dtc
	...
	if [ ! -f $DTC_DEP_FILE ]; then
		...
		DTC_DEP_FILE=${PACK_TOPDIR}/linux-sunxi/arch/$ARCH/boot/dts/sunxi/.${PACK_CHIP}-soc${SUFFIX}.dtb.d.dtc.tmp
		DTC_SRC_FILE=${PACK_TOPDIR}/linux-sunxi/arch/$ARCH/boot/dts/sunxi/.${PACK_CHIP}-soc${SUFFIX}.dtb.dts.tmp
