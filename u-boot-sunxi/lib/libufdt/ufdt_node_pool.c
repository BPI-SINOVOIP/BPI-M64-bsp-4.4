...
/* Define DEBUG_DISABLE_POOL to use malloc and free directly */
...
static struct ufdt_node_pool_block_header *_ufdt_node_pool_create_block(void)
{
  ...
  for (entry_buf = block_buf_start; entry_buf < block_buf_end;
...
static void _ufdt_node_pool_destory_block(struct ufdt_node_pool_block_header *block)
{
  dto_free(block);
}

static void *_ufdt_node_pool_block_alloc(struct ufdt_node_pool_block_header *block)
{
