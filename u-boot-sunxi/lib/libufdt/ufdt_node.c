...
int ufdt_node_add_child(struct ufdt_node *parent, struct ufdt_node *child)
{
  if (!parent || !child)
		return -1;
  if (ufdt_node_tag(parent) != FDT_BEGIN_NODE)
		return -1;
													...
													const char *name, int len)
{
  ...
  for_each_node(it, node) {
    if (ufdt_node_name_eq(*it, name, len))
		return *it;
  }
  return NULL;
}
...
struct ufdt_node *ufdt_node_get_property_by_name_len(const struct ufdt_node *node,
													const char *name, int len)
{
  if (!node)
		return NULL;
...
bool ufdt_node_name_eq(const struct ufdt_node *node, const char *name, int len)
{
  ...
  if (!name)
			return false;
  if (dto_strncmp(ufdt_node_name(node), name, len) != 0)
			return false;
  if (ufdt_node_name(node)[len] != '\0')
			return false;
  return true;
}
