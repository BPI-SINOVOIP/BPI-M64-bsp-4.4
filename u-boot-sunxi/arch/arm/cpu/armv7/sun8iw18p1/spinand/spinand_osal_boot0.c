...
void SPINAND_PIORequest(__u32 nand_index)
{
	...
	/*spi0_cs1, pc6, pc5, spi0_miso, spi_cs0, spi0_mosi, pc1, spi0_clk*/
	writel(0x47744474, GPIO_BASE_ADDR + 0x48);
	/*(pc15, spi0_wp, output), pc14~pc8*/
	writel(0x17777777, GPIO_BASE_ADDR + 0x4c);
	/*(pc16, spi0_hold, output), output*/
	writel(0x1, GPIO_BASE_ADDR + 0x50);
	/*pc_dat, configure spi0_wp and spi0_hold output*/
	writel(0x18000, GPIO_BASE_ADDR + 0x58);
	/*set (cs0,pc3) (cs1,pc7) Pull-up*/
	writel(0x4040, GPIO_BASE_ADDR + 0x64);
#else
