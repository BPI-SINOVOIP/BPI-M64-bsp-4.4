					...
					u8 *src_addr, u32 src_len)
{
	...
	task0.key_descriptor = ((u32)total_package_len >> SS_BIT_SHIFT);
	task0.iv_descriptor = 0;
	task0.ctr_descriptor = 0;
	task0.data_len = total_bit_len;
	task0.source[0].addr = ((uint)src_addr >> SS_BIT_SHIFT);
	task0.source[0].length = ALIGN(src_len, 4)/4;
	task0.destination[0].addr = ((uint)p_sign >> SS_BIT_SHIFT);
	...
	ss_set_drq(((u32)&task0 >> SS_BIT_SHIFT));
				   ...
				   u8 *src_addr, u32 src_len)
{
	...
	task0.source[0].addr = ((uint)p_e >> SS_BIT_SHIFT);
	task0.source[0].length = data_word_len;
	task0.source[1].addr = ((uint)p_n >> SS_BIT_SHIFT);
	task0.source[1].length = data_word_len;
	task0.source[2].addr = ((uint)p_src >> SS_BIT_SHIFT);
	...
	task0.destination[0].addr = ((uint)p_dst >> SS_BIT_SHIFT);
