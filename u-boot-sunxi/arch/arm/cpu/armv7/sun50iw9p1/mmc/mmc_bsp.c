	...
	if (boot0_para & BOOT0_PARA_USE_INTERNAL_DEFAULT_TIMING_PARA) {
		...
		if (boot0_para & BOOT0_PARA_USE_EXTERNAL_INPUT_TIMING_PARA) {
			...
			if ((sdc_no != 2) || (spd_md_id > 4) || (freq_id > 5)) {
				mmcinfo
				    ("wrong input para: mmc %d/2, mode %d/[0,4], freq %d/[0,5]\n",
				 ...
				 u32 freq_id)
{
	...
	if (mode == SUNXI_MMC_TIMING_MODE_1) {
		...
		odly = 0xff;
		sdly = 0xff;
		if (mmc_get_timing_cfg
		    (mmchost->mmc_no, spd_md, freq, &odly, &sdly)) {
		...
		if (spd_md == HS400) {
			...
			dsdly = 0xff;
			if (mmc_get_timing_cfg
			    (mmchost->mmc_no, spd_md, freq, &odly, &dsdly)) {
				mmcinfo("get timing para error %d\n", __LINE__);
				ret = -1;
				goto OUT;
			}
...
static int mmc_trans_data_by_dma(struct mmc *mmc, struct mmc_data *data)
{
	...
	for (i = 0; i < buff_frag_num; i++, des_idx++) {
		...
		if (i == buff_frag_num - 1) {
			...
			pdes[des_idx].buf_addr_ptr2 = ((size_t) &pdes[des_idx + 1]) >> 2;
		}
		mmcdbg("mmc %d frag %u, remain %u, des[%u](%x): "
		       "[0] = %x, [1] = %x, [2] = %x, [3] = %x\n",
		       mmchost->mmc_no, i, remain, des_idx,
		       (u32) &pdes[des_idx],
