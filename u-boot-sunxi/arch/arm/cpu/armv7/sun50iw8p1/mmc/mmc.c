...
int mmc_berase(int dev_num, unsigned long start, unsigned blkcnt)
{
	...
	if (!mmc) {
		mmcinfo("Can not find mmc dev %u\n", dev_num);
		return -1;
	}

	memset(src, 0, 512 * blkcnt);
	mmcinfo("mmc %u erase blk %u ~ %u\n", mmc->control_num, start,
		start + blkcnt - 1);
	err = mmc_bwrite(dev_num, start, blkcnt, src);
	/*mmcinfo("erase flag%d",err); */
	if (!err) {
		mmcinfo("mmc %u erase failed\n", mmc->control_num);
	}
...
mmc_write_blocks(struct mmc *mmc, unsigned long start, unsigned blkcnt,
		 const void *src)
{
	struct mmc_cmd cmd;
	...
	if ((start + blkcnt) > mmc->lba) {
		mmcinfo("mmc %u: block number 0x%lx exceeds max(0x%lx)\n",
			mmc->control_num, start + blkcnt, mmc->lba);
		return 0;
	}
	...
	if (mmc_send_cmd(mmc, &cmd, &data)) {
		mmcinfo("mmc %u mmc write failed\n", mmc->control_num);
		return 0;
	}
...
mmc_bwrite(int dev_num, unsigned long start, unsigned blkcnt, const void *src)
{
	...
	if (!mmc) {
		mmcinfo("Can not found device %u\n", dev_num);
		return 0;
	}
...
int mmc_change_freq(struct mmc *mmc)
{
	...
	do {
		err =
		    mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING,
			       1);
		if (!err) {
			break;
		}
...
int sd_change_freq(struct mmc *mmc)
{
	...
	if (err) {
		mmcinfo
		    ("update clock failed after send cmd6 to switch to sd high speed mode\n");
		return err;
	}
...
int mmc_startup(struct mmc *mmc)
{
	...
	char ext_csd[512];
	int timeout = 1000;
	char *spd_name[] = { "DS26/SDR12", "HSSDR52/SDR25", "HSDDR52/DDR50", "HS200/SDR104",
     "HS400" };
	...
	if (IS_SD(mmc)) {
		...
		if (mmc->card_caps & MMC_MODE_8BIT) {
			...
			if ((mmc->card_caps & MMC_MODE_DDR_52MHz)) {
				...
				if (err) {
					mmcinfo
					    ("mmc %u switch bus width8 failed\n",
					     mmc->control_num);
					return err;
				}
				mmc_set_bus_width(mmc, 8);
...
#if 0
	if (IS_SD(mmc)) {
		sprintf(mmc->revision, "PRV %u.%u", mmc->cid[2] >> 28,
			(mmc->cid[2] >> 24) & 0xf);
	} else {
		sprintf(mmc->revision, "PRV %u.%u", (mmc->cid[2] >> 20) & 0xf,
			(mmc->cid[2] >> 16) & 0xf);
	}
	mmcinfo("%s\n", mmc->revision);
...
int mmc_init(struct mmc *mmc)
{
	...
	if (err) {
		...
		if (err) {
			mmcinfo
			    ("mmc %u Card did not respond to voltage select!\n",
			     mmc->control_num);
	...
	if (priv_info->card_type == CARD_TYPE_SD) {
		...
		if (err) {
			mmcinfo
			    ("SD card %u Card did not respond to voltage select!\n",
			     mmc->control_num);
			mmcinfo("***SD/MMC %u init error!!!***\n",
				mmc->control_num);
			return UNUSABLE_ERR;
		...
		if (err) {
			mmcinfo
			    ("MMC card %u Card did not respond to voltage select!\n",
			     mmc->control_num);
			mmcinfo("***SD/MMC %u init error!!!***\n",
				mmc->control_num);
			return UNUSABLE_ERR;
