...
const __s16 para0[6][4] = {
    {0x00, 0x06, 0x0A, 0x06},    {0x00, -0x03, -0x07, -0x08},
    {0x00, -0x06, -0x0D, -0x0F}, {0x00, -0x0B, -0x14, -0x17},
    {0x00, 0x00, -0x1A, -0x1E},  {0x00, 0x00, -0x20, -0x25} };
const __s16 para1[6][4] = {
    {0x00, 0x06, 0x0a, 0x06},    {0x00, -0x03, -0x07, -0x08},
    {0x00, -0x06, -0x0d, -0x0f}, {0x00, -0x09, -0x14, -0x17},
    {0x00, 0x00, -0x1a, -0x1e},  {0x00, 0x00, -0x20, -0x25} };
			 ...
			 __u8 read_mode)
{
	...
	if (read_mode == 1) {
		...
		NFC_WRITE_REG(
		    NFC_REG_ECC_CTL,
		    (NFC_READ_REG(NFC_REG_ECC_CTL) & (~NFC_ECC_MODE)) |
			ecc_mode_temp);
...
*			  *mainbuf	-- point to data buffer address,
it must be four bytes align.
*                     *sparebuf	-- point to spare buffer address.
*                     dma_wait_mode	-- how to deal when dma start, 0 =
wait till dma finish,
							    1 = dma interrupt
was set and now sleep till interrupt occurs.
*			  page_mode  -- 0 = normal command, 1 = page mode
* Returns		: 0 = success.
			  1 = success & ecc limit.
			  -1 = too much ecc err.
* Notes		:  if align page data required，page command mode is
used., if the commands do
...
* Arguments	: nand_info -- structure with flash bus width,pagesize
,serial access mode and other configure parametre
...
__s32 NFC_Init(NFC_INIT_INFO *nand_info)
{
	...
	writel(0x88000306, NAND_CLK_BASE_ADDR + 0x814);
	NFC_SetEccMode(0);
...
__s32 _vender_get_param(__u8 *para, __u8 *addr, __u32 count)
{
	...
	if (read_retry_mode < 0x10) /* hynix mode*/ {
		cmd_r = 0x37;
		 ...
		 (read_retry_mode < 0x20)) /* toshiba mode*/ {
		return ret;
	}
#endif
...
__s32 _vender_set_param(__u8 *para, __u8 *addr, __u32 count)
{
	...
	if (read_retry_mode < 0x10) /* hynix mode*/ {
		...
		cmd_done1 = 0xff;
		   ...
		   (read_retry_mode < 0x50)) /* micron mode*/ {
		cmd_w = 0xef;
		cmd_end = 0xff;
		cmd_done0 = 0xff;
		cmd_done1 = 0xff;
	...
	for (i = 0; i < count; i++) {
		if ((read_retry_mode >= 0x0) && (read_retry_mode < 0x10)) {
			...
			/* para[i]);*/
			   ...
			   (read_retry_mode < 0x50)) /* micron read retry mode*/ {
			/* send cmd to set param*/
...
__s32 _vender_get_param_otp_hynix(__u8 *para, __u8 *addr, __u32 count)
{
	...
	for (j = 0; j < 8; j++) {
		...
		for (i = 0; i < 64; i++) {
			...
			param_reverse[i] =
			    NFC_READ_RAM_B(NFC_RAM0_BASE + 128 * j + 64 + i);
...
{
	__u32 bit, byte;
	__u32 cnt_1; /* the total number of bit '1' on specified bit position in
			all input bytes */
	__u32 cnt_0;
			  ...
			  __u8 *otp)
{
	...
	if (((*rr_cnt) != 8) || ((*rr_reg_cnt) != 4)) {
		PRINT("read retry value from otp error: rr_cnt %d rr_reg_cnt "
		      "%d!\n",
		      (*rr_cnt), (*rr_reg_cnt));
		return -1;
	}
	...
	for (nbyte = 0; nbyte < rr_tab_size; nbyte++) {
		...
		if (ret < 0) {
			PRINT("_get_read_retry_parameters, get the %d-th byte "
			      "of rr table failed!\n",
			      nbyte);
			err_flag = 1;
			break;
...
__s32 _read_otp_info_hynix(__u32 chip, __u8 *otp_chip)
{
	...
	for (ndie = 0; ndie < 1; ndie++) {
		...
		if ((ndie == 1) || (ndie == 2) || (ndie == 3)) {
			...
			_set_addr(&abuf[0], 3);
			cfg =
			    (NFC_SEND_CMD1 | NFC_SEND_ADR | (0x2 << 16) | 0x78);
			_wait_cmdfifo_free();
			NFC_WRITE_REG(NFC_REG_CMD, cfg);
			_wait_cmd_finish();
		}
		...
		/* dummy read(address don't care) 0x00 cmd + 0x0 addr + 0x30
		 * cmd*/
		abuf[0] = 0x00;
		abuf[1] = 0x00;
		abuf[2] = 0x00;
...
__s32 NFC_ReadRetry(__u32 chip, __u32 retry_count, __u32 read_retry_type)
{
	...
	if (read_retry_mode < 0x10) /* for hynix read retry mode*/ {
		if ((read_retry_mode == 0) || (read_retry_mode == 1)) {
			...
			ret =
			    _vender_set_param(&param[0], &read_retry_reg_adr[0],
					      read_retry_reg_num);
		} else if (read_retry_mode == 4) {
			for (i = 0; i < read_retry_reg_num; i++)
				param[i] =
				    hynix16nm_read_retry_otp_value[chip]
								  [retry_count]
								  [i];
		   ...
		   (read_retry_mode < 0x50)) /* for micron readretry mode*/ {
		for (i = 0; i < read_retry_reg_num; i++)
...
__s32 NFC_ReadRetryInit(__u32 read_retry_type)
{
	...
	} else if (read_retry_mode ==
		   0x41) /* mode 0x41
			    L95B_128_256_512Gb_1Tb_2Tb_Async_Sync_NAND*/ {
		read_retry_reg_adr[0] = 0x89;
		for (i = 0; i < read_retry_cycle; i++) {
			for (j = 0; j < read_retry_reg_num; j++) {
	...
	if (read_retry_mode < 0x10) /* hynix read retry mode*/ {
		if ((read_retry_mode == 0x0) || (read_retry_mode == 0x1)) {
			...
			for (i = 0; i < 8; i++) {
				for (j = 0; j < 8; j++) {
					default_value[8 * i + j] =
					    hynix_read_retry_otp_value[chip][i]
								      [j];
				}
			}
		} else if (read_retry_mode == 0x4) {
			...
			while (flag == 0) {
				PHY_DBG(
				    "_vender_get_param_otp_hynix time %d!\n",
				    Count);
			...
			for (i = 0; i < 8; i++) {
				for (j = 0; j < 4; j++) {
					default_value[4 * i + j] =
					    hynix16nm_read_retry_otp_value[0][i]
									  [j];
				}
			}
		}
	...
	if (read_retry_mode < 0x10) /* hynix read retry mode*/ {
		for (i = 0; i < read_retry_reg_num; i++) {
			if ((read_retry_mode == 0x0) ||
				...
				default_value[i] =
				    hynix16nm_read_retry_otp_value[chip][0][i];
		}
