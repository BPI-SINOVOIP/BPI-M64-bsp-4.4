...
/* define the count of the data block in a zone, the value is based on the*/
/* ration of the invalid blocks*/
...
/* define the count of the log block in a zone, the value is configurable,*/
...
#define NORMAL_MERGE_MODE 0x00 /* normal merge mode, there is not enough log*/
			       /* item*/
...
*
*Arguments  : nLba      the logic block address on the logic area from where to
*read;
*             nLength   the size of the data need be read, based on sector;
*             pBuf      the pointer to the buffer where will store the data
*readout of nand.
*
*Return     : read result;
*               = 0     read successful;
...
*             pBuf      the pointer to the buffer where stored the data write to
*nand flash.
*
*Return     : write result;
*               = 0     write successful;
...
*             nBitmap   the bitmap of the sectors in the page which need be read
*data;
*             pBuf      the pointer to the buffer where will store the data read
*out.
*
...
*             nBitmap   the bitmap of sectors in the page which need be write,
*it is always full;
...
*
*Description: Repair the logic block whose data has reach the limit of the
*ability of
...
*
*Notes      : if read a physical page millions of times, there may be some bit
*error in
*             the page, and the error bit number will increase along with the
*read times,
*             so, if the number of the error bit reachs the ECC limit, the data
*should be
...
*
*Notes      : The erase cycle of a physical block is limited, if the erase cycle
*overun this
*             limit, the physical block may be invalid. so a policy is needed to
*equate the
...
*                           = 0     normal merge, the log block  table is not
*full;
*                           = 1     special merge, the log block table is full;
*             nLogicBlk     the number of the logical block, which need be
*merged.
*
*Return     : merge result;
...
*
*Arguments  : nZone         the number of the zone which the super block
*belonged to;
...
*
*Description: Calculate the paramter for physical operation with the number of
*zone, number of
...
*
*Arguments  : nLogBlkPst    the position of the log block in the log block
*table.
*
*Return     : switch result;
