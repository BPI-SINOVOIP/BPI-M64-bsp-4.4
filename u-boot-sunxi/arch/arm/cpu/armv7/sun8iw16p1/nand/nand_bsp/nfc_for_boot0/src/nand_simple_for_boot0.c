...
/****************************************************************************
*********************translate (block + page+ sector) into 5 bytes
*addr***************
*****************************************************************************/
...
__s32 _read_single_page(struct boot_physical_param *readop, __u8 read_mode)
{
	...
	if (SUPPORT_READ_RETRY && (read_mode == 0)) {
		...
		if (k > 0) {
			PHY_DBG("[Read_single_page] NFC_ReadRetry %d cycles, "
				"chip = %d, block = %d ",
			...
			if (ret == -ERR_ECC)
				PHY_DBG("ecc error!\n");
			/* PHY_DBG("spare buf: %x, %x, %x, %x, %x, %x, %x,*/
			/* %x\n",*/
...
__s32 PHY_GetDefaultParam(__u32 bank)
{
	...
	if ((READ_RETRY_MODE == 2) || (READ_RETRY_MODE == 3)) {
		/* PHY_DBG("uboot_start_block %d uboot_next_block*/
		/* %d\n",uboot_start_block,uboot_next_block);*/
		/* PHY_DBG("nand_specialinfo_page %d nand_specialinfo_offset*/
		...
		for (i = uboot_start_block; i < uboot_next_block; i++) {
			...
			if ((ret >= 0) && (oob[0] == 0xff) &&
				...
				if (0 == check_nand_special_info(
					...
					otp_ok_flag = 1;
					PHY_DBG("find good otp value in chip "
						"%d, block %d \n",
...
__s32 PHY_ChangeMode(__u8 serial_mode)
{
	...
	if (!ndfc_dma_desc) {
		ndfc_dma_desc =
		    (_ndfc_dma_desc_t *)MALLOC(sizeof(_ndfc_dma_desc_t) * 5);
		if (!ndfc_dma_desc)
			return -1;
	}
...
*               = 0     read chip ID successful, the chip ID has been stored in
*given buffer;
...
*
*Description: Sync nand flash operation, check nand flash program/erase
*operation status.
*
*Arguments  : nBank     the number of the bank which need be synchronized;
*             bMode     the type of synch,
*                       = 0     sync the chip which the bank belonged to, wait
*the whole chip
*                               to be ready, and report status. if the chip
*support cacheprogram,
*                               need check if the chip is true ready;
*                       = 1     only sync the the bank, wait the bank ready and
*report the status,
*                               if the chip support cache program, need not
...
__s32 PHY_SynchBank(__u32 nBank, __u32 bMode)
{
	...
	while (1) {
		...
		if (status < 0) {
			PHY_ERR("PHY_SynchBank : read status invalid ,chip = "
				"%x, bank = %x, cmd value = %x, status = %x\n",
		...
		if (timeout-- < 0) {
			PHY_ERR("PHY_SynchBank : wait nand ready timeout,chip "
				"= %x, bank = %x, cmd value = %x, status = "
				"%x\n",
				chip, nBank, cmd_value, status);
			return -ERR_TIMEOUT;
		}
	...
	if (status & NAND_OPERATE_FAIL) {
		PHY_ERR("PHY_SynchBank : last W/E operation fail,chip = %x, "
			"bank = %x, cmd value = %x, status = %x\n",
			chip, nBank, cmd_value, status);
		ret = NAND_OP_FALSE;
	}
