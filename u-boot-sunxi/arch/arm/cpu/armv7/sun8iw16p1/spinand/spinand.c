...
__s32 SpiNand_Read(__u32 sector_num, void *buffer, __u32 N)
{
	...
	para.block = blk_num;
	start_page =
	    (sector_num % (SPN_BLOCK_SIZE >> SCT_SZ_WIDTH)) / scts_per_page;
	para.oobbuf = oob_buf;
...
*备    注: 1. 本函数只载入，不校验
*******************************************************************************/
__s32 Spinand_Load_Boot1_Copy(__u32 start_blk, void *buf, __u32 size,
			      __u32 blk_size, __u32 *blks)
{
	__u32 buf_base;
	__u32 buf_off;
	__u32 size_loaded;
	...
	for (blk_num = start_blk, buf_base = (__u32)buf, buf_off = 0;
		...
		if (SpiNand_Read(cur_blk_base >> SCT_SZ_WIDTH, (void *)buf_base,
				 size_loaded >> SCT_SZ_WIDTH) != NAND_OP_TRUE)
			return NAND_OP_FALSE;
