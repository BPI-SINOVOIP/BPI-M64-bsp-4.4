...
/* 	__u32			   *tmp_group_port_addr; */
	...
	for (first_port = 0; first_port < group_count_max; first_port++) {
		...
		port_num =
		    tmp_user_gpio_data->port_num; /* 读出端口中的某一个GPIO */
		if (!port) {
			continue;
		}
		...
		tmp_group_func_addr =
		    PIO_REG_CFG(port, port_num_func); /* 更新功能寄存器地址 */
		tmp_group_pull_addr =
		    PIO_REG_PULL(port, port_num_pull); /* 更新pull寄存器 */
		tmp_group_dlevel_addr =
		    PIO_REG_DLEVEL(port, port_num_pull); /* 更新level寄存器 */
		...
		if (set_gpio) {
			tmp_group_func_data |=
			    (tmp_user_gpio_data->mul_sel & 0x07) << tmp_val;
		}
	...
	for (i = first_port + 1; i < group_count_max; i++) {
		tmp_user_gpio_data =
		    gpio_list + i; /* gpio_set依次指向用户的每个GPIO数组成员 */
		port = tmp_user_gpio_data->port; /* 读出端口数值 */
		port_num =
		    tmp_user_gpio_data->port_num; /* 读出端口中的某一个GPIO */
		if (!port) {
		...
		if ((port_num_pull != pre_port_num_pull) ||
			...
			GPIO_REG_WRITE(
			    tmp_group_func_addr,
			    tmp_group_func_data); /* 回写功能寄存器 */
			GPIO_REG_WRITE(
			    tmp_group_pull_addr,
			    tmp_group_pull_data); /* 回写pull寄存器 */
			GPIO_REG_WRITE(
			    tmp_group_dlevel_addr,
			    tmp_group_dlevel_data); /* 回写driver level寄存器 */
			if (data_change) {
				data_change = 0;
				GPIO_REG_WRITE(
				    tmp_group_data_addr,
				    tmp_group_data_data); /* 回写data寄存器 */
			}

			tmp_group_func_addr = PIO_REG_CFG(
			    port, port_num_func); /* 更新功能寄存器地址 */
			tmp_group_pull_addr = PIO_REG_PULL(
			    port, port_num_pull); /* 更新pull寄存器 */
			tmp_group_dlevel_addr = PIO_REG_DLEVEL(
			    port, port_num_pull); /* 更新level寄存器 */
			tmp_group_data_addr =
			    PIO_REG_DATA(port); /* 更新data寄存器 */

			tmp_group_func_data =
			    GPIO_REG_READ(tmp_group_func_addr);
			tmp_group_pull_data =
			    GPIO_REG_READ(tmp_group_pull_addr);
			tmp_group_dlevel_data =
			    GPIO_REG_READ(tmp_group_dlevel_addr);
			tmp_group_data_data =
			    GPIO_REG_READ(tmp_group_data_addr);
		} else if (pre_port_num_func != port_num_func) {
			/* 如果发现当前引脚的功能寄存器不一致 */
			GPIO_REG_WRITE(
			    tmp_group_func_addr,
			    tmp_group_func_data); /* 则只回写功能寄存器 */
			tmp_group_func_addr = PIO_REG_CFG(
			    port, port_num_func); /* 更新功能寄存器地址 */
		...
		/* 保存当前硬件寄存器数据 */
		pre_port_num_pull =
		    port_num_pull; /* 设置当前GPIO成为前一个GPIO */
		pre_port_num_func = port_num_func;
		pre_port = port;
		...
		if (tmp_user_gpio_data->mul_sel >= 0) {
			...
			if (set_gpio) {
				tmp_group_func_data |=
				    (tmp_user_gpio_data->mul_sel & 0x07)
				    << tmp_val;
			}
		}
		/* 根据pull的值决定是否更新pull寄存器 */
	...
	/* 只要更新过寄存器地址，就可以对硬件赋值 */
	if (tmp_group_func_addr) { /* 那么把所有的值全部回写到硬件寄存器 */
		GPIO_REG_WRITE(tmp_group_func_addr,
			       tmp_group_func_data); /* 回写功能寄存器 */
		GPIO_REG_WRITE(tmp_group_pull_addr,
			       tmp_group_pull_data); /* 回写pull寄存器 */
		GPIO_REG_WRITE(
		    tmp_group_dlevel_addr,
		    tmp_group_dlevel_data); /* 回写driver level寄存器 */
