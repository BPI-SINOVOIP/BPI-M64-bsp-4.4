...
int sunxi_smc_config(uint dram_size, uint secure_region_size)
{
	...
	{
		/* 设置安全区域起始地址*/
		writel((region_start * 1024 * 1024) & 0xffff8000,
		       SMC_REGIN_SETUP_LOW_REG(2));
	} else {
		...
		writel(long_regin_start & 0xffff8000,
		       SMC_REGIN_SETUP_LOW_REG(2));
	}
	/* 设置安全区域访问属性 */
	...
	/* set secure os area */
	region_size = (__tzasc_calc_2_power(16*1024/32) + 0b001110)<<1;
	permission  = 0b1100<<28;	/* 设置只允许安全模式访问 */
