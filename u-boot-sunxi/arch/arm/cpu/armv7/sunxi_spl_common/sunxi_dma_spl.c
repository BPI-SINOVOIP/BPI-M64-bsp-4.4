...
void sunxi_dma_init(void)
{
	...
	memset((void *)dma_channal_source, 0,
	       SUNXI_DMA_MAX * sizeof(struct sunxi_dma_source_t));

	for (i = 0; i < SUNXI_DMA_MAX; i++) {
		dma_channal_source[i].used = 0;
		dma_channal_source[i].channal =
		    (struct sunxi_dma_channal_set_t *)(ulong)(
			SUNXI_DMA_BASE + i * SUNXI_DMA_CHANANL_SIZE + 0x100);
		dma_channal_source[i].config =
		    (sunxi_dma_start_t *)malloc_noncache(
			sizeof(sunxi_dma_start_t));
	}
...
{
	struct sunxi_dma_source_t *dma_channal =
...
int sunxi_dma_install_int(ulong hdma, interrupt_handler_t dma_int_func, void *p)
{
	...
	if (!dma_channal->dma_func.m_func) {
		...
		printf("dma 0x%lx int is used already, you have to free it "
		       "first\n",
		       hdma);
...
int sunxi_dma_enable_int(ulong hdma)
{
	...
	channal_count = dma_channal->channal_count;
	if (channal_count < 8) {
		if (dma_status->irq_en0 &
		    (DMA_PKG_END_INT << channal_count * 4)) {
			printf("dma 0x%lx int is avaible already\n", hdma);
			return 0;
		}
		dma_status->irq_en0 |= (DMA_PKG_END_INT << channal_count * 4);
	} else {
		if (dma_status->irq_en1 &
		    (DMA_PKG_END_INT << (channal_count - 8) * 4)) {
			printf("dma 0x%lx int is avaible already\n", hdma);
			return 0;
		}
		dma_status->irq_en1 |=
