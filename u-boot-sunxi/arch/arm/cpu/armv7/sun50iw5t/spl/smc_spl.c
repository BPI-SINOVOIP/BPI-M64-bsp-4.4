...
int sunxi_smc_config(uint dram_size, uint secure_region_size)
{
	...
	/* 0x1f: 4G area */
	/* 0x1 : enable */
	writel((0xf << 28) | (0x1f << 1) | (0x1 << 0),
	       SMC_REGIN_ATTRIBUTE_REG(1));
	...
	/* OPTEE */
	/* 0xc:010100b 0x17:2M area(0x48600000 ~ 0x48800000)
	 * config secure dram to secure rw, non-secure can't access
	 */
	writel(0x08600000, SMC_REGIN_SETUP_LOW_REG(2));
	writel((0xc << 28) | (0x14 << 1) | (0x1 << 0),
	       SMC_REGIN_ATTRIBUTE_REG(2));

	/* 0xc:010111b 0x17:8M area(0x48800000 ~ 0x49000000)
	 * config secure dram to secure rw, non-secure can't access
	 */
	writel(0x08800000, SMC_REGIN_SETUP_LOW_REG(3));
	writel((0xc << 28) | (0x16 << 1) | (0x1 << 0),
