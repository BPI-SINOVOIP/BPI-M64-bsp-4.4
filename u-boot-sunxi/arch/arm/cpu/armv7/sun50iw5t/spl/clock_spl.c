...
static void cfg_pll_24M(void)
{
	...
	if (osc_type == HOSC_38M) {
		reg_value = (readl(CCMU_PLL_24M_CTRL_REG));
		reg_value &=
		    (~((0x1f << 16) | (0xff << 8) | (0x1 << 1) | (0x1 << 0)));
		reg_value |=
		    (((0x13 << 16) | (0x31 << 8) | (0x1 << 1) | (0x1 << 0)));
		writel(reg_value, CCMU_PLL_24M_CTRL_REG);
...
static u32 prode_pll_init(void)
{
	...
	if (val_1 < 0) {
		/*select CPUX  clock src: OSC24M,AXI divide ratio is 2, system
		 * apb clk ratio is 4*/
...
static void enable_cpu_pll(u32 reg_addr, u32 pll_enable, u32 lock_enable,
			   u32 lock_flag)
{
	__u32 reg_val;
