...
* 68h    Arbitration lost in address as master, slave address + Write bit
*received, ACK transmitted
* 70h    General Call address received, ACK transmitted
* 78h    Arbitration lost in address as master, General Call address received,
*ACK transmitted
...
* B0h    Arbitration lost in address as master, slave address + Read bit
*received, ACK transmitted
	...
	if (data_count == 1) {
		...
		*data_addr = i2c->data;
		...
		for (i = 0; i < data_count - 1; i++) {
			time = 0xffff;
			/* host should send ack every time when a data packet
			 * finished */
...
static inline int axp_i2c_write(unsigned char chip, unsigned char addr,
				unsigned char data)
{
	return i2c_write(chip, addr, 1, &data, 1);
}

static inline int axp_i2c_read(unsigned char chip, unsigned char addr,
...
static int axp_probe(void)
{
	...
	if (axp_i2c_read(CONFIG_SYS_I2C_SLAVE, BOOT_POWER806_VERSION,
			 &pmu_type)) {
...
static int set_dcdcd_voltage(int vol)
{
	...
	ret = axp_i2c_write(CONFIG_SYS_I2C_SLAVE, BOOT_POWER806_DCDOUT_VOL,
			    vol_set);
