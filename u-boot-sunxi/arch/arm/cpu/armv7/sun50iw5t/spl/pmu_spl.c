...
axp_contrl_info axp_ctrl_tbl[] = {
    /*   name        min,     max,    reg,    mask,  step0,   split1_val,
       step1,   ctrl_reg,           ctrl_bit */
...
axp_contrl_info axp_ctrl_tbl[] = {
    ...
    {"dcdc2", 500, 1300, 0x21, 0x7f, 10, 1200, 20, BOOT_POWER81X_OUTPUT_CTL1,
     1},
    {"dcdc5", 800, 1840, 0x24, 0x7f, 10, 1120, 20, BOOT_POWER81X_OUTPUT_CTL1,
     4},
...
{
	u8 reg_val = 0;
	if (pmu_bus_read(RSB_RUNTIME_ADDR, BOOT_POWER81X_HOTOVER_CTL,
			 &reg_val)) {
		return -1;
	}
	/* PWROK drive low restart function enable  */
	/* for watchdog reset */
	reg_val |= 1;
	if (pmu_bus_write(RSB_RUNTIME_ADDR, BOOT_POWER81X_HOTOVER_CTL,
			  reg_val)) {
		return -1;
	}
	return 0;
...
int probe_power_key(void)
{
	...
	if (reg_value) {
		if (pmu_bus_write(RSB_RUNTIME_ADDR, PMU_POWER_KEY_STATUS,
				  reg_value)) {
			return -1;
		}
	}
