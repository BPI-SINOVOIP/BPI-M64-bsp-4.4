...
static unsigned long mmc_write_blocks(struct mmc *mmc, unsigned long start,
				      unsigned blkcnt, const void *src)
{
	struct mmc_cmd cmd;
...
unsigned long mmc_bwrite(int dev_num, unsigned long start, unsigned blkcnt,
			 const void *src)
{
...
retry_scr:
	...
	if (err) {
		mmcinfo("update clock failed after send cmd6 to switch to sd "
			"high speed mode\n");
		return err;
	}
...
int mmc_init(struct mmc *mmc)
{
	...
	if (err) {
		...
		if (err) {
			mmcinfo(
			    "mmc %d Card did not respond to voltage select!\n",
			    mmc->control_num);
	...
	if (priv_info->card_type == CARD_TYPE_SD) {
		...
		if (err) {
			mmcinfo("SD card %u Card did not respond to voltage "
				"select!\n",
				mmc->control_num);
		...
		if (err) {
			mmcinfo("MMC card %u Card did not respond to voltage "
				"select!\n",
				mmc->control_num);
			mmcinfo("***SD/MMC %u init error!!!***\n",
				mmc->control_num);
		...
		if (err) {
			...
			if (err) {
				mmcinfo("mmc %u Card did not respond to "
					"voltage select!\n",
					mmc->control_num);
				mmcinfo("***SD/MMC %u init error!!!***\n",
					mmc->control_num);
