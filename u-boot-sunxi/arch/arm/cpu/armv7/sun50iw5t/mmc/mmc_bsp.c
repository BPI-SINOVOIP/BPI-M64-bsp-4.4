...
void set_mmc_para(int smc_no, void *addr)
{
	struct spare_boot_head_t *uboot_buf =
	    (struct spare_boot_head_t *)CONFIG_SYS_TEXT_BASE;
	u32 *p = NULL;
	int i = 0;
...
static int mmc_update_clk(struct mmc *mmc)
{
	...
	writel(readl(&mmchost->reg->clkcr) & (~(0x1 << 31)),
	       &mmchost->reg->clkcr);
...
f5: CLK_200M
*/
/* some information is passed in boot0's extend data buffer:
   BT0_head.prvt_head.storage_data
    these information is write to BT0_head.prvt_head.storage_data before write
   boot0 during product.
				  ...
				  u8 *odly, u8 *sdly)
{
	...
	struct boot_sdmmc_private_info_t *priv_info =
	    (struct boot_sdmmc_private_info_t *)(mmc_config_addr +
						 SDMMC_PRIV_INFO_ADDR_OFFSET);
	...
	if (boot0_para & BOOT0_PARA_USE_INTERNAL_DEFAULT_TIMING_PARA) {
		...
		if (boot0_para & BOOT0_PARA_USE_EXTERNAL_INPUT_TIMING_PARA) {
			...
			if ((sdc_no != 2) || (spd_md_id > 4) || (freq_id > 5)) {
				mmcinfo("wrong input para: mmc %d/2, mode "
					"%d/[0,4], freq %d/[0,5]\n",
			...
			spd_md_sdly =
			    tune_sdly->tm4_smx_fx[spd_md_id * 2 + freq_id / 4];
...
static int mmc_set_mclk(struct sunxi_mmc_host *mmchost, u32 clk_hz)
{
	...
	if (clk_hz <= 4000000) { /* 400000 */
		...
		src = 1;
		sclk_hz =
		    _get_pll_periph0() * 2 * 1000000; /*use 2x pll-per0 clock */
	}
				 ...
				 u32 freq_id)
{
	...
	if (mode == SUNXI_MMC_TIMING_MODE_1) {
		...
		/* mode 3 is optional on sdc0. we don't support config delay
		 * here. */
		odly = 0;
		sdly = 0;
		...
		if (spd_md == HS400)
			spd_md = HS200_SDR104; /* use HS200's sdly for HS400's
						  CMD line */
		...
		mmcdbg(
		    "%s: spd_md:%u, freq:%u, odly: %d; sdly: %d; dsdly: %d\n",
...
static int mmc_trans_data_by_dma(struct mmc *mmc, struct mmc_data *data)
{
	...
	/* OSAL_CacheRangeFlush(buff, (unsigned long)byte_cnt,
	 * CACHE_CLEAN_FLUSH_D_CACHE_REGION); */
	for (i = 0; i < buff_frag_num; i++, des_idx++) {
		...
		pdes[des_idx].buf_addr_ptr1 =
		    ((u32)buff + i * SDXC_DES_BUFFER_MAX_LEN) >> 2;
		if (i == 0)
	...
	/* OSAL_CacheRangeFlush(pdes, sizeof(struct sunxi_mmc_des) *
	 * (des_idx+1), CACHE_CLEAN_FLUSH_D_CACHE_REGION); */
	__asm("DSB");
	__asm("ISB");
	...
	writel(((u32)pdes) >> 2, &mmchost->reg->dlba);
	writel(
	    (2U << 28) | (7 << 16) | 248,
	    &mmchost->reg->ftrglevel); /* burst-8, rx/tx trigger level=7/248 */
...
out:
	if (data && usedma) {
		...
		writel(readl(&mmchost->reg->gctrl) & (~(1 << 5)),
		       &mmchost->reg->gctrl);
		/* OSAL_CacheRangeFlush(data->b.dest, data->blocksize *
		 * data->blocks, 4); */
	}
	if (error) {
