...
struct __ScanDieInfo_t {
	...
	__u8 TblBitmap;   /* the bitmap that mark the block mapping table status
			     in a die */
	...
	__u16 *pPhyBlk; /* the pointer to the physical block information buffer
			   of a whole die */
	struct __ScanZoneInfo_t *ZoneInfo; /* the pointer to the zone table
					      detail information of the whole
					      die */
};
...
/* define the sector bitmap in a super page to get the logical information in
 * the spare area */
...
#define DATA_TBL_OFFSET                                                        \
	0 /* page0 and page1 of a table group are used for data & free block   \
	     table */
#define LOG_TBL_OFFSET                                                         \
	2 /* page2 of a table group is used for log block table */
...
/* define the macro for get the zone number in the logical information of
 * physical block */
#define GET_LOGIC_INFO_ZONE(a) ((((unsigned int)(a)) >> 10) & 0x0f)
/* define the macro for get the block used type in the logical information of
 * physical block */
#define GET_LOGIC_INFO_TYPE(a) ((((unsigned int)(a)) >> 14) & 0x01)
/* define the macro for get the logical block number in the logical information
 * of physical block */
...
*
*Note       : This function look for the mapping information on the nand flash
*first, if the find all
*             mapping information and check successful, format nand disk
*successful; if the mapping
*             information has some error, need repair it. If find none mapping
*information, create it!
************************************************************************************************************************
*/
