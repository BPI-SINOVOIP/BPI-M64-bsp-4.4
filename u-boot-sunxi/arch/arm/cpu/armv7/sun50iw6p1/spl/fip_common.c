...
/* int len; */
	...
	for (i = 0; i < toc1_head->items_nr; i++, toc1_item++) {
		...
		if (strncmp(toc1_item->name, ITEM_UBOOT_NAME,
			...
			*use_monitor = 1;
			...
			toc1_flash_read(toc1_item->data_offset / 512,
					...
					(void *)SCP_SRAM_BASE);
			toc1_flash_read(
			    (toc1_item->data_offset + 0x18000) / 512,
...
#endif
		else if (strncmp(toc1_item->name, ITEM_PARAMETER_NAME,
				 sizeof(ITEM_PARAMETER_NAME)) == 0) {
			toc1_flash_read(toc1_item->data_offset / 512,
					(toc1_item->data_len + 511) / 512,
					(void *)(CONFIG_SUNXI_PARAMETER_ADDR));
		} else if (strncmp(toc1_item->name, ITEM_LOGO_NAME,
				   sizeof(ITEM_LOGO_NAME)) == 0) {
			*(uint *)(SUNXI_LOGO_COMPRESSED_LOGO_SIZE_ADDR) =
			    toc1_item->data_len;
				toc1_flash_read(
			    ...
			    (void *)SUNXI_LOGO_COMPRESSED_LOGO_BUFF);
			       ...
			       toc1_item->name, ITEM_SHUTDOWNCHARGE_LOGO_NAME,
			       sizeof(ITEM_SHUTDOWNCHARGE_LOGO_NAME)) == 0) {
			*(uint
			      *)(SUNXI_SHUTDOWN_CHARGE_COMPRESSED_LOGO_SIZE_ADDR) =
			    toc1_item->data_len;
			toc1_flash_read(
			    toc1_item->data_offset / 512,
			    (toc1_item->data_len + 511) / 512,
			...
			*(uint
			      *)(SUNXI_ANDROID_CHARGE_COMPRESSED_LOGO_SIZE_ADDR) =
			    toc1_item->data_len;
			toc1_flash_read(
			    toc1_item->data_offset / 512,
			    (toc1_item->data_len + 511) / 512,
		...
		} else if (strncmp(toc1_item->name, ITEM_ESM_IMG_NAME,
				   sizeof(ITEM_ESM_IMG_NAME)) == 0) {
		...
		else if (strncmp(toc1_item->name, ITEM_BDCFG_NAME,
				 sizeof(ITEM_BDCFG_NAME)) == 0) {
			toc1_flash_read(
			    toc1_item->data_offset / 512,
			    (toc1_item->data_len + 511) / 512,
			    (void *)CONFIG_BDCFG_STORE_IN_DRAM_BASE);
		}
#endif
	}
	if (*use_monitor) {
