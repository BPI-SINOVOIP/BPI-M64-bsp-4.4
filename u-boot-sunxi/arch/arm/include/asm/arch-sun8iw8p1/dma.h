...
typedef struct
{
    unsigned int      src_drq_type     : 5; //src storage eg:DRAM, SPI,NAND see __ndma_drq_type_t
    unsigned int      src_addr_mode    : 2; //type of src addr 0:inc  1:keep
    unsigned int      src_burst_length : 2; //src burst length 0:1   1:4   2:8
    unsigned int      src_data_width   : 2; //src data width 0:8bit 1:16bit 2:32bit 3:reserve
    unsigned int      reserved0        : 5;
    unsigned int      dst_drq_type     : 5; //dst storage eg:DRAM, SPI,NAND
    unsigned int      dst_addr_mode    : 2; //type of dst addr incre or keep  0:inc  1:keep
    unsigned int      dst_burst_length : 2; //burst length 0:1 1:4,
    unsigned int      dst_data_width   : 2; //data width 0:8bit 1:16bit 2:32bit 3:reserve
