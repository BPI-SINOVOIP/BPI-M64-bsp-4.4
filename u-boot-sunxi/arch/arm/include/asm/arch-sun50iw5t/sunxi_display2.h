...
typedef enum {
	...
	DISP_FORMAT_YUV444_P =
	    0x46, /* MSB  P3-2-1-0 LSB,  YYYY UUUU VVVV, reserved */
...
typedef enum {
	...
	DISP_SCAN_INTERLACED_ODD_FLD_FIRST =
	    1 << 0, /* interlace ,odd field first */
	DISP_SCAN_INTERLACED_EVEN_FLD_FIRST =
	    1 << 1, /* interlace,even field first */
} disp_scan_flags;
...
typedef struct {
	unsigned long long
	    addr[3]; /* address of frame buffer,single addr for interleaved
			fomart,double addr for semi-planar fomart triple addr
			for planar format */
	disp_rectsz size[3]; /* size for 3 component,unit: pixels */
	unsigned int
	    align[3]; /* align for 3 comonent,unit: bits(align=2^n,i.e.
			 1/2/4/8/16/32..) */
	disp_pixel_format format;
	disp_color_space color_space;   /* color space */
	unsigned int trd_right_addr[3]; /* right address of 3d fb,used when in
...
typedef struct {
	...
	unsigned char zorder;      /*specifies the front-to-back ordering of the
				      layers on the screen,
									     the top
				      layer having the highest Z value
									     can't
				      set zorder, but can get */
	...
	unsigned int
	    id; /* frame id, can get the id of frame display currently
...
typedef struct {
	disp_rect
	    window; /*  capture window, rectangle of screen to be captured */
...
typedef enum {
	...
	FB_MODE_SCREEN1 = 1,
	FB_MODE_SCREEN2 = 2,
	FB_MODE_DUAL_SAME_SCREEN_TB = 3, /* two screen, top buffer for screen0,
					    bottom buffer for screen1 */
	FB_MODE_DUAL_DIFF_SCREEN_SAME_CONTENTS =
	    4, /* two screen, they have same contents; */
} disp_fb_mode;
...
typedef enum {
	...
	DISP_INIT_MODE_TWO_DIFF_SCREEN =
	    3, /* fb0 for screen0 and fb1 for screen1 */
	DISP_INIT_MODE_TWO_SAME_SCREEN =
	    4, /* fb0(up buffer for screen0, down buffer for screen1) */
	DISP_INIT_MODE_TWO_DIFF_SCREEN_SAME_CONTENTS =
	    5, /* fb0 for two different screen(screen0 layer is normal layer,
...
typedef struct {
	...
	int (*tv_get_video_timing_info)(unsigned int sel,
