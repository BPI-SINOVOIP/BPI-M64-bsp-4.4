...
typedef struct {
	unsigned int
	    addr[3]; /*  frame buffer的内容地址，对于rgb类型，只有addr[0]有效 */
	...
	disp_3d_src_mode
	    trd_mode; /* source 3d mode, used for scaler mode layer */
	unsigned int trd_right_addr[3]; /* used when in frame packing 3d mode */
	bool pre_multiply;		/* TRUE: pre-multiply fb */
	disp_window src_win; /*  framebuffer source window,only take x,y if is
				not scaler mode */
#if 0
	bool                  interlace;
...
typedef struct {
	...
	unsigned char
	    zorder; /* layer priority,can get layer prio,but never set layer
		       prio,从底至顶,优先级由低至高 */
	unsigned char alpha_mode;  /* 0: pixel alpha;  1: global alpha;  2:
				      global pixel alpha */
	...
	disp_3d_out_mode
	    out_trd_mode; /* output 3d mode, used for scaler mode layer */

	unsigned int id; /* get the id of the frame display now by
			    DISP_CMD_LAYER_GET_FRAME_ID */
} disp_layer_info;
...
typedef struct {
	...
	unsigned int
	    red_match_rule; /* 0/1:always match; 2:match if min<=color<=max;
			       3:match if color>max or color<min */
	unsigned int
	    green_match_rule; /* 0/1:always match; 2:match if min<=color<=max;
				 3:match if color>max or color<min */
	unsigned int blue_match_rule; /* 0/1:always match; 2:match if
					 min<=color<=max; 3:match if color>max
					 or color<min */
} disp_colorkey;

typedef struct {
	disp_size screen_size; /* used when the screen is not displaying on any
				  output device(lcd/hdmi/vga/tv) */
...
typedef struct {
	...
	disp_lcd_lvds_colordepth
	    lcd_lvds_colordepth; /* color depth, 0:8bit; 1:6bit */
...
struct sunxi_disp_source_ops {
	...
	int (*sunxi_lcd_cpu_write_data)(unsigned int scree_id,
					unsigned int data);
	int (*sunxi_lcd_dsi_write)(unsigned int scree_id, unsigned char command,
				   unsigned char *para, unsigned int para_num);
...
typedef enum {
	FB_MODE_SCREEN0 = 0,
	FB_MODE_SCREEN1 = 1,
	FB_MODE_DUAL_SAME_SCREEN_TB = 2, /* two screen, top buffer for screen0,
					    bottom buffer for screen1 */
	FB_MODE_DUAL_DIFF_SCREEN_SAME_CONTENTS =
	    3, /* two screen, they have same contents; */
} disp_fb_mode;
...
typedef struct {
	...
	unsigned int
	    primary_screen_id; /* used when
				  FB_MODE_DUAL_DIFF_SCREEN_SAME_CONTENTS */
	unsigned int
	    aux_output_width; /* used when
				 FB_MODE_DUAL_DIFF_SCREEN_SAME_CONTENTS */
	unsigned int
	    aux_output_height; /* used when
				  FB_MODE_DUAL_DIFF_SCREEN_SAME_CONTENTS */
} disp_fb_create_info;
...
typedef enum {
	...
	DISP_INIT_MODE_TWO_DIFF_SCREEN =
	    2, /* fb0 for screen0 and fb1 for screen1 */
	DISP_INIT_MODE_TWO_SAME_SCREEN =
	    3, /* fb0(up buffer for screen0, down buffer for screen1) */
	DISP_INIT_MODE_TWO_DIFF_SCREEN_SAME_CONTENTS =
	    4, /* fb0 for two different screen(screen0 layer is normal layer,
		  screen1 layer is scaler layer); */
} disp_init_mode;

typedef struct {
	bool b_init;
	disp_init_mode disp_mode; /* 0:single screen0(fb0); 1:single
				     screen1(fb0);  2:dual diff screen(fb0,
				     fb1); 3:dual same screen(fb0 up and down);
