...
#define MBR_RESERVED                                                           \
	(MBR_SIZE - 20 -                                                       \
	 (MBR_MAX_PART_COUNT * sizeof(PARTITION))) /* mbr保留的空间 */
...
typedef struct tag_one_part_info {
	...
	unsigned char
	    part_name[12]; /* 所烧写分区的名称，和MBR中的分区 classname 对应 */
	unsigned char
	    dl_filename[16]; /* 所烧写分区的文件名称，长度固定16字节 */
	unsigned char
	    vf_filename[16]; /* 所烧写分区的校验文件名称，长度固定16字节 */
	unsigned int
	    encrypt; /* 所烧写分区的数据是否进行加密 0:加密   1：不加密 */
} dl_one_part_info;
/* 分区烧写信息 */
