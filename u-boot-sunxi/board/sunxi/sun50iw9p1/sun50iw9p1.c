...
int board_init(void)
{
    ...
    if (uboot_spare_head.boot_data.work_mode != WORK_MODE_USB_PRODUCT) {
	    ...
	    reg_val = readl(SUNXI_SYSCRL_BASE);
	    reg_val &= ~(0x7FFFFFFF);
	    writel(reg_val, SUNXI_SYSCRL_BASE);
	    ...
	    reg_val = readl(CCMU_DE_BGR_REG);
	    reg_val &= ~(0x1 << 0);
	    reg_val &= ~(0x1 << 16);
	    writel(reg_val, CCMU_DE_BGR_REG);
...
{
    static char *hardware_info  = "sun50iw9p1";
...
static void sunxi_random_ether_addr(void)
{
    ...
    sprintf(mac, "%02x:%02x:%02x:%02x:%02x:%02x", tmp_mac, usb_net_addr[1],
	    usb_net_addr[2], usb_net_addr[3], usb_net_addr[4], usb_net_addr[5]);
