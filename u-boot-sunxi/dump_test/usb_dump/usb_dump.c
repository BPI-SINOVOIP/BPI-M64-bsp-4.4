...
#endif
static  volatile int sunxi_usb_efex_send_ready = 1;
...
static int sunxi_efex_init(void)
{
    ...
    trans_data.base_recv_buffer = (u8 *)malloc(1024 * 1024);
	...
	trans_data.base_send_buffer = (u8 *)malloc(1024 * 1024);
...
static int __sunxi_usb_efex_op_cmd(u8 *cmd_buffer)
{
	...
	switch(cmd->app_cmd)
	{
		...
		case FEX_CMD_fes_query_dram_size:
			sunxi_usb_dbg("FEX_CMD_fes_query_dram_size\n");
			
			{
				uint *dram_size = (uint *)trans_data.base_send_buffer;
				__dram_para_t  *dram_para = (__dram_para_t *)DRAM_PARA_STORE_ADDR;
				
				*dram_size = (dram_para->dram_para2 >> 16) & 0xffff;
				*dram_size *= 1024 * 1024;
	...
	switch(sunxi_usb_efex_status)
	{
	  	...
	  	case SUNXI_USB_EFEX_RECEIVE_DATA:
            ...
            case SUNXI_USB_EFEX_SETUP_NEW:      //
            {
                ...
                else if(trans_data.app_next_status == SUNXI_USB_EFEX_APPS_RECEIVE_DATA)
                {
                    ...
                    if(sunxi_ubuf->request_size)
                    {
                        sunxi_usb_dbg("dma recv addr = 0x%x, size =0x%x\n", (uint)trans_data.act_recv_buffer,sunxi_ubuf->request_size);
                        sunxi_udc_start_recv_by_dma(trans_data.act_recv_buffer, sunxi_ubuf->request_size);  //start dma to receive data
                    }
                    else
                    {
            ...
            case SUNXI_USB_EFEX_SEND_DATA_NEW:
                sunxi_usb_dbg("SUNXI_USB_EFEX_SEND_DATA_NEW\n");
                {
                    ...
                    {
                        //sunxi_udc_send_data((void *)trans_data.act_send_buffer, tx_length);
                        sunxi_usb_efex_send_ready = 0;       
                        sunxi_udc_start_send_by_dma((void *)trans_data.act_send_buffer, tx_length);                                                                 
                    }
                    sunxi_usb_efex_status = SUNXI_USB_EFEX_STATUS;
                }
                break;
		...
		case SUNXI_USB_EFEX_STATUS:
...
#endif
			{
                ...
                sunxi_usb_dbg("sunxi_usb_efex_send_ready=%d\n", sunxi_usb_efex_send_ready);
                if (!sunxi_usb_efex_send_ready) {
                	while( !sunxi_usb_efex_send_ready);
                	sunxi_udc_wait_dma_send_finish();                	
                }
