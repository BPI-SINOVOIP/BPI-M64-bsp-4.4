...
static void __usb_send_by_dma_isr(void *p_arg)
{
	...
	old_ep_idx = USBC_GetActiveEp(sunxi_udc_source.usbc_hd);
	USBC_SelectActiveEp(sunxi_udc_source.usbc_hd, SUNXI_USB_BULK_IN_EP_INDEX);			//选择TXEP

	//选择使用IO方式搬运数据
	sunxi_usb_dbg("select io mode to transfer data\n");
	USBC_Dev_ClearEpDma(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX);

	//如果当前dma传输的不是完整包，则需要手动清除中断
	if(sunxi_ubuf.request_size % sunxi_udc_source.bulk_ep_max)
	{
		USBC_Dev_WriteDataStatus(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX, 1);
...
************************************************************************************************************
*/
int sunxi_udc_start_send_by_dma(void* mem_base, uint length)
{
	uint old_ep_idx;
	ulong mem_buf = (ulong)mem_base;
	usb_dma_trans_unaliged_bytes = length & (sizeof(int) - 1);
	...
	old_ep_idx = USBC_GetActiveEp(sunxi_udc_source.usbc_hd);
	USBC_SelectActiveEp(sunxi_udc_source.usbc_hd, SUNXI_USB_BULK_IN_EP_INDEX);			//选择当前EP
	//usb控制器选择dma传输方式
	USBC_Dev_ConfigEpDma(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX);
	...
	sunxi_ubuf.request_size = length;
	sunxi_usb_dbg("dma start 0x%x, length 0x%x\n", (uint)mem_buf, length);
	usb_dma_start(sunxi_udc_source.dma_send_channal, mem_buf, length);
	//恢复EP
	USBC_SelectActiveEp(sunxi_udc_source.usbc_hd, old_ep_idx);			//恢复原有EP
...
void sunxi_udc_wait_dma_send_finish(void)
{
	int ret = 1;
	u32 old_ep_idx = 0;
	...
	while(ret) {
		ret = USBC_Dev_IsWriteDataReady(sunxi_udc_source.usbc_hd, USBC_EP_TYPE_TX);
		//printf("ret=0x%x\n", readl(0x1c19080));
	}
	USBC_SelectActiveEp(sunxi_udc_source.usbc_hd, old_ep_idx);
