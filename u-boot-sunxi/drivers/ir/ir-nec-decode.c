...
int ir_nec_decode(struct ir_raw_buffer *ir_raw, struct ir_raw_event ev)
{
	...
	*/
	...
	print_debug("NEC decode started at state %d (%uus %s)\n", data->state,
	...
	case STATE_HEADER_SPACE:
		...
		if (eq_margin(ev.duration, NEC_HEADER_SPACE, NEC_UNIT)) {
			...
			return 0;
				     ...
				     NEC_UNIT / 2)) {
			/* if (!dev->keypressed) { */
			/*	printf("Discarding last key repeat: event after */
			/*key up\n"); */
			/*} else { */
			/*	rc_repeat(dev); */
			printf("Repeat last key\n");
	...
	case STATE_TRAILER_SPACE:
		...
		if ((command ^ not_command) != 0xff) {
			printf("NEC checksum error: received 0x%08x\n",
			       data->bits);
			send_32bits = true;
		}
		...
		if (send_32bits) {
			...
			scancode = data->bits;
			print_debug("NEC (modified) scancode 0x%08x\n",
				    scancode);
		} else {
			/* Extended NEC */
			scancode = address << 8 | not_address << 16 | command;
			print_debug("NEC scancode 0x%06x\n", scancode);
		}

		if (data->is_nec_x)
			data->necx_repeat = true;

		printf("scancode key down 0x%06x\n", scancode);
		rc_keydown(ir_raw, scancode, 0);
		data->state = STATE_INACTIVE;
		return 0;
	}

	print_debug("NEC decode failed at count %u state %d (%uus %s)\n",
