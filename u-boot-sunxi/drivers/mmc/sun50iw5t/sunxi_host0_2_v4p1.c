...
#define SDXC_IntErrBit                                                         \
	(SDXC_RespErr | SDXC_RespCRCErr | SDXC_DataCRCErr | SDXC_RespTimeout | \
	 SDXC_DataTimeout | SDXC_FIFORunErr | SDXC_HardWLocked |               \
	 SDXC_StartBitErr | SDXC_EndBitErr) /* 0xbfc2 */
...
static int mmc_set_mclk(struct sunxi_mmc_host *mmchost, u32 clk_hz)
{
	...
	if (clk_hz <= 4000000) { /* 400000 */
		...
		src = 1;
		sclk_hz = sunxi_clock_get_pll6() * 2 *
...
static unsigned mmc_get_mclk(struct sunxi_mmc_host *mmchost)
{
	...
	if (src == 0)
		...
		sclk_hz =
		    sunxi_clock_get_pll6() * 2 * 1000000; /* use 2x pll6 */
	else if (src == 2) {
		/*todo*/
	} else {
...
static unsigned mmc_config_delay(struct sunxi_mmc_host *mmchost)
{
	...
	if (mode == SUNXI_MMC_TIMING_MODE_1) {
		...
		if (mmchost->tm1.odly[spd_md * MAX_CLK_FREQ_NUM + freq] != 0xFF)
			odly =
			    mmchost->tm1.odly[spd_md * MAX_CLK_FREQ_NUM + freq];
		else
			odly = mmchost->tm1
				   .def_odly[spd_md * MAX_CLK_FREQ_NUM + freq];
		if (mmchost->tm1.sdly[spd_md * MAX_CLK_FREQ_NUM + freq] != 0xFF)
			sdly =
			    mmchost->tm1.sdly[spd_md * MAX_CLK_FREQ_NUM + freq];
		else
			sdly = mmchost->tm1
				   .def_sdly[spd_md * MAX_CLK_FREQ_NUM + freq];
		...
		if (mmchost->tm3.odly[spd_md * MAX_CLK_FREQ_NUM + freq] != 0xFF)
			odly =
			    mmchost->tm3.odly[spd_md * MAX_CLK_FREQ_NUM + freq];
		else
			odly = mmchost->tm3
				   .def_odly[spd_md * MAX_CLK_FREQ_NUM + freq];
		if (mmchost->tm3.sdly[spd_md * MAX_CLK_FREQ_NUM + freq] != 0xFF)
			sdly =
			    mmchost->tm3.sdly[spd_md * MAX_CLK_FREQ_NUM + freq];
		else
			sdly = mmchost->tm3
				   .def_sdly[spd_md * MAX_CLK_FREQ_NUM + freq];
		...
		MMCDBG(
		    "%s: spd_md:%d, freq:%d, odly: %d; sdly: %d; dsdly: %d\n",
...
static int mmc_calibrate_delay_unit(struct sunxi_mmc_host *mmchost)
{
	...
	struct mmc_reg_v4p1 *reg = (struct mmc_reg_v4p1 *)mmchost->reg;
	unsigned rval = 0;
	unsigned clk[4] = {50 * 1000 * 1000, 100 * 1000 * 1000,
...
#define SMC_RESP_TIMEOUT 0x2
	...
	writel((SMC_DATA_TIMEOUT << 8) | SMC_RESP_TIMEOUT,
	       &reg->timeout); /* Set Data & Response Timeout Value */
...
static void mmc_set_ios(struct mmc *mmc)
{
	...
	/* change clock */
	if (mmc->clock &&
	    mmc_config_clock((struct sunxi_mmc_host *)mmc->priv, mmc->clock)) {
		MMCINFO("[mmc]: mmc %d update clock failed\n", mmchost->mmc_no);
		mmchost->fatal_err = 1;
		return;
	}

/* for card2, if clock frequency is greater than 100MHz, increase gpio dirver
 * strength */
#if 0
...
static int mmc_trans_data_by_dma(struct mmc *mmc, struct mmc_data *data)
{
	...
	for (i = 0; i < buff_frag_num; i++, des_idx++) {
		...
		pdes[des_idx].buf_addr_ptr1 =
		    ((ulong)buff + i * SDXC_DES_BUFFER_MAX_LEN) >> 2;
		if (i == 0)
	...
	flush_cache((unsigned long)pdes,
		    sizeof(struct mmc_des_v4p1) * (des_idx + 1));
	__asm("DSB");
	__asm("ISB");
	...
	writel(((unsigned long)pdes) >> 2, &reg->dlba);
	if (mmchost->mmc_no == 2)
		writel(
		    (3U << 28) | (15U << 16) | 240,
		    &reg->ftrglevel); /* burst-16, rx/tx trigger level=15/240 */
	else
		writel(
		    (2U << 28) | (7U << 16) | 248,
		    &reg->ftrglevel); /* burst-8, rx/tx trigger level=7/248 */
			...
			struct mmc_data *data)
{
	...
	if ((cmd->cmdidx == 12) && !(cmd->flags & MMC_CMD_MANUAL)) {
		MMCDBG("usually, cmd12 is sent after cmd18/cmd25 "
		       "automantically.\n");
		/* don't wait write busy here, because no cmd12 will be sent for
		cmd24.
	...
	if (data) {
		...
		if ((cmd->cmdidx == 12) && (cmd->flags & MMC_CMD_MANUAL)) {
			cmdval |=
			    1
			    << 14; /* stop current data transferin progress. */
			cmdval &= ~(1 << 13); /* Send command at once, even if
						 previous data transfer has
						 notcompleted */
		}
	}
	...
	do {
		...
		if (!timeout-- || (status & 0xbfc2)) {
			...
			if (!error)
				error =
				    0xffffffff; /* represet software timeout */
	...
	if (data) {
		...
		timeout = usedma
			      ? (50 * bytecnt / 25)
			      : 0xffffff; /* 0.04us(25M)*2(4bit width)*25() */
		if (timeout < 0xffffff) {
			timeout = 0xffffff;
		}
		...
		do {
			...
			if (!timeout-- || (status & 0xbfc2)) {
				error = status & 0xbfc2;
				if (!error)
					error = 0xffffffff; /* represet software
							       timeout */
				MMCMSG(mmc, "mmc %d data timeout %x\n",
				       mmchost->mmc_no, error);
				goto out;
			}
			if ((timeout == 0xFF0000) && mmc->do_tuning &&
			    (data->flags & MMC_DATA_READ) /*(bytecnt==512)*/) {
				error =
				    0xffffffff; /* represet software timeout */
				MMCMSG(
				    mmc,
				    "mmc %d data timeout %x -----------err\n",
				    mmchost->mmc_no, error);
				goto out;
			}
			...
			if ((data->blocks > 1) &&
			    !(cmd->flags &
			      MMC_CMD_MANUAL)) /* not wait auto stop when
						  MMC_CMD_MANUAL is set */
			{
				if (usedma)
					done = ((status & (1 << 14)) &&
						(readl(&reg->idst) & 0x3))
	...
	if (cmd->resp_type & MMC_RSP_136) {
		...
		MMCDBG("mmc %d mmc resp 0x%08x 0x%08x 0x%08x 0x%08x\n",
		       mmchost->mmc_no, cmd->response[3], cmd->response[2],
...
out:
	...
	if (error) {
		...
		/* during tuning sample point, some sample point may cause
		timing problem.
		for example, if a RTO error occurs, host may stop clock and
		device may still output data.
		we need to read all data(512bytes) from device to avoid to
		update clock fail.
		*/
		if (mmc->do_tuning && data && (data->flags & MMC_DATA_READ) &&
		    (bytecnt == 512)) {
			writel(readl(&reg->gctrl) | 0x80000000, &reg->gctrl);
			writel(0xdeb, &reg->dbgc);
			timeout = 1000;
...
static int sunxi_decide_rty(struct mmc *mmc, int err_no, uint rst_cnt)
{
	...
	if (err_no &&
	    (!(err_no & SDXC_RespTimeout) || (err_no == 0xffffffff))) {
		mmchost->retry_cnt++;
		...
		if (tmode == SUNXI_MMC_TIMING_MODE_1) {
			...
			freq = mmchost->tm1.cur_freq;
			sdly = &mmchost->tm1
				    .sdly[spd_md * MAX_CLK_FREQ_NUM + freq];

			if (mmchost->retry_cnt * tm1_retry_gap <
			    MMC_CLK_SAMPLE_POINIT_MODE_1) {
				if ((*sdly + tm1_retry_gap) <
				    MMC_CLK_SAMPLE_POINIT_MODE_1) {
				...
				MMCINFO("Get next samply point %d at spd_md %d "
					"freq_id %d\n",
					*sdly, spd_md, freq);
			} else {
			...
			if (mmchost->retry_cnt * tm3_retry_gap <
			    MMC_CLK_SAMPLE_POINIT_MODE_3) {
				if ((*sdly + tm3_retry_gap) <
				    MMC_CLK_SAMPLE_POINIT_MODE_3) {
			...
			if (spd_md == HS400)
				...
				sdly = &mmchost->tm4
					    .sdly[spd_md * MAX_CLK_FREQ_NUM +
						  freq];
			MMCINFO("Current spd_md %d freq_id %d sldy %d\n",
				spd_md, freq, *sdly);

			if (mmchost->retry_cnt * tm4_retry_gap <
			    MMC_CLK_SAMPLE_POINIT_MODE_4) {
				if ((*sdly + tm4_retry_gap) <
				    MMC_CLK_SAMPLE_POINIT_MODE_4) {
