...
#undef readl
#define  readl(a)   (*(volatile unsigned int *)(ulong)(a))

#undef writel
#define  writel(v, c) (*(volatile unsigned int *)(ulong)(c) = (v))
		...
		*/
		if (mmc->do_tuning && data && (data->flags&MMC_DATA_READ) && (bytecnt == 512)) {
			...
			while (readl(&reg->bbcr) < 512) {
				...
				__usdelay(1);
				if (!(timeout--)) {
					MMCMSG(mmc, "Read remain data timeout tmp val %x\n", tmp);
					break;
				}
			}
		}
