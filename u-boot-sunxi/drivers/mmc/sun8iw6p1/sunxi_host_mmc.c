...
static int mmc_clear_timing_para(int sdc_no)
{
	...
	for (i=0; i<MAX_SPD_MD_NUM; i++)
	{
		...
		{
			mmchost->tm0.odly[i*MAX_CLK_FREQ_NUM+j] = 0xFF;
			mmchost->tm0.sdly[i*MAX_CLK_FREQ_NUM+j] = 0xFF;
			mmchost->tm0.def_odly[i*MAX_CLK_FREQ_NUM+j] = 0xFF;
			mmchost->tm0.def_sdly[i*MAX_CLK_FREQ_NUM+j] = 0xFF;
...
static int mmc_init_default_timing_para(int sdc_no)
{
	...
	if (sdc_no == 0)
	{
		...
		mmchost->tm1.def_odly[DS26_SDR12*MAX_CLK_FREQ_NUM+CLK_400K] = 0;
		mmchost->tm1.def_sdly[DS26_SDR12*MAX_CLK_FREQ_NUM+CLK_400K] = 0;
		mmchost->tm1.def_odly[DS26_SDR12*MAX_CLK_FREQ_NUM+CLK_25M] = 0;
		mmchost->tm1.def_sdly[DS26_SDR12*MAX_CLK_FREQ_NUM+CLK_25M] = 0;

		mmchost->tm1.def_odly[HSSDR52_SDR25*MAX_CLK_FREQ_NUM+CLK_25M] = 1;
		mmchost->tm1.def_sdly[HSSDR52_SDR25*MAX_CLK_FREQ_NUM+CLK_25M] = 1;
		mmchost->tm1.def_odly[HSSDR52_SDR25*MAX_CLK_FREQ_NUM+CLK_50M] = 1;
		mmchost->tm1.def_sdly[HSSDR52_SDR25*MAX_CLK_FREQ_NUM+CLK_50M] = 1;

		mmchost->tm1.def_odly[HSDDR52_DDR50*MAX_CLK_FREQ_NUM+CLK_25M] = 1;
		mmchost->tm1.def_sdly[HSDDR52_DDR50*MAX_CLK_FREQ_NUM+CLK_25M] = 1;
		mmchost->tm1.def_odly[HSDDR52_DDR50*MAX_CLK_FREQ_NUM+CLK_50M] = 1;
		mmchost->tm1.def_sdly[HSDDR52_DDR50*MAX_CLK_FREQ_NUM+CLK_50M] = 1;
...
static int mmc_get_dly_manual_sample(int sdc_no)
{
	...
	{
		if (mmchost->timing_mode == SUNXI_MMC_TIMING_MODE_0)
		{
			struct sunxi_mmc_timing_mode0 *p = &mmchost->tm0;
...
static int mmc_update_timing_para(int sdc_no)
{
	...
	{
		MMCINFO("use default timing para!\n");
...
int sunxi_mmc_init(int sdc_no)
{
	...
	if ((sdc_no == 0) || (sdc_no == 1))
		host->timing_mode = SUNXI_MMC_TIMING_MODE_0;
	else if ((sdc_no == 2))
		host->timing_mode = SUNXI_MMC_TIMING_MODE_1;
	...
	if (host->cfg.host_no == 2)
		host->cfg.host_caps |= MMC_MODE_DDR_52MHz | MMC_MODE_8BIT;
