...
int spinor_init(int stage)
{
	...
	if (spinor_flash_inited) {
		...
		printf("sunxi spinor is initing...");
		if (spic_init(0)) {
			printf("Fail\n");
	...
	if (uboot_spare_head.boot_data.work_mode != WORK_MODE_BOOT) {
		...
		if (!spinor_store_buffer) {
			printf("memory malloced fail for store buffer\n");
	...
	if (!spinor_write_cache) {
		printf("memory malloced fail for spinor data buffer\n");
...
static void spinor_enter_4bytes_addr(int enable)
{
	...
	if (spinor_4bytes_addr_mode == 1 && enable == 1) {
		printf("spinor enter 4bytes addr\n");
		command = 0xB7;
	}
	else if (spinor_4bytes_addr_mode == 1 && enable == 0) {
		printf("spinor exit 4bytes addr\n");
		command = 0xE9;
	} else {
		return ;
	}
