...
int de_fce_init(unsigned int sel, unsigned int chno, unsigned int reg_base)
{
	...
	memory = kmalloc(sizeof(__fce_reg_t), GFP_KERNEL | __GFP_ZERO);
	if (NULL == memory) {
	...
	//hist
	g_hist_status[sel][chno] = kmalloc(sizeof(__hist_status_t), GFP_KERNEL | __GFP_ZERO);	//FIXME where to FREE?
	...
	g_hist[sel][chno] = kmalloc(1024, GFP_KERNEL | __GFP_ZERO);	//FIXME where to FREE?
	...
	g_hist_p[sel][chno] = kmalloc(1024, GFP_KERNEL | __GFP_ZERO);	//FIXME where to FREE?
	...
	//ce
	g_ce_status[sel][chno] = kmalloc(sizeof(__ce_status_t), GFP_KERNEL | __GFP_ZERO);	//FIXME where to FREE?
	...
	g_celut[sel][chno] = kmalloc(256, GFP_KERNEL | __GFP_ZERO);	//FIXME where to FREE?
