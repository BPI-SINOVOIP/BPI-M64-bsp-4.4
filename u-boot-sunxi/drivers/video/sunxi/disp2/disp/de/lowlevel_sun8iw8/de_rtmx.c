...
int de_rtmx_init(unsigned int sel, unsigned int reg_base)
{
	...
	memory = kmalloc(sizeof(__glb_reg_t), GFP_KERNEL | __GFP_ZERO);
	if (NULL == memory) {
	...
	for (j = 0; j < vi_chno; j++) {
		...
		memory =
		    kmalloc(sizeof(__vi_ovl_reg_t), GFP_KERNEL | __GFP_ZERO);
		if (NULL == memory) {
	...
	memory = kmalloc(sizeof(__bld_reg_t), GFP_KERNEL | __GFP_ZERO);
	if (NULL == memory) {
