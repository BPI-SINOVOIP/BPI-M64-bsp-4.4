...
#define SPIC0_BASE                      0x01c05000
#define SPI_TX_IO_DATA                  (SPIC0_BASE + 0x200)
#define SPI_RX_IO_DATA                  (SPIC0_BASE + 0x300)

#define  NAND_DRV_VERSION_0             0x03
#define  NAND_DRV_VERSION_1             0x6019
#define  NAND_DRV_DATE                  0x20181104
#define  NAND_DRV_TIME                  0x1256
...
__u32 _Getpll6Clk(void)
{
	...
	reg_val  = get_wvalue(NAND_CLK_BASE_ADDR + 0x28);
...
__s32 _get_spic_clk_v1(__u32 spinand_index, __u32 *pdclk)
{
	...
	__u32 reg_val, sclk0;
	__u32 mclk = spi_get_mlk(spinand_index)/1000000;
//	__u32 mclk = _Getpll6Clk();

	if (spinand_index == 0) {
		sclk0_ccr_adr = (SPIC0_BASE+0x24);
	} else {
	...
	/* sclk0 */
	reg_val = get_wvalue(sclk0_ccr_adr);
	cdr_sel = (reg_val>>12) & 0x1;
	cdr1 = (reg_val>>8) & 0xf;
	cdr2 = (reg_val) & 0xff;
...
//	printf("_change_spic_clk: mclk=0x%x\n", mclk);
...
__s32 _open_spic_ahb_gate_and_reset_v1(__u32 spinand_index)
{
	...
	*/
	...
	if (spinand_index == 0) {
		// reset
		reg_val = get_wvalue(NAND_CLK_BASE_ADDR + 0x02c0);
		reg_val &= (~(0x1<<20));
		put_wvalue((NAND_CLK_BASE_ADDR + 0x02c0), reg_val);
		...
		// ahb clock gate
		reg_val = get_wvalue(NAND_CLK_BASE_ADDR + 0x60);
		reg_val |= (0x1<<20);
		put_wvalue((NAND_CLK_BASE_ADDR + 0x60), reg_val);
	} else {
...
{
	tx_dma_chan = sunxi_dma_request(0); //0: normal DMA
	if (tx_dma_chan == 0) {
...
{
	rx_dma_chan = sunxi_dma_request(0); //0: normal DMA
	if (rx_dma_chan == 0) {
		printf("uboot nand_request_tx_dma: request genernal dma failed!\n");
		return -1;
	} else
...
int spinand_dma_config_start(__u32 tx_mode, __u32 addr, __u32 length)
{
	...
	if (tx_mode) {
		dma_set.cfg.src_drq_type = DMAC_CFG_SRC_TYPE_DRAM;
		dma_set.cfg.src_addr_mode = DMAC_CFG_SRC_ADDR_TYPE_LINEAR_MODE;
		dma_set.cfg.src_burst_length = DMAC_CFG_SRC_1_BURST;
		dma_set.cfg.src_data_width = DMAC_CFG_SRC_DATA_WIDTH_32BIT;
		dma_set.cfg.reserved0 = 0;

		dma_set.cfg.dst_drq_type = DMAC_CFG_TYPE_SPI0;
		dma_set.cfg.dst_addr_mode = DMAC_CFG_DEST_ADDR_TYPE_IO_MODE;
		dma_set.cfg.dst_burst_length = DMAC_CFG_DEST_1_BURST;
		dma_set.cfg.dst_data_width = DMAC_CFG_DEST_DATA_WIDTH_32BIT;
		dma_set.cfg.reserved1 = 0;
	} else {
	...
	if (tx_mode) {
		if (sunxi_dma_setting(tx_dma_chan, &dma_set) < 0) {
			NAND_Print("uboot: sunxi_dma_setting for tx faild!\n");
			return -1;
		}
	} else {
		if (sunxi_dma_setting(rx_dma_chan, &dma_set) < 0) {
			NAND_Print("uboot: sunxi_dma_setting for rx faild!\n");
			return -1;
		}
	}
	...
	if (ret < 0) {
		NAND_Print("uboot: sunxi_dma_start for spi nand faild!\n");
		return -1;
	}
