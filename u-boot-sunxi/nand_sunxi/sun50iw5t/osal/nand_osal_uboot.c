...
__s32 NAND_AllocMemoryForDMADescs(__u32 *cpu_addr, __u32 *phy_addr)
{
	...
	if (p == NULL) {
		NAND_Print(
		    "NAND_AllocMemoryForDMADescs(): alloc dma des failed\n");
		return -1;
	} else {
		*cpu_addr = (__u32)p;
		*phy_addr = (__u32)p;
		NAND_Print_DBG(
		    "NAND_AllocMemoryForDMADescs(): cpu: 0x%x  physic: 0x%x\n",
		    *cpu_addr, *phy_addr);
	}
...
int NAND_ClkRequest(__u32 nand_index)
{
	...
	if (ndfc_version == 1) {
		...
		if (get_storage_type() == 1) {
			...
			if (ret < 0) {
				NAND_Print(
				    "NAND_ClkRequest, set dclk failed!\n");
				return -1;
			}
			...
			if (ret < 0) {
				printf(
				    "NAND_ClkRequest, set spi0 dclk failed!\n");
				return -1;
			}
		}
...
int NAND_SetClk(__u32 nand_index, __u32 nand_clk0, __u32 nand_clk1)
{
	...
	if (ndfc_version == 1) {
		...
		if (get_storage_type() == 1) {
			...
			if (ret < 0) {
				NAND_Print(
				    "NAND_SetClk, change ndfc clock failed\n");
				return -1;
			}
		} else if (get_storage_type() == 2) {
			ret =
			    _change_spic_clk_v1(nand_index, dclk_src_sel, dclk);
			if (ret < 0) {
				printf(
				    "NAND_SetClk, change spic clock failed\n");
				return -1;
			}
		}
	} else {
...
__s32 NAND_PIOFuncChange_DQSc(__u32 nand_index, __u32 en)
{
	...
	if (ndfc_version == 1) {
		NAND_Print(
		    "NAND_PIOFuncChange_EnDQScREc: invalid ndfc version!\n");
		return 0;
	}
...
void *NAND_Malloc(unsigned int Size)
{
	...
	/*buf = malloc(Size);*/
	buf = malloc_align(Size,
			   64); /*songwj For A50, Buffer Must 64 Bytes align*/
	if (buf == NULL)
...
int nand_request_dma(void)
{
	...
	if (dma_chan == 0) {
		NAND_Print(
		    "uboot nand_request_dma: request genernal dma failed!\n");
		return -1;
	} else
		NAND_Print_DBG("uboot nand_request_dma: reqest genernal dma "
			       "for nand success, 0x%x\n",
			       dma_chan);
...
int spinand_dma_config_start(__u32 tx_mode, __u32 addr, __u32 length)
{
	...
	if (tx_mode) {
		...
		ret = sunxi_dma_start(tx_dma_chan, addr, (__u32)SPI_TX_IO_DATA,
				      length);
	} else {
		flush_cache((uint)addr, length);
		ret = sunxi_dma_start(rx_dma_chan, (__u32)SPI_RX_IO_DATA, addr,
				      length);
	}
	if (ret < 0) {
...
__u32 NAND_GetNandIDNumCtrl(void)
{
	...
	ret = script_parser_fetch("nand0_para", "nand0_id_number_ctl",
				  &id_number_ctl, 1);
	if (ret < 0) {
...
__u32 NAND_GetNandCapacityLevel(void)
{
	...
	ret = script_parser_fetch("nand0_para", "nand0_capacity_level",
				  &CapacityLevel, 1);
	if (ret < 0) {
