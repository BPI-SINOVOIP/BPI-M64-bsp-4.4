...
#define  NAND_DRV_VERSION_1		0x6005
#define  NAND_DRV_DATE			0x20170624
...
__s32 _change_ndfc_clk_v1(__u32 nand_index, __u32 dclk_src_sel, __u32 dclk)
{
	...
	if (nand_index == 0) {
		...
		NAND_Print("_change_ndfc_clk error, wrong nand index: %d\n", nand_index);
		return -1;
	}
...
__s32 _close_ndfc_clk_v1(__u32 nand_index)
{
	...
	if (nand_index == 0) {
		...
		NAND_Print("close_ndfc_clk error, wrong nand index: %d\n", nand_index);
		return -1;
	}
...
//		*(volatile __u32 *)(0x01c20000 + 0x2c0) = reg_val;
		...
		NAND_Print("open ahb gate and reset, wrong nand index: %d\n", nand_index);
		return -1;
	}
...
__s32 _cfg_ndfc_gpio_v1(__u32 nand_index)
{
	if (nand_index == 0) {
		...
		*(volatile __u32 *)(0x01c20800 + 0x68) = 0x00001555;
		NAND_Print_DBG("NAND_PIORequest, nand_index: 0x%x\n", nand_index);
		...
		NAND_Print_DBG("Reg 0x01c20854: 0x%x\n", *(volatile __u32 *)(0x01c20854));
	} else {
		NAND_Print("_cfg_ndfc_gpio_v1, wrong nand index %d\n", nand_index);
		return -1;
	}
